...
module Language.PlutusCore.Merkle.Constant.Typed
    ( TypeScheme (..)
    , TypedBuiltinName (..)
...
type Evaluator f m = DynamicBuiltinNameMeanings -> f TyName Name Integer -> m EvaluationResultDef
...
see the "Language.PlutusCore.Merkle.Constant.Dynamic.Instances" module).
-}
...
evaluate built-in applications. The type of evaluators is this then:
    ...
    type Evaluator f m = DynamicBuiltinNameMeanings -> f TyName Name Integer -> m EvaluationResult
...
class KnownType a where
    ...
    toTypeAst :: proxy a -> Type TyName Integer

    -- | Convert a Haskell value to the corresponding PLC value.
    makeKnown :: a -> Term TyName Name Integer

    -- See Note [Evaluators].
    -- | Convert a PLC value to the corresponding Haskell value using an explicit evaluator.
    readKnown :: Monad m => Evaluator Term m -> Term TyName Name Integer -> ReflectT m a
...
readKnownM
    :: (Monad m, KnownType a)
    => Term TyName Name Integer -> EvaluateT ReflectT m a
...
newtype OpaqueTerm (text :: Symbol) (unique :: Nat) = OpaqueTerm
    { unOpaqueTerm :: Term TyName Name Integer
    }

instance Pretty (OpaqueTerm text unique) where
    pretty = pretty . unOpaqueTerm

-- Encode '(-1)' from Haskell as @all r. r -> r@ from PLC.
...
instance KnownType () where
    ...
    -- evaluated and we use 'unsafePerformIO' for logging, so we want to compute the '(-1)' just
