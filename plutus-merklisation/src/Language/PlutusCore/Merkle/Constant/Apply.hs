...
module Language.PlutusCore.Merkle.Constant.Apply
    ( ConstAppError (..)
    , ConstAppResult (..)
    , ConstAppResultDef
    , EvaluateConstApp
...
import           Language.PlutusCore.Merkle.Constant.Name
...
data ConstAppError
    = ExcessArgumentsConstAppError [Value TyName Name Integer]
    ...
    | NonConstantConstAppError (Value TyName Name Integer)
    | UnreadableBuiltinConstAppError (Value TyName Name Integer) Text
...
type ConstAppResultDef = ConstAppResult (Term TyName Name Integer)

instance PrettyBy config (Term TyName Name Integer) => PrettyBy config ConstAppError where
...
instance ( PrettyBy config (Value TyName Name Integer)
         , PrettyBy config a
...
-- a 'Value'.
type EvaluateConstAppDef m = EvaluateConstApp m (Value TyName Name Integer)
...
extractBuiltin :: (Monad m, KnownType a) => Value TyName Name Integer -> EvaluateConstApp m a
...
applyTypeSchemed
    :: Monad m => TypeScheme a r -> a -> [Value TyName Name Integer] -> EvaluateConstAppDef m
applyTypeSchemed = go where
    go
        :: Monad m
        => TypeScheme a r
        -> a
        -> [Value TyName Name Integer]
...
applyTypedBuiltinName
    :: Monad m => TypedBuiltinName a r -> a -> [Value TyName Name Integer] -> EvaluateConstAppDef m
...
applyBuiltinName
    :: Monad m => BuiltinName -> [Value TyName Name Integer] -> EvaluateConstAppDef m
...
-- given evaluator.
runApplyBuiltinName
    :: Monad m => Evaluator Term m -> BuiltinName -> [Value TyName Name Integer] -> m ConstAppResultDef
