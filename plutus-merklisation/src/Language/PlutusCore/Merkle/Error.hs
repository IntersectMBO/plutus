...
module Language.PlutusCore.Merkle.Error
    ( ParseError (..)
    , AsParseError (..)
...
data ParseError ann
    = LexErr String
    | UnexpectedToken -- Used to refer to PLC.Lexer.Type
...
data UniqueError ann
    = MultiplyDefined PLC.Unique ann ann
    | IncoherentUsage PLC.Unique ann ann
    | FreeVariable PLC.Unique ann
    deriving (Show, Eq, Generic, NFData)
...
newtype UnknownDynamicBuiltinNameError
    = UnknownDynamicBuiltinNameErrorE PLC.DynamicBuiltinName
...
data InternalTypeError ann
    = OpenTypeOfBuiltin (Type PLC.TyName ()) (Builtin ())
    deriving (Show, Eq, Generic, NFData)
makeClassyPrisms ''InternalTypeError

data TypeError ann
    = KindMismatch ann (Type PLC.TyName ()) (Kind ()) (Kind ())
    | TypeMismatch ann
        (Term PLC.TyName PLC.Name ())
        (Type PLC.TyName ())
        (Normalized (Type PLC.TyName ()))
    | UnknownDynamicBuiltinName ann UnknownDynamicBuiltinNameError
    | InternalTypeErrorE ann (InternalTypeError ann)
    | FreeTypeVariableE (PLC.TyName ann)
    | FreeVariableE (PLC.Name ann)
    | OutOfGas
    deriving (Show, Eq, Generic, NFData)
...
data Error ann
    ...
    | ValueRestrictionErrorE (ValueRestrictionError PLC.TyName ann)
    | UniqueCoherencyErrorE (UniqueError ann)
    | TypeErrorE (TypeError ann)
    | NormCheckErrorE (NormCheckError PLC.TyName PLC.Name ann)
    deriving (Show, Eq, Generic, NFData)
...
instance tyname ~ PLC.TyName => AsValueRestrictionError (Error ann) tyname ann where
...
instance (tyname ~ PLC.TyName, name ~ PLC.Name) =>
...
instance Pretty ann => Pretty (ParseError ann) where
    ...
    pretty (UnexpectedToken)  = "Unexpected token"  -- <+> squotes (pretty t) <+> "at" <+> pretty (loc t)
