...
module Language.PlutusCore.Merkle.Type
    ( Term (..)
    , termSubterms
    , termSubtypes
    , termVars
...
instance Recursive (Type tyname ann) where
    ...
    project (TyPruned ann h)       = TyPrunedF ann h
...
instance Corecursive (Type tyname ann) where
    ...
    embed (TyPrunedF ann h)       = TyPruned ann h
