...
- Open the first PR against master, the second PR against the first PR, and so on.
...
The first two options are often referred to as "trunk-based development", while the third "long-lived feature branches".
...
Rebasing and force pushing to other branches you own is fine.
...
That being said, there are not many cases where "rebase and merge" is appropriate, you might as well rebase it yourself.
...
- Historically, plutus master rarely breaks.
