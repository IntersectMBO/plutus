...
import           Language.Haskell.Interpreter (CompilationError (CompilationError, RawError), column, filename, row,
                                               text)
import           Playground.API               (SimpleArgumentSchema (SimpleIntSchema, SimpleObjectSchema, SimpleStringSchema),
...
toSimpleArgumentSchemaSpec =
    describe "toSimpleArgumentSchema" $ do
        it "should convert a simple integer argument" $
            toSimpleArgumentSchema integerSchema `shouldBe` SimpleIntSchema
        it "should convert a simple string argument" $
            toSimpleArgumentSchema stringSchema `shouldBe` SimpleStringSchema
        it "should convert an object argument" $
            toSimpleArgumentSchema objectSchema `shouldBe`
            SimpleObjectSchema
                [ ( "vestingTranche1"
                  , SimpleObjectSchema
                        [ ("vestingTrancheDate", SimpleIntSchema)
                        , ("vestingTrancheAmount", SimpleIntSchema)
                        ])
                , ( "vestingTranche2"
                  , SimpleObjectSchema
                        [ ("vestingTrancheDate", SimpleIntSchema)
                ...
                , ( "vestingOwner"
                  , SimpleObjectSchema [("getPubKey", SimpleIntSchema)])
                ]
...
objectSchema =
    Schema
        ...
        , _schemaDescription = Nothing
        , _schemaRequired =
              ["vestingTranche1", "vestingTranche2", "vestingOwner"]
        , _schemaAllOf = Nothing
        , _schemaProperties =
              fromList
                  [ ( "vestingTranche1"
