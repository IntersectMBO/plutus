...
import           Ledger                    (Address, DataScript (DataScript), PubKey, RedeemerScript (RedeemerScript),
                                            TxId, ValidatorScript (ValidatorScript), applyScript, compileScript, hashTx,
                                            lifted, scriptAddress)
import           Ledger.Slot               (Slot, SlotRange)
import qualified Ledger.Slot               as Slot
import           Ledger.Validation         (PendingTx, pendingTxValidRange, valueSpent)
...
------------------------------------------------------------
data ScheduleCollectionArguments = ScheduleCollectionArguments
  { scheduleCollectionDeadline           :: Slot
...
data MutationAPI m =
    MutationAPI
        { mutationAPIScheduleCollection :: Resolver m MUTATION ScheduleCollectionArguments Bool
...
rootResolver =
    GQLRootResolver
        ...
        , mutationResolver =
              MutationAPI
                  { mutationAPIScheduleCollection = liftUnitResolver (\ScheduleCollectionArguments {..} -> scheduleCollection scheduleCollectionDeadline scheduleCollectionTarget scheduleCollectionCollectionDeadline scheduleCollectionOwnerWallet)
                  , mutationAPIContribute = liftUnitResolver (\ContributeArguments {..} -> contribute contributeDeadline contributeTarget contributeCollectionDeadline contributeOwnerWallet contributeValue)
