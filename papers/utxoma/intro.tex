...
%We observe that the \UTXO\ ledger rules do not fundamentally depend on the fact that an output locks only a single integral value. Instead, all they require is that locked values follow the algebraic structure of a monoid. We exploit that insight by generalising the notion of locked values to cryptocurrency \emph{bundles.} Furthermore, we extend transactions by a \emph{mint} field, where user-defined tokens can be minted (or minted) and destroyed. That process is controlled by smart contract scripts called \emph{minting policies.}
