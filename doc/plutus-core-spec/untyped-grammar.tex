...
then $\kase{i}{M_1 \ldots M_5}$ will cause the term $M_{i+1}$ to be
evaluated if $i$ lies between 0 and 4, with \texttt{(error)} occurring if $i<0$
or $i>4$.  See Section~\ref{sec:cardano-builtins} for details of the types which
can currently be used with \texttt{case} on the Cardano blockchain.
