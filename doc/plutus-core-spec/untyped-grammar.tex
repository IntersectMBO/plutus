...
\paragraph{Scoping.} We do not require the body of a Plutus Core program to be
closed or well scoped; terms may contain free variables and may contain
unresolvable variable occurrences. Name resolution is lexical: a variable
occurrence refers to the \textit{most recent} (ie, innermost) suitable binding
introduced by an enclosing \texttt{lam}. If evaluation encounters a variable for
which no binding is available in the current environment, it halts with an
error (see Figure~\ref{fig:untyped-cek-transitions}). No pre-execution scope
checking is mandated by this specification.
