...
\begin{minipage}{\linewidth}
    ...
    \textrm{Term}       & L,M,N  & ::= & x                               & \textrm{variable}\\
                        ...
                        &        &     & \kase{M}{N_1 \ldots N_m}        & \textrm{case analysis with $m$ alternatives ($m \geq 0$)}\\
                        &        &     & \errorU                         & \textrm{error}\\
...
value, and then applies that to the arguments in the value. Note that
$\keyword{case}$ does \emph{not} strictly evaluate the case branches, only
applying (and hence evaluating) the one that is eventually selected.  The list
of branches in $\keyword{case}$ is allowed to be empty, but in that case there
will be an error if it is ever applied to a scrutinee.
