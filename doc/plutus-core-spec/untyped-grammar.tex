...
An application of a term $M$ to a term $N$ is represented by $\appU{M}{N}$. We
may occasionally write $\appU{M}{N_1 \ldots N_k}$ or $\appU{M}{\repetition{N}}$
as an abbreviation for an iterated application
$\mathtt{[}\ldots\mathtt{[[}M\;N_1\mathtt{]}\;N_2\mathtt{]}\ldots $ %% Try to avoid a bad line break 
$N_k\mathtt{]}$, and tools may also use this as
concrete syntax.  If the sequence ${N_1 \ldots N_k}$ is empty (which will happen
when $k=0$, for instance) then $\appU{M}{\repetition{N}}$ is just $M$.
...
See Section~\ref{sec:cardano-builtins} for a description of the types and
functions which have already been deployed on the Cardano blockchain (or will be
in the near future).

\paragraph{Casing on built-in types.}  Some built-in types
support branching using \texttt{case}.  For example, with Cardano's default
universe, if $i$ is a variable bound to a value of the \texttt{integer} type
then $\kase{i}{M_1 \ldots M_5}$ will cause the term $M_{i+1}$ to be evaluated if
$i$ lies between 0 and 4, with \texttt{(error)} occurring if $i<0$ or $i>4$.
See Section~\ref{sec:builtin-casing} for details and
Section~\ref{sec:cardano-builtins}
(Table~\ref{table:builtin-matching-functions}) for the types which can currently
be used with \texttt{case} on the Cardano blockchain and a specification of how
\texttt{case} works for them.
