...
\noindent We define auxiliary functions $\etype: \Uni \rightharpoonup \N^*$ and
$\dtype: \N^* \rightharpoonup \N^* \times \Uni$. Both $\etype$ and $\dtype$ are
partial and $\Uni$ denotes the universe of types defined in
Sections~\ref{sec:default-builtins-1}, \ref{sec:default-builtins-2},
and~\ref{sec:default-builtins-3}.
...
$\uparrow$ operation, which fails if the currency identifiers or the token
identifiers for a given currency are not in strictly ascending order; it also
fails if it encounters a bytestring with length greater than 32 or an integer
value which is zero or lies outside the range $[-2^{127}, 2^{127}-1]$.
