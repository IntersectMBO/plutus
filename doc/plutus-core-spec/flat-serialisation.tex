...
sequences of 7-bit blocks, then emitting all but last chunks as a list with
the \textbf{least significant block first}. The last chunk(most significant chunk)
will be encoded as regular 7-bit block and appended right after the list of chunks.

$$
\E_{\N} (s, \sum_{i=0}^{n-1}k_i2^{7i}) = \E_7(\Elist_7(s, [k_0, \ldots, k_{n-2}]), k_{n-1})
$$
\noindent(where $k_i \in \Z$ and $0 \leq k_i \leq 127$).
\noindent The decoder is
$$
\D_{\N}(s) = (s'', \sum_{i=0}^{n-1}k_i2^{7i}) \quad \text{if $\Dlist_7(s) = (s', [k_0, \ldots, k_{n-2}])$ and $\D_7(s') = (s'', k_{n-1})$}.
$$
