...
encoders and decoders for names. For example, for textual names one could use
$\Ebinder = \Ename = \E_{\U^*}$ and $\Dbinder = \Dname = \D_{\U^*}$.

\section{Cardano-specific serialisation issues}
\label{sec:cardano-issues}
\subsection{Scope checking}
No scope checking is required prior to evaluation. Programs need not be
well-scoped; evaluation proceeds according to the CEK semantics, and an
unbound variable encountered at run time causes an immediate error
(Figure~\ref{fig:untyped-cek-transitions}).
