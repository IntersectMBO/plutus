...
One can write \texttt{(case $M$ $M_1$ \ldots $M_n$)} where $M$ evaluates to a
value of some built-in type $T$, and one of the terms $M_i$ will be evaluated,
perhaps applied to some built-in values which will generally be obtained from
$x$.  For example, \texttt{(case (con integer $k$) $M_1$ \ldots $M_n$)} will
evaluate $M_{k+1}$ if $0 \leq k \leq n-1$ and will evaluate to \texttt{(error)}
otherwise.  If $x$ is \texttt{(con (pair boolean integer) (False, 5))} then
\texttt{(case $x$ $M_1$)} will cause the application \texttt{[[$M_1$ False] 5]}
to be evaluated (possibly failing if $M_1$ is of the wrong type).  For pairs,
exactly one branch is required, with \texttt{(error)} occurring otherwise.  In
general the number of branches for a particular type is variable, with an error
occurring if \texttt{case} would cause a non-existent branch to be selected.

\medskip
To specify this behaviour in general we require some more notation.  For each
built-in type $T \in \Uni$ we assume that there is a \textit{casing function}
$$
...
WHAT ABOUT 64-BIT BRANCH NUMBERS?
...
evaluated with the arguments $\reify{v_1}, \ldots, \reify{v_k}$.  The case $k=0$ is allowed, in
which case the expression $M_i$ is evaluated as normal.
