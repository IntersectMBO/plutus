...
$$
\match{T}: \denote{T} \rightarrow \N \times \denote{\Uni}^*
$$
and a \textit{branch bound} $\maxbranch(T) \in \N \cup \{\infty\}$.
...
Given a built-in type $T$ and $x \in \denote{T}$, $\match{T}(x)$ returns a pair
$(i, (v_1, \ldots, v_k))$ with $i$ a natural number and the $v_i$ built-in values
in $\Uni$. If
$M$ is a term which evaluates to the constant \texttt{(con $T$ $x$)} then
the expression
$$
