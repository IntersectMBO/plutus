...
built-in type $T \in \Uni$ we assume that there is a \textit{matching function}
$$
\match{T}: \denote{T} \rightarrow \N \times \denote{\Uni}^* \disj \{\errorX\}
$$
and a \textit{branch bound} $\maxbranch(T) \in \N \disj \{\infty\}$.
\nomenclature[Dz7]{$\match{T}$}{Matching function for a built-in type $T$}
\nomenclature[Dz8]{$\maxbranch(T)$}{Maximum number of branches in a \texttt{case} expression on a built-in type $T$}
...
natural number and the $v_i$ built-in values in $\Uni$. If $M$ is a term which
