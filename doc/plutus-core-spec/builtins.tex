...
\subsubsection{Casing on built-in types}
...
value $x$ of some built-in type $T$, and one of the terms $M_i$ will be
evaluated, perhaps applied to some built-in values which will generally be
obtained from $x$.  For example, $\kase{\con{\ty{integer}}{k}}{M_1 \ldots
...
\texttt{(error)} otherwise.  If $x$ is \texttt{(con (pair boolean integer)
  (False, 5))} then $\kase{x}{M_1}$ will cause the application
\texttt{[[$M_1$ (con boolean False)] (con integer 5)]} to be evaluated (possibly
failing if $M_1$ is of the wrong type).  For pairs, exactly one branch is
required, with \texttt{(error)} occurring otherwise.  In general the number of
branches for a particular type is variable, with an error occurring if
\texttt{case} would cause a non-existent branch to be selected.
...
natural number and the $v_i$ built-in values in $\denote{\Uni}$. If $M$ is a term which
evaluates to the constant \texttt{(con $T$ $x$)} then the expression
$$
