...
\begin{align*}
    ...
    \Sext(\star) &= \star.
\end{align*}%
\nomenclature[Dr4]{$\Sext$}{The extension of a type assignment $S$ to $\Unihash \cup \VarStar$}
...
\noindent It is also convenient to let $\denote{\Inputs} = \Inputs$ and define both
  ...
  function, and similarly for $\Inputs^{+}$.
...
Given a builtin $b$ with signature $[\iota_1, \ldots, \iota_n] \rightarrow
\omega$, the symbol following the arrow indicates the return type of the
function $b$s.  All built-in functions conceptually return a non-empty list of
inputs, and we will denote such a list by a symbol of the form $(v|v_1, \ldots,
v_k)$ with $v, v_i \in \Inputs$ and $k \geq 0$, the case $k=0$ indicating a list
$(v|)$ with a single entry.  The majority of builtins return a single value, and
to simplify notation we will identify $\Inputs$ with $\{(v|): v \in \Inputs\}
\subseteq \Inputs^{+}$.  If $\omega \in \Unihash$ then $b$ returns a single
value of some built-in type; if $\omega = \star$ then $b$ returns a single
element of $\Inputs$ (probably not of a built-in type), and if $\omega = \ap$
then $b$ returns a sequence of values $(v|v_1, \ldots, v_k)$ with $k>0$.  The
intention is that this will immediately be interpreted as an application $v \;v_1\;
\ldots\; v_k$ in the ambient language (eg, typed or untyped Plutus Core); the number
of arguments $k$ may depend on the values of the inputs to the function, as is
the case for the $\texttt{caseList}$ and $\texttt{caseData}$ functions described
in Appendix~\ref{sec:default-builtins-6}.
...
\noindent where
$$
\denote{\star} = \Inputs \quad\text{and}\quad \denote{\ap} = \Inputs^{+}.
$$
...
three possibilities:
\begin{enumerate}
\item $r \in \denote{\tn} \enspace \text{for some $\tn \in \Uni$}$.  \red{Really?}
\item $r \in \Inputs^{+}$ (remember that we identify $\Inputs$ with the subset of length-one elements of $\Inputs^{+}$).
\item $r = \errorX$.
\end{enumerate}
In other words,
$$
...
$$
which converts results of built-in functions back into inputs \red{plus apps} (or the $\errorX$ symbol):
\begin{enumerate}
\item If $r \in \denote{\tn}$, then $\reify{r} = \reify{r}_{\tn} \in \Con{\tn} \subseteq \Inputs$.
%% \item If $r \in \Inputs$ then  $\reify{r} = r$.
\item If $r = (v|v_1, \ldots, v_k) \in \Inputs^{+}$ then  $\reify{r} = r$. \red{Or $(\reify{v}|\reify{v_1}, \ldots,\reify{v_k})$?}
...
\red{I'm worrying about what happens if we return an $\ap$ that contains
  built-in constants, not terms.  Maybe that's OK: it's handed over to the
  application mechanism in the ambient language and that should do the right
  thing.}
...
types; other arguments (occurring as $\star$ in $\sigmabar(b)$) are treated opaquely,
...
the function. \red{Or if we've got an $\ap$, its head must have been an argument.}
...
\noindent\red{Do we ever say what ``\#-polymorphic'' means?}

\noindent\red{Do we really use $\tau$ consistently?}
...
Section~\ref{sec:builtin-denotations}.  \red{In fact, $\Eval$ now takes us into
  $\Inputs \disj \Inputs{+}$ and the evaluator has to deal with deferred
  applications. We have to not identify $\Inputs$ with a subset of $\Inputs^{+}$
  because we treat $v$ differently from $(v|)$ -- or do we?  Maybe it's safe to do the identification.}
