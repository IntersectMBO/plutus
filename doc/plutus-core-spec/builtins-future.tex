...
> \tilde{b}$.  We extend this terminology to $\Fqq$ by saying that $a+bu$ is
larger than $c+du$ if either $b$ is larger than $d$ or $b=d$ and $a$ is larger
than $c$.  \red{Is this the right way round? Ie, we compare the $u$-parts first?}
...
\item Otherwise, let $x = \pi(c) \in \Fq$ and let $z = x^3+4$. If $z$ is not a
