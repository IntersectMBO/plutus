...
\noindent Here $G_1$ and  $G_2$ are both cyclic groups of prime order $r$, where 
$$
r = \mathtt{0x73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000001}.
$$
        
\noindent To define these groups we need the finite field $\Fq$ where
\begin{align*}
...
is larger than $c$.  \red{Is this the right way round? Ie, we compare the
$u$-parts first?}
...
$G_1$ with distinct $y$ coordinates, one the negative of the other.  We denote
the smaller of the possible $y$ coordinates by $\ymin(x)$ and the larger by
$\ymax(x)$.

\item For $(x,y) \in G_1\backslash \mathcal{O}$ we define
$$
...
Note that in all cases the most significant bit of a compressed point is 1.
...
The two $\uncompress$ functions are obtained by inverting the process described
in Note~\ref{note:group-compression}.

\paragraph{$G_1$ uncompression.}  Given a bytestring $b$, it is checked that
$b$ contains exactly 48 bytes.  If not, then $\uncompress(b) = \errorX$ (ie,
...
\item If $b_{382} = 1$ then
$\uncompress_{G_1}(b) =
\begin{cases}
\mathcal{O}_{G_1} & \text{if $b_{381} = b_{380} = \cdots = b_0 = 0$}\\
\errorX & \text{otherwise}.
\end{cases}$
\item If $b_{382}=0$, set $c=\sum_{i=0}^{380}b_i2^i \in \N$.
\begin{itemize}
...
$b$ contains exactly 96 bytes.  If not, then $\uncompress(b) = \errorX$ (ie,
...
\errorX & \text{otherwise}.
\end{cases}$
\item If $b_{766} = 0$, set $c=\sum_{i=0}^{383}b_i2^i$ and $d=\sum_{i=384}^{764}b_i2^{i-384} \in \N$.
\begin{itemize}
