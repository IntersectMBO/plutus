...
\noindent\textbf{Note.} The \texttt{Array} constructor will not be available until
a future protocol version. Prior to that, the \texttt{data} type contains only
the first five constructors: \texttt{Constr}, \texttt{Map}, \texttt{List},
\texttt{I}, and \texttt{B}.
...
X^* \disj \denote{\ty{integer}} \disj \denote{\ty{bytestring}} \disj X^*$, so that
...
\begin{align*}
  ...
  \inj_A: \denote{\ty{data}}^* & \to \denote{\ty{data}} \\
\end{align*}
...
\begin{align*}
  ...
  \proj_A: \denote{\ty{data}} & \to \withError{\denote{\ty{data}}^* }\\
\end{align*}
...
$$
\is_C, \is_M, \is_L, \is_I, \is_B, \is_A: \denote{\ty{data}} \to \denote{\ty{bool}}
$$
...
\begin{minipage}{0.6\linewidth}
    ...
    \syn{data} & ::= & \texttt{(Constr} \ \syn{integer} \ \syn{list(data)} \texttt{)}\\
               ...
               &  & \texttt{(Array} \ \syn{list(data)} \texttt{)}.
    \end{array}\]
...
\begin{longtable}[H]{|l|p{5cm}|p{5.5cm}|c|c|}
    ...
    \TT{chooseData}               & $[\forall\star, \ty{data}, \star, \star, \star, \star, \star, \star] \to \star$
    & $ (d,t_C, t_M, t_L, t_A, t_I, t_B) $
    \smallskip
    ...
    \TT{mapData}                  & $[\listOf{\pairOf{\ty{data}}{\ty{data}}}] \to \ty{data}$     & $\inj_M$ & No & \\[2mm]
