...
From Protocol Version 12 onwards, Plutus Core supports casing on some of the
built-in types described in Tables~\ref{table:built-in-types-1} and
\ref{table:built-in-type-operators-1}.  See Table~\ref{table:builtin-casing-functions}
and Section~\ref{sec:builtin-casing} for details.  Casing is not available for
types not mentioned in this table, and for such types $T$ one should assume that
$\maxbranch(T) = 0$ and $\match{T}(x) = (0,())$: the semantics of Plutus Core as
given in Figure~\ref{fig:untyped-term-reduction} and implemented in the CEK
machine (Figure~\ref{fig:untyped-cek-machine}) will ensure that this
...
% pair
        ...
        \hline
        \end{tabular}
   \caption{Casing functions for built-in types}
    \label{table:builtin-casing-functions}
\end{table}
}
