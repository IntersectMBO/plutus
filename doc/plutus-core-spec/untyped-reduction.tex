...
\medskip
\noindent Recall also that $\appU{M}{N_1 \ldots N_k}$ is an abbreviation for
iterated application
$\mathtt{[}\ldots\mathtt{[[}M\;N_1\mathtt{]}\;N_2\mathtt{]}\ldots N_k\mathtt{]}$
and that $\appU{M}{N_1 \ldots N_k}= M$ if $k=0$.
   ...
   % [(lam x M) V] -> [V/x]M
    ...
    \begin{prooftree}
      ...
      \AxiomC{$\nextArg(A) \in \Unihash \cup \VarStar$}
        ...
        \BinaryInfC{$\step{\app{A}{V}}{\EvalBuiltin'(\beta(A), \args(A)\snoc V)}$}
    \end{prooftree}
    ...
    \begin{prooftree}
        ...
        \UnaryInfC{$\step{\kase{\constr{i}{\repetition{V}}}{M_1 \ldots M_n}}{\app{M_{i+1}}{\repetition{V}}}$}
    \end{prooftree}

    % case (constr i vs) cs -> (error) if i is out of range
    \begin{prooftree}
      \AxiomC{$i<0\enspace \text{or}\enspace i > n-1$}
        % \RightLabel{\textsf{case-of-constr-error}}
        \UnaryInfC{$\step{\kase{\constr{i}{\repetition{V}}}{M_1 \ldots M_n}}{\errorU}$}
    \end{prooftree}

    % case (con T c) cs -> c_i, depending on \match{T}(c)
    \begin{prooftree}
      \AxiomC{$n \leq \maxbranch(T)$}
    ...
    % case (con T c) cs -> (error) if too many branches.
    \begin{prooftree}
      \AxiomC{$n > \maxbranch(T)$}
        % \RightLabel{\textsf{case-of-constr-err-1}}
        \UnaryInfC{$\step{\kase{\con{T}{c}}{M_1 \ldots M_n}}{\errorU}$}
    \end{prooftree}

    % case (con T c) -> (error) if matching fails
    \begin{prooftree}
      \AxiomC{$n \leq \maxbranch(T)$}
      \AxiomC{$\match{T}(c) = \errorX$}
        % \RightLabel{\textsf{case-of-constr-err-2}}
        \BinaryInfC{$\step{\kase{\con{T}{c}}{M_1 \ldots M_n}}{\errorU}$}
    \end{prooftree}

    % case (con T c) -> (error) if matching returns an index out of bounds
    \begin{prooftree}
      \AxiomC{$n \leq \maxbranch(T)$}
      \AxiomC{$\match{T}(c) = (i,(v_1, \ldots, v_k))$}
      \AxiomC{$i<1\enspace \text{or}\enspace i>n$}
        % \RightLabel{\textsf{case-of-constr-err-3}}
        \TrinaryInfC{$\step{\kase{\con{T}{c}}{M_1 \ldots M_n}}{\errorU}$}
    \end{prooftree}
...
\begin{subfigure}[c]{\linewidth}
    ...
    \begin{prooftree}
      ...
      \AxiomC{$\nextArg(A) \in \QVar$}
        ...
        \BinaryInfC{$\step{\force{A}}{\EvalBuiltin'(\beta(A), \args(A))}$}
    \end{prooftree}
...
% \\
    \caption{Reduction via contextual semantics (continued)}
    \label{fig:untyped-reduction-continued}
\end{subfigure}
...
\begin{subfigure}[c]{\linewidth}
  $$
  \EvalBuiltin'(b, [V_1, \ldots, V_n]) =
  \begin{cases}
    \errorU  & \text{if $\EvalBuiltin(b,[V_1, \ldots, V_n]) = \errorX$}\\
    \EvalBuiltin(b,[V_1, \ldots, V_n]) &\text{otherwise}.
  \end{cases}
  $$
...
\caption[]{Term reduction for Plutus Core (continued)}
\end{figure}
