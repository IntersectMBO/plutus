...
\inAppRightFrame{\VBuiltin{b}{\repetition{V}}{[\iota]}} \cons s  & \return V &~\mapsto~&
                         \EvalBuiltinCEK\,(s, b, \repetition{V}\snoc V) \enskip \text{if $\iota \in \Unihash \cup \VarStar$}\\
...
\inForceFrame{} \cons s & \return \VBuiltin{b}{\repetition{V}}{[\iota]}   &~\mapsto~&
                         \EvalBuiltinCEK\,(s, b, \repetition{V}) \enskip \text{if $\iota \in \QVar$}\\
...
\inCaseFrame{(M_1 \ldots M_n, \rho)} \cons s & \return \VConstr{i}{V_1 \ldots V_m}   &~\mapsto~&
                         \inAppLeftFrame{V_1} \cons \cdots \cons \inAppLeftFrame{V_m} \cons s ;\rho \compute M_{i+1}
                         \enskip \text{if $0 \leq i \leq n-1$} \\
\inCaseFrame{(M_1 \ldots M_n, \rho)} \cons s & \return \VConstr{i}{V_1 \ldots V_m}   &~\mapsto~&
                         \cekerror
                         \enskip \text{if $i<0$ or $i > n-1$} \\
\inCaseFrame{(M_1 \ldots M_n, \rho)} \cons s & \return \VCon{T}{c} &~\mapsto~& \CaseCEK(s,\rho, T, c, (M_1, \ldots, M_n))
\end{alignat*}
\end{minipage}
...
\bigskip
  \begin{subfigure}[c]{\linewidth}
   $$ \EvalBuiltinCEK(s, b, [V_1, \ldots, V_n]) =
   \begin{cases}
      \cekerror  & \text{if $\EvalBuiltin\,(b,[V_1, \ldots, V_n]) = \errorX$}\\
      s \return \EvalBuiltin\,(b,[V_1, \ldots, V_n]) & \text{otherwise}
   \end{cases}
   $$
...
% The figure is too long to fit on a single page, so it's split into two parts
% here.  \ContinuedFloat makes the second part keep the same figure number and
% continue the subfigure lettering.
\begin{figure}[H]\ContinuedFloat
  \begin{subfigure}[c]{\linewidth}
   $$ \CaseCEK(s,\rho, T, c, (M_1, \ldots, M_n)) =
    \begin{cases}
      \inAppLeftFrame{\reify{v_1}} \cons \cdots \cons \inAppLeftFrame{\reify{v_m}} \cons s ;\rho \compute M_{i}
      & \text{if\;
        $
        \begin{cases}
          n \leq \maxbranch(T) \\
          \match{T}(c) = (i,(v_1, \ldots, v_m)) \\
          1 \leq i \leq n
        \end{cases}
      $}\\
      \cekerror  & \text{otherwise}.  % This includes the case when \match{T}(c) = \errorX
   \end{cases}
   $$
   \caption{Casing on built-in constants}
   \label{fig:untyped-cek-case-const}
\end{subfigure}
...
of ways.  For example, when casing on a constant of a built-in type one frame
is pushed onto the stack for each of the values $\reify{V_1}, \ldots,
\reify{V_m}$ and these then have to be popped off and processed one by one. It
would be more efficient to have a single frame which contains all of the values,
and in fact this optimisation is implemented in Cardano's production CEK
machine.%
\footnote{This is implemented in Haskell: see
\url{https://github.com/IntersectMBO/plutus}, and specifically
\url{https://github.com/IntersectMBO/plutus/tree/master/plutus-core/untyped-plutus-core/src/UntypedPlutusCore/Evaluation/Machine}.}
However we have chosen to keep the evaluator in the specification minimal
because optimisations are typically implementation-dependent and may or may not
be sensible depending on the implementation language and implementation strategy
and we do not wish to impose unsuitable optimisations on other implementers.
Using an un-optimised machine also simplifies specification and formalisation;
optimisations can then be proved to be correct in the sense that it should be
possible to translate constructs in an optimised machine into constructs in the
basic machine and show that the translation respects the semantics of the
language.
