...
\section{Plutus V3 Built-in Types and Functions}
\label{appendix:default-builtins-V3}
Plutus V3 (not yet released on Cardano) will add support for
\begin{itemize}
\item The \texttt{Blake2b-224} and \texttt{Keccak-256} hash functions (Section \ref{sec:V3-misc-builtins})
...
(see~\cite{CIP-0381}, \cite{BLS12-381}, \cite[4.2.1]{IETF-pairing-friendly-curves}, \cite{BLST-library}).
 For clarity these are described separately in Sections~\ref{sec:V3-bls-types} and \ref{sec:V3-bls-builtins}.
\end{itemize}

\noindent In addition there is a slight change to the semantics of \texttt{consByteString}.

\medskip
\noindent It is expected that support for language version 1.1.0 of
Plutus Core will become available at the same time as Plutus V3.

\subsection{Miscellaneous built-in functions}
\label{sec:V3-misc-builtins}
...
%%    \caption[]{Built-in Functions}
    ...
    \label{table:V3-misc-built-in-functions}
    \endlastfoot
%% G1
    \hline
...
\note{Changed semantics for \texttt{consByteString}.}
\label{note:consbytestring-v2}
In Plutus V1 and Plutus V2 the first argument of \texttt{consByteString} is an
arbitrary integer which will be reduced modulo 256 before being prepended to the
second argument.  In Plutus V3 we require that the first argument lies between 0
and 255 (inclusive): in any other case an error will occur.


\subsection{BLS12-381 built-in types}
\label{sec:V3-bls-types}
...
notation, most of which is used only in Sections~\ref{sec:V3-bls-types} and~\ref{sec:V3-bls-builtins}.

\bigskip
\noindent Table~\ref{table:V3-built-in-types} describes three new built-in
types.
...
\subsection{BLS12-381 built-in functions}
\label{sec:V3-bls-builtins}
...
%%    \caption[]{Built-in Functions}
    ...
    \label{table:V3-built-in-functions}
    \endlastfoot
%% G1
