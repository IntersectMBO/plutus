...
5. If no release blocking issue is found, open a PR for updating the cabal files and the changelog files.
Once the PR is merged, tag the commit `x.y.z.0`, and open a PR in the https://github.com/input-output-hk/cardano-haskell-packages[CHaP repository] for publishing the new version.
...
- This step can be omitted if no additional tests and checks are needed in the release QA process. See "Release QA Process" below.
5. Run the release QA process. See "Release QA Process" below.
6. If no release blocking issue is found, open a PR for updating the cabal files and the changelog files.
Once the PR is merged, tag the commit `x.y.z.w`, and open a PR in the CHaP repository for publishing the new version.
