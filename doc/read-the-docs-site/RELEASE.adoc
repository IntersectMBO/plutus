...
6. Once the PR is merged, tag the commit `x.y.z.0` using https://github.com/input-output-hk/plutus/releases/new[Github's release interface].
7. Open a PR in the https://github.com/input-output-hk/cardano-haskell-packages[CHaP repository] for publishing the new version. Run `./scripts/add-from-github.sh "https://github.com/input-output-hk/plutus" COMMIT-SHA LIST-OF-UPDATED-PACKAGES` (see https://github.com/input-output-hk/cardano-haskell-packages#-from-github[the README on CHaP]).
...
8. Once the CHaP PR has been merged, make a PR to update the version used in `cardano-ledger`.
