// LookupCoin plot configuration and rendering

// Configuration
const FUNCTION_NAME = 'LookupCoin';  // CSV uses PascalCase
...
function updateInfoPanel() {
  ...
  const stats = calculateStats(benchmarkData, 2);
...
function renderPlot() {
  ...
  // LookupCoin has 3 args: policy id (4 bytes), token name (4 bytes), and Value size
  // We plot the 3rd argument (Value size) on X-axis
  const benchmarkX = benchmarkData.map(d => d.args[2]);
  ...
  if (showModel && modelPredictions.length > 0) {
    const modelX = modelPredictions.map(d => d.args[2]);
