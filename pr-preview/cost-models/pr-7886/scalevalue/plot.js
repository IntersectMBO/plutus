// ScaleValue plot configuration and rendering (2D)
...
function updateInfoPanel() {
  ...
  // Update range for X axis (Value size)
  if (benchmarkData.length > 0) {
    const xValues = benchmarkData.map(d => d.args[1]);
...
function renderPlot() {
  // Prepare benchmark trace (2D scatter)
  const benchmarkX = benchmarkData.map(d => d.args[1]);
  ...
  if (showModel && modelPredictions.length > 0) {
    const modelX = modelPredictions.map(d => d.args[1]);
