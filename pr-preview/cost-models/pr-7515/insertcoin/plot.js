// InsertCoin plot configuration and rendering

// Configuration
const FUNCTION_NAME = 'InsertCoin';  // CSV uses PascalCase
...
function updateInfoPanel() {
  ...
  const stats = calculateStats(benchmarkData, 3);
...
function renderPlot() {
  ...
  // InsertCoin benchmarks record 4 arguments; the 4th captures Value size (log outer + log max inner)
  const benchmarkX = benchmarkData.map(d => d.args[3]);
  ...
  if (showModel && modelPredictions.length > 0) {
    const modelX = modelPredictions.map(d => d.args[3]);
    ...
    const modelTrace = {
      ...
      marker: { size: 4, color: '#E53E3E', opacity: 0.4 }
    };
  ...
  const layout = {
    ...
    xaxis: {
      title: 'Value size metric (log outer + log max inner)',
      gridcolor: '#E0E0E0'
    },
    yaxis: {
