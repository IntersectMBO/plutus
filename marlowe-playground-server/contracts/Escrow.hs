...
contract = When [Case (Deposit "alice" "alice" adaSymbol adaToken price) inner]
                10
                Close
...
agreement =
  If
    ...
    (Pay "alice" (Party "bob") adaSymbol adaToken price Close)
    Close
...
arbitrate =
  When  [ Case carolRefund Close,
          Case carolPay (Pay "alice" (Party "bob") adaSymbol adaToken price Close) ]
        100
        Close
