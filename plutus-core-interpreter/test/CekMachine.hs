...
import           Language.PlutusCore.CkMachine (CkEvalResult(..))
...
-- defined in terms of generic fix (see 'getBuiltinFix'), feed the resulting 'Term' to the CEK machine
-- (see 'evaluateCek') and check that the original 'Integer' and the computed @integer@ are in sync.
...
test_NatRoundtrip = testProperty "NatRoundtrip" . property $ do
    ...
    evaluateCek term === CkEvalSuccess n
...
-- sum elements of the list (see 'getBuiltinSum'), feed the resulting 'Term' to the CEK machine
-- (see 'evaluateCek') and check that 'sum' applied to the original list is in sync with the computed sum.
...
test_ListSum = testProperty "ListSum" . property $ do
    ...
    for_ mayPlcSum $ \plcSum -> evaluateCek term === CkEvalSuccess plcSum
