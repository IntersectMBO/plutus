...
import PlutusBenchmark.Marlowe.Core.V1.Data.Semantics as Semantics (MarloweData (..),
                                                                    ...
                                                                    computeTransaction,
                                                                    totalBalance)
import PlutusBenchmark.Marlowe.Core.V1.Semantics.Data.Types
import PlutusBenchmark.Marlowe.Scripts.Data.RolePayout (rolePayoutValidatorHash)
import PlutusLedgerApi.Data.V2 (Credential (..), Datum (Datum), DatumHash (DatumHash),
                                Extended (..), Interval, LowerBound (..), POSIXTime (..),
                                POSIXTimeRange, ScriptContext, ScriptHash (..), ScriptPurpose,
                                SerialisedScript, TxInInfo, TxInfo, UpperBound (..), pattern Finite,
                                pattern Interval, pattern LowerBound, pattern PubKeyCredential,
                                pattern ScriptContext, pattern ScriptCredential, pattern Spending,
                                pattern TxInInfo, pattern TxInfo, pattern UpperBound,
                                scriptContextPurpose, scriptContextTxInfo, serialiseCompiledCode,
                                txInInfoOutRef, txInInfoResolved, txInfoInputs, txInfoOutputs,
                                txInfoValidRange)
import PlutusLedgerApi.V2.Data.Contexts (findDatum, findDatumHash, txSignedBy, valueSpent)
import PlutusLedgerApi.V2.Data.Tx (OutputDatum, TxOut, pattern OutputDatumHash, pattern TxOut,
                                   txOutAddress, txOutDatum, txOutValue)
...
import PlutusTx.Prelude as PlutusTxPrelude (AdditiveGroup ((-)), AdditiveMonoid (zero),
                                            ...
                                            check, elem, filter, foldMap, null, otherwise, snd,
                                            toBuiltin, ($), (&&), (.), (/=), (||))
...
import PlutusLedgerApi.Data.V2 qualified as Ledger (Address, pattern Address)
import PlutusLedgerApi.V1.Data.Address qualified as Address (scriptHashAddress)
    ...
    case computedResult of
        TransactionOutput {txOutPayments, txOutState, txOutContract} -> do
            ...
            let marloweData = MarloweData {
                ...
                payoutsByParty = AssocMap.toList $ Data.List.foldMap payoutByParty txOutPayments
  ...
  where
    ...
    (ownInput@TxInInfo{txInInfoResolved=TxOut{txOutAddress=ownAddress}}, noOthers) =
        case findOwnInput ctx of
            Just ownTxInInfo -> examineScripts (sameValidatorHash ownTxInInfo) Nothing True (Data.List.toSOP $ txInfoInputs scriptContextTxInfo)
    ...
    checkState tag expected State{..} =
      let
        ...
        noDuplicates am =
          ...
          in
            test $ Data.List.toSOP $ Data.List.fromBuiltinList $ AssocMap.keys am
      in
           -- [Marlowe-Cardano Specification: "Constraint 5. Input value from script".]
           -- and/or
           -- [Marlowe-Cardano Specification: "Constraint 18. Final balance."]
    ...
    ~getContinuingOutput = case filter (\TxOut{txOutAddress} -> ownAddress == txOutAddress) (Data.List.toSOP allOutputs) of
        [out] -> out
    ...
    hasNoOutputToOwnScript = Data.List.all ((/= ownAddress) . txOutAddress) allOutputs

    -- All of the script outputs.
    allOutputs :: Data.List TxOut
    ...
    valuePaidToAddress address = Data.List.foldMap txOutValue $ Data.List.filter ((== address) . txOutAddress) allOutputs
