...
import PlutusLedgerApi.Data.V2 (Credential (..), Datum (Datum), DatumHash (DatumHash),
                                Extended (..), Interval, LowerBound, POSIXTime (..), POSIXTimeRange,
                                ScriptContext, ScriptHash (..), ScriptPurpose, SerialisedScript,
                                TxInInfo, TxInfo, UpperBound (..), pattern Finite, pattern Interval,
                                pattern LowerBound, pattern PubKeyCredential, pattern ScriptContext,
                                pattern ScriptCredential, pattern Spending, pattern TxInInfo,
                                pattern TxInfo, pattern UpperBound, scriptContextPurpose,
                                scriptContextTxInfo, serialiseCompiledCode, txInInfoOutRef,
                                txInInfoResolved, txInfoInputs, txInfoOutputs, txInfoValidRange)
...
mkMarloweValidator
  ...
  where
        ...
        case findOwnInput ctx of
            Just ownTxInInfo ->
              examineScripts
                ...
                Nothing
                True
                (Data.List.toSOP . txInfoInputs $ scriptContextTxInfo)
    ...
    allOutputs :: Data.List.List TxOut
