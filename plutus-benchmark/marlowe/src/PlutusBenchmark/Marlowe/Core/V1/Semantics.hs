...
{-| = Marlowe: financial contracts domain specific language for blockchain
  ...
  <https://github.com/input-output-hk/marlowe/blob/stable/src/Semantics.hs>
    ...
    (low, high)
      ...
      | otherwise ->
          let
            ...
            -- newLow is both new "low" and new "minTime"
            -- (the lower bound for slotNum)
...
convertReduceWarnings =
  foldr -- Note that `foldr` is used here for efficiency,
  -- differing from Isabelle.
    ( \warn acc ->
        case warn of
          ReduceNoWarning -> acc
...
computeTransaction tx state contract =
  let
    inputs = txInputs tx
   in
    case fixInterval (txInterval tx) state of
      IntervalTrimmed env fixState ->
        case applyAllInputs env fixState contract inputs of
          ...
          ApplyAllNoMatchError ->
            Error TEApplyNoMatchError
          ApplyAllAmbiguousTimeIntervalError ->
            Error TEAmbiguousTimeIntervalError
          ApplyAllHashMismatch ->
            Error TEHashMismatch
...
contractLifespanUpperBound contract = case contract of
  ...
  If _ contract1 contract2 ->
    max
      (contractLifespanUpperBound contract1)
      (contractLifespanUpperBound contract2)
  When cases timeout subContract ->
    let
     ...
     in
      Haskell.maximum
        ( timeout
            : contractLifespanUpperBound subContract
            : contractsLifespans
        )
...
instance Eq Payment where
  {-# INLINEABLE (==) #-}
  Payment a1 p1 t1 i1
    == Payment a2 p2 t2 i2 = a1 == a2 && p1 == p2 && t1 == t2 && i1 == i2
