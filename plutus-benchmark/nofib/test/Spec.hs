...
testClausify = testGroup "clausify"
               ...
               , runTestNested $
                  Tx.goldenSize "formula1 (size)" (Clausify.mkClausifyCode Clausify.F1)
               , runTestNested $
...
testKnights = testGroup "knights"  -- Odd sizes call "error" because there are no solutions
              ...
              , runTestNested $ Tx.goldenSize "depth 10, 4x4 (size)" (Knights.mkKnightsCode 10 4)
...
testQueens = testGroup "queens"
             ...
             , runTestNested $ Tx.goldenSize "Bt (size)" (Queens.mkQueensCode 5 Queens.Bt)
             ]
