...
testClausify =
  ...
  where
    formula5example = Clausify.mkClausifyCode Clausify.F5
...
testKnights =
  testGroup
    "knights" -- Odd sizes call "error" because there are no solutions
  ...
  where
    knightsExample = Knights.mkKnightsCode 10 4
...
testQueens =
  ...
  where
    queens4btExample = Queens.mkQueensCode 4 Queens.Bt
