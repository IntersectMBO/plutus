...
benchClausify name f = bench name $ nf Clausify.runClausify f

benchKnights :: String -> Integer -> Integer -> Benchmark
benchKnights name depth sz = bench name $ nf (Knights.runKnights depth) sz

benchPrime :: String -> Prime.PrimeID -> Benchmark
benchPrime name pid = bench name $ nf Prime.runFixedPrimalityTest pid

benchQueens :: String -> Integer -> Queens.Algorithm -> Benchmark
benchQueens name sz alg = bench name $ nf (Queens.runQueens sz) alg
