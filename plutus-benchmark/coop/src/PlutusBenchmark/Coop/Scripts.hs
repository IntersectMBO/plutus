...
fsV :: Datum -> Redeemer -> ScriptContext -> BuiltinUnit
fsV _ _  = mustBurnOwnSingletonValue
{-# INLINE fsV #-}

certV :: Datum -> Redeemer -> ScriptContext -> BuiltinUnit
certV _ _ = mustBurnOwnSingletonValue
{-# INLINE certV #-}

fsMp :: FsMpParams -> Redeemer -> ScriptContext -> BuiltinUnit
fsMp
  _params
...
authMp :: AuthMpParams -> Redeemer -> ScriptContext -> BuiltinUnit
authMp
  _
...
certMp :: CertMpParams -> Redeemer -> ScriptContext -> BuiltinUnit
certMp
  (CertMpParams {..})
  (Redeemer (unsafeFromBuiltinData @CertMpRedeemer -> CertMpMint))
  (ScriptContext
   (TxInfo {..})
   (Minting ownCs)
  ) =
...
certMp
  _
  (Redeemer (unsafeFromBuiltinData @CertMpRedeemer -> CertMpBurn))
  (ScriptContext
   (TxInfo {..})
   (Minting ownCs)
  ) =
