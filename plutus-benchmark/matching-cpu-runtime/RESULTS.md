...
Sparse capture makes `matchDataConstr` competitive with shallow `Match`. A `Data` marker in the static
...
Sparse `matchDataConstr` beat traditional deconstruction in all 22 cases, shallow `Match` in 3, and
...
Against the fastest existing implementation per case, sparse `matchDataConstr` ranged from `0.756532x`
...
`sparse / fastest` is directed as written; less than one means sparse `matchDataConstr` won. These are
host wall times, not portable cost-model coefficients.

| Case | Shallow (us) | Nested (us) | Traditional (us) | Sparse `matchDataConstr` (us) | Sparse / fastest |
|---|---:|---:|---:|---:|---:|
...
- The gap-only and sparse-tag `matchDataConstr` measurements use their respective reported revisions;
...
- Runner logic, arguments, case order, expected values, and arithmetic were identical. `Main.hs`
  differed only in its explicit `_shallow`, `_nested`, `_traditional`, or `_matchdataconstr` matcher
...
- All 66 emitted baseline terms were inspected structurally. Traditional terms
  ...
  continuation without administrative capture lambdas/applications. The `matchDataConstr` emitter uses
  a canonical hidden `ByteString` containing only reachable constructor tags and their gap
  programs, cases the returned `VConstr` at its compact local index, and binds only selected
  fields.
...
- Criterion wall-clock time, `-L 2`, 1000 bootstrap resamples, with implementation order rotated
  across the three baseline branches by case. Sparse `matchDataConstr` was run in listed order.
...
## Gap-ByteString execution-budget results (2026-08-17)

`MatchDataConstr` uses the canonical builtin-cost benchmark and repository cost-model generator. The
full-duration 97-point run used a fresh two-argument no-op measurement from the same build and
fitted CPU as `1,964,948*x`; memory is `1 + 8*x`. Here `x` is the ordinary ByteString size in
eight-byte words. The CPU slope is fitted to dedicated skip-saturated inputs up to a
131,072-field constructor and the line covers every measured mean. A single X unit can contain
eight gap bytes and each gap byte can traverse 255 fields, so the ordinary ByteString measure
requires this conservative slope. The memory slope similarly covers at most eight captures per
X unit.

The second `Data` argument has no model term. Constructor tags are bounded to `Word64`; every
field traversal is bounded by the encoded gaps; and captured payloads are moved by reference.
Independent payloads containing a 65,536-bit integer, a 1 MiB bytestring, and a 10,000-node spine
did not affect denotation time. The raw measurements are in
[`benching-matchdata-gap-bytestring-2026-08-17.csv`](../../plutus-core/cost-model/data/benching-matchdata-gap-bytestring-2026-08-17.csv)
and
[`benching-matchdata-gap-bytestring-nop2-2026-08-17.csv`](../../plutus-core/cost-model/data/benching-matchdata-gap-bytestring-nop2-2026-08-17.csv).

All 22 cases returned the expected integer and passed the 16 KiB script-size limit,
10,000,000,000 CPU limit, and 14,000,000 memory limit. `MatchDataConstr` uses less CPU than traditional
matching in 1 of 22 cases and less CPU than shallow and nested matching in 2 cases each. It uses
less memory than traditional matching in all 22 cases, shallow matching in 20, and nested
matching in 12. It produces a smaller script than traditional matching in all 22 cases.

### CPU

| Case | Shallow | Nested | Traditional | MatchDataConstr |
|---|---:|---:|---:|---:|
| `constr_flat_d1_w1_c1` | 251,720 | 339,976 | 481,765 | 2,209,792 |
| `constr_flat_d1_w16_c4` | 1,388,354 | 1,387,462 | 2,005,238 | 2,959,648 |
| `constr_flat_d1_w1000_c1` | 17,544,410 | 13,383,928 | 2,982,974 | 2,209,792 |
| `constr_flat_d1_w1000_c16` | 21,929,330 | 17,589,406 | 9,640,109 | 9,696,968 |
| `constr_spine_front_d4_w16_c8` | 3,618,046 | 3,746,106 | 6,082,994 | 10,174,300 |
| `constr_spine_middle_d4_w16_c8` | 3,618,046 | 3,746,106 | 6,598,823 | 10,174,300 |
| `constr_spine_last_d4_w16_c8` | 3,618,046 | 3,575,250 | 5,854,164 | 10,174,300 |
| `constr_spine_irregular_d4_w16_c8` | 3,618,046 | 3,746,106 | 6,151,999 | 10,174,300 |
| `constr_spine_irregular_d8_w8_c8` | 3,924,046 | 4,410,546 | 10,189,609 | 18,546,092 |
| `constr_spine_front_d64_w2_c8` | 9,315,886 | 14,943,370 | 29,303,115 | 135,751,180 |
| `constr_spine_zigzag_d100_w2_c10` | 13,900,862 | 20,320,430 | 47,413,347 | 211,565,212 |
| `constr_binary_d3_w16_c8` | 4,678,426 | 4,683,342 | 7,979,231 | 16,453,144 |
| `constr_ternary_d3_w8_c10` | 5,583,602 | 6,115,850 | 13,173,734 | 29,478,736 |
| `constr_quaternary_d3_w8_c17` | 9,349,738 | 10,379,768 | 20,190,093 | 57,684,724 |
| `constr_rootfork2_d6_w12_c8` | 4,762,186 | 5,105,942 | 9,408,724 | 20,639,040 |
| `constr_rootfork3_d5_w10_c9` | 4,992,534 | 5,466,452 | 11,034,419 | 22,965,940 |
| `constr_rootfork4_d4_w8_c8` | 3,924,046 | 4,296,642 | 8,336,134 | 20,511,040 |
| `constr_spine_stress_d10_w100_c20` | 23,787,142 | 20,349,186 | 18,625,485 | 25,539,412 |
| `constr_binary_stress_d8_w8_c32` | 64,039,978 | 76,680,422 | 193,778,602 | 541,119,096 |
| `constr_alt_spine_d16_w8_c8` | 5,695,816 | 11,368,843 | 15,748,590 | 35,624,051 |
| `constr_alt_rootfork3_d5_w10_c9` | 5,044,464 | 9,205,495 | 11,152,344 | 23,300,315 |
| `constr_alt_binary_d8_w8_c32` | 64,091,908 | 147,351,747 | 193,009,841 | 541,453,471 |

### Memory

| Case | Shallow | Nested | Traditional | MatchDataConstr |
|---|---:|---:|---:|---:|
| `constr_flat_d1_w1_c1` | 1,700 | 1,236 | 2,565 | 1,541 |
| `constr_flat_d1_w16_c4` | 6,806 | 4,440 | 7,579 | 4,043 |
| `constr_flat_d1_w1000_c1` | 101,600 | 61,182 | 4,333 | 1,541 |
| `constr_flat_d1_w1000_c16` | 119,630 | 72,696 | 25,239 | 12,867 |
| `constr_spine_front_d4_w16_c8` | 17,914 | 10,455 | 22,710 | 9,406 |
| `constr_spine_middle_d4_w16_c8` | 17,914 | 10,455 | 23,810 | 9,406 |
| `constr_spine_last_d4_w16_c8` | 17,914 | 10,437 | 21,450 | 9,406 |
| `constr_spine_irregular_d4_w16_c8` | 17,914 | 10,455 | 22,882 | 9,406 |
| `constr_spine_irregular_d8_w8_c8` | 19,914 | 10,525 | 36,054 | 12,642 |
| `constr_spine_front_d64_w2_c8` | 54,314 | 15,387 | 148,914 | 57,946 |
| `constr_spine_zigzag_d100_w2_c10` | 81,918 | 21,651 | 223,478 | 88,538 |
| `constr_binary_d3_w16_c8` | 24,214 | 13,368 | 29,649 | 11,833 |
| `constr_ternary_d3_w8_c10` | 28,818 | 14,526 | 48,791 | 18,155 |
| `constr_quaternary_d3_w8_c17` | 47,632 | 23,894 | 77,245 | 29,805 |
| `constr_rootfork2_d6_w12_c8` | 24,814 | 13,178 | 35,791 | 13,451 |
| `constr_rootfork3_d5_w10_c9` | 25,716 | 13,485 | 40,974 | 14,994 |
| `constr_rootfork4_d4_w8_c8` | 19,914 | 10,513 | 32,006 | 12,650 |
| `constr_spine_stress_d10_w100_c20` | 128,938 | 75,939 | 59,076 | 23,068 |
| `constr_binary_stress_d8_w8_c32` | 369,862 | 151,706 | 736,289 | 230,081 |
| `constr_alt_spine_d16_w8_c8` | 30,614 | 24,401 | 58,082 | 20,646 |
| `constr_alt_rootfork3_d5_w10_c9` | 26,016 | 21,883 | 41,814 | 16,526 |
| `constr_alt_binary_d8_w8_c32` | 370,162 | 286,767 | 735,045 | 231,613 |

### Script bytes

| Case | Shallow | Nested | Traditional | MatchDataConstr |
|---|---:|---:|---:|---:|
| `constr_flat_d1_w1_c1` | 15 | 12 | 29 | 23 |
| `constr_flat_d1_w16_c4` | 43 | 37 | 76 | 49 |
| `constr_flat_d1_w1000_c1` | 266 | 638 | 45 | 27 |
| `constr_flat_d1_w1000_c16` | 383 | 712 | 256 | 137 |
| `constr_spine_front_d4_w16_c8` | 99 | 90 | 221 | 121 |
| `constr_spine_middle_d4_w16_c8` | 99 | 90 | 232 | 121 |
| `constr_spine_last_d4_w16_c8` | 99 | 90 | 212 | 121 |
| `constr_spine_irregular_d4_w16_c8` | 99 | 90 | 224 | 121 |
| `constr_spine_irregular_d8_w8_c8` | 116 | 94 | 348 | 181 |
| `constr_spine_front_d64_w2_c8` | 378 | 197 | 1,663 | 973 |
| `constr_spine_zigzag_d100_w2_c10` | 569 | 294 | 2,600 | 1,497 |
| `constr_binary_d3_w16_c8` | 124 | 123 | 305 | 163 |
| `constr_ternary_d3_w8_c10` | 164 | 135 | 488 | 264 |
| `constr_quaternary_d3_w8_c17` | 270 | 218 | 808 | 430 |
| `constr_rootfork2_d6_w12_c8` | 132 | 123 | 362 | 194 |
| `constr_rootfork3_d5_w10_c9` | 142 | 124 | 409 | 216 |
| `constr_rootfork4_d4_w8_c8` | 116 | 94 | 320 | 179 |
| `constr_spine_stress_d10_w100_c20` | 453 | 741 | 605 | 295 |
| `constr_binary_stress_d8_w8_c32` | 2,025 | 1,853 | 8,879 | 4,133 |
| `constr_alt_spine_d16_w8_c8` | 197 | 280 | 584 | 309 |
| `constr_alt_rootfork3_d5_w10_c9` | 175 | 240 | 419 | 229 |
| `constr_alt_binary_d8_w8_c32` | 2,167 | 3,697 | 8,861 | 4,149 |

Full current `MatchDataConstr` data is in
[`results/2026-08-17-matchdata-gap-bytestring-validation.csv`](results/2026-08-17-matchdata-gap-bytestring-validation.csv);
...
runtime and sparse `matchDataConstr` on the prototype branch. Those evaluators are not
...
Sparse `matchDataConstr` ranges from 24.3468% faster to 135.6582% slower than the fastest historical
