...
-- Make a set of golden tests with results stored in subdirectories determined by the GHC version.
runTestGhcM :: TestNested -> TestTree
runTestGhcM = runTestNestedM ["script-contexts", "test"] . testNestedGhcM
...
testCheckScEquality = testGroup "checkScriptContextEquality" . pure $
    runTestGhcM $ do
        Tx.goldenBudget "checkScriptContextEqualityData-20" $
