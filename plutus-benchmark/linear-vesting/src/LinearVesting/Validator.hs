...
{-# INLINE txSignedBy' #-}
txSignedBy' :: BI.BuiltinList BI.BuiltinData -> BI.BuiltinByteString -> Bool
txSignedBy' signatories pkh =
  BI.caseList'
    False
    ( \s ss ->
...
{-# INLINE validateVestingPartialUnlock #-}
validateVestingPartialUnlock
  ...
  -> BI.BuiltinData
  -> Bool
validateVestingPartialUnlock txInputs txOutputs txValidRange txSignatories ownRef vestingDatum =
  let ownInput = findInputByOutRef ownRef txInputs
      ...
      signed = txSignedBy' txSignatories beneficiaryHash
   in if
        | not signed ->
...
{-# INLINE validateVestingFullUnlock #-}
validateVestingFullUnlock
  :: BI.BuiltinData
  -> BI.BuiltinList BI.BuiltinData
  -> BI.BuiltinData
  -> Bool
validateVestingFullUnlock txValidRange txSignatories vestingDatum =
  let !vdFields = BI.snd (BI.unsafeDataAsConstr vestingDatum)
      ...
      beneficiaryHash = getPubKeyHashFromAddress beneficiaryAddr
   in if
        | not (txSignedBy' txSignatories beneficiaryHash) ->
            traceError "Missing beneficiary signature"
...
untypedValidator :: BI.BuiltinData -> BI.BuiltinUnit
          ...
          ( BI.trace
              ...
              ( BI.caseInteger
                  ...
                  , BI.trace
                      ...
                      (validateVestingFullUnlock txValidRange txSignatories datumData)
                  ]
              )
          )
   in if result
...
validatorCode :: CompiledCode (BI.BuiltinData -> BI.BuiltinUnit)
