...
constitutionValidator (ConstitutionConfig cfg) =
    Common.withChangedParams (runRules cfg . DM.toBuiltinList)
...
runRules :: [Param]  -- ^ the config (sorted by default)
         -> BuiltinList (BuiltinPair BuiltinData BuiltinData) -- ^ the params (came sorted by the ledger)
         -> Bool
runRules ((expectedPid, paramValue) : cfgRest) cparams = caseList'
  True
  (\ pair cparamsRest ->
    case B.unsafeDataAsI (BI.fst pair) `compare` expectedPid of
        EQ ->
            Common.validateParamValue paramValue (BI.snd pair)
...
runRules [] cparams = caseList' True (\ _ _ -> False) cparams
