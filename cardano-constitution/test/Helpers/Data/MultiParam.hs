...
module Helpers.Data.MultiParam
...
import Helpers.Data.CekTests
import Helpers.Data.Guardrail as G
  ...
  TSQ.forAll (combine2Gen gen extraGen) $
   \(params',extraParams) -> monadicIO $ do
    ...
    when (length joinedResults /= 1) $
      fail $ "Validator results are not the same: " ++ show results'
...
allValidAndOneCustom :: (GenericParam -> Maybe (Gen Printable))
                     -> [GenericParam]
...
allValidAndOneCustom customGen params = do
  ...
  tryGenerate = do
    ...
    tailValues <- mapM (\(MkGenericParam gr) -> inRangeSingleParamValues gr)
                       $ tail xs
...
allValidButOnePlusOneUnknown xs = do
  ...
  values <- oneof [allValidAndOneLessThanLower xs
...
inRangeSingleParamValues :: forall a. Guardrail (Param a)
                         -> Gen (ParamIx,Printable)
...
lessThanLowerParamValue :: forall a. GeneratorSpectrum
                        -> Guardrail (Param a)
                        -> Maybe (Gen Printable)
...
lessThanLowerParamValue _ gr@(G.WithinDomain _ _) = lessThanLowerParamValue' range
  ...
  lessThanLowerParamValue' (MkParamRangeWithinDomain (a,_) domain') = Just $
...
greaterThanUpperParamValue _ gr@(G.WithinDomain _ _) = greaterThanUpperParamValue' range
  ...
  greaterThanUpperParamValue' (MkParamRangeWithinDomain (_,b) domain') = Just $
...
outOfRangeParamValue _ gr@(G.WithinDomain _ _) = outOfRangeParamValue' range
  where
  (_,range) = paramRange gr
  outOfRangeParamValue' (MkParamRangeWithinDomain (a,b) (start,end)) | a > start && b < end = Just $
