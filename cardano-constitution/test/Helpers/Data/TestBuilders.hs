...
module Helpers.Data.TestBuilders where
...
updateSingleParamState :: (ToJSON n,Show n,ToData n)
                       => ParamId
                       -> n
                       -> Bool
                       -> TestState
...
updateSingleParamStateRef :: (ToJSON n,Show n,ToData n)
                          => ParamId
                          -> n
                          -> Bool
                          -> IORef TestState
...
updateMultiParamState :: TestNumber
                      -> ParamValues
                      -> Bool
                      -> TestState
                      -> TestState
...
updateMultiParamStateRef :: TestNumber
                         -> ParamValues
                         -> Bool
                         -> IORef TestState
                         -> IO ()
...
unitTestTemplatePositive :: (ToJSON b,Show b, ToData b)
                          => ParamKey
                          -> b
...
unitTestTemplateNegative :: (ToJSON b,Show b, ToData b)
                         => ParamKey
                         -> b
                         -> AssertionWithTestState
...
oneParamProp :: (ToJSON b,Show b, Testable prop, ToData b)
              => ParamKey
              -> Gen b
              -> ((Bool, b) -> prop)
...
oneParamChange :: (ToJSON a, Show a, ToData a)
               => ParamIx
               -> a
               -> [(ParamIx, Printable)]
oneParamChange paramIx value = [(paramIx, pack value)]

oneParamProp' :: (ToJSON a,Show a, Testable prop, ToData a)
              => ParamId
              -> (a -> ParamValues)
              -> Gen a
              -> ((Bool, a) -> prop)
...
pbtParamValidRange :: (ToJSON a,Show a,ToData a,HasRange a)
                   => ParamKey
                   -> (a, a)
                   -> PropertyWithTestState
...
mergeDomains (a,b) (c,d) | b < c = [(a,b),(c,d)]
...
data ParamRange
  = forall a . (Num a, Show a, ToJSON a, HasRange a, ToData a, Ord a)
