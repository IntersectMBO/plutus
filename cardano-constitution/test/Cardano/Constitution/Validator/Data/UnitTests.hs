...
module Cardano.Constitution.Validator.Data.UnitTests
  ( unitTests
  , singleParamTests
  ) where
...
import Helpers.Data.Guardrail qualified as Guards
...
test_pos = const $ testGroup "Positive" $
            fmap (\(n, c) -> testProperty n $ allVldtrsPassed (M.elems defaultValidators) c)
            [ ("pos1", V3.mkFakeParameterChangeContext @Integer
                  [ (0, 30) -- in limits
                  , (1, 10_000_000) -- in limits
                  , (17, 6_250) -- in limits
                  ])
            , ("pos2", V3.mkFakeParameterChangeContext
                  [ (10, NonCanonicalRational $ Tx.unsafeRatio 1 1000) -- in limits
                  ])
            , ("pos3", V3.mkFakeParameterChangeContext
                  [ (19,
            ...
            , ("pos4", V3.mkFakeParameterChangeContext @Integer
                 [ -- empty params
                 ])
            ] ++ [
              testProperty "pos5" $ forAll V3.treasuryWithdrawalsCtxGen
...
test_neg = const $ testGroup "Negative" $
            fmap (\(n, c) -> testProperty n $ allVldtrsErred (M.elems defaultValidators) c)
      [ ("neg1", V3.mkFakeParameterChangeContext @Integer
                 [ (0, 29) -- **smaller than minbound**
                 , (1, 10_000_000) -- in limits
                 , (2, -10_000_000_000) -- unknown param
                 , (17, 6_250) -- in limits
                 ])
      , ("neg2", V3.mkFakeParameterChangeContext @Integer
                 [ (0, 29) -- **smaller than minbound**
                 , (1, 10_000_000) -- in limits
                 , (17, 6_251) -- ** larger than maxbound **
                 ])
      , ("neg3", V3.mkFakeParameterChangeContext @Integer
                 [ (-1, -1_000) -- unknown param
                 ])
      , ("neg4", V3.mkFakeParameterChangeContext @Integer
                 [ (10, 1) -- type mismatch, 10 is supposed to be Rational
                 ])
      , ("neg5", V3.mkFakeParameterChangeContext
                 [ (0, NonCanonicalRational $ Tx.unsafeRatio 1 1) -- type mismatch, 0 is supposed to be integer
                 ])
      , ("neg6", V3.mkFakeParameterChangeContext
                 [ (10, NonCanonicalRational $ Tx.unsafeRatio 0 1000) -- out of limits
                 ])
      , ("neg7", V3.mkFakeParameterChangeContext
                 [ (19,
                       [ NonCanonicalRational $ Tx.unsafeRatio 2_000 10_000 -- exmem in limits
                       , NonCanonicalRational $ Tx.unsafeRatio 0 10_000_000 -- exsteps out of limits
                       ])
                 ])
      , ("neg8", V3.mkFakeParameterChangeContext
                 [ (19, [ NonCanonicalRational $ Tx.unsafeRatio 2_000 10_000 -- exmem in limits
                        , NonCanonicalRational $ Tx.unsafeRatio 500 10_000_000 -- exsteps in limits
                        , NonCanonicalRational $ Tx.unsafeRatio 500 10_000_000 -- TOO MUCH SUBS in list SUPPLIED
                        ])
                 ])
      , ("neg9", V3.mkFakeParameterChangeContext
                  [ (19, [ NonCanonicalRational $ Tx.unsafeRatio 2_000 10_000 -- exmem in limits
                         -- TOO FEW SUBS in list SUPPLIED
                         ])
                  ])
      , ("neg10", V3.mkFakeParameterChangeContext @[Integer]
                  [ (19, [
                            -- TOO FEW SUBS in list SUPPLIED
                         ])
                  ])
      , ("neg11", V3.mkFakeParameterChangeContext
                  [ (19,
                        -- TOO DEEPLY NESTED
                        [
                            [ NonCanonicalRational $ Tx.unsafeRatio 2_000 10_000 -- exmem in limits
...
test_unsorted2 = const $ testProperty "unsorted2" $
...
test_LargeProposalSize = const $ testCaseInfo "largeProposalSize" $ do
     let largeSize =  Tx.lengthOfByteString $ Tx.serialiseData $ toBuiltinData $
