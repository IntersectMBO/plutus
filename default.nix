...
in rec {
  ...
  haskell = rec {
    ...
    muslPackages = import ./nix/haskell.nix { inherit (pkgsMusl) lib stdenv pkgs haskell-nix buildPackages; inherit metatheory checkMaterialization sources; };
    # Extra Haskell packages which we use but aren't part of the main project definition.
    extraPackages = pkgs.callPackage ./nix/haskell-extra.nix { inherit (localLib) index-state; inherit checkMaterialization sources; };
  };
