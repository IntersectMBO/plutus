...
if [ -z "$(git merge-base HEAD origin/zliu41/bench-guardrail)" ]; then
   echo "The command 'git merge-base HEAD origin/zliu41/bench-guardrail' returned an empty string."
...
git checkout "$(git merge-base HEAD origin/zliu41/bench-guardrail)"
