...
```bash
$(nix-build -A marlowe-dashboard.server-invoker)/bin/marlowe-dashboard webserver
```

Check the [backend documentation](../marlowe-dashboard-server/README.md) for more information.

Now we will build and run the front end:
```bash
# First generate the purescript bridge files
$(nix-build -A marlowe-dashboard.server-invoker)/bin/marlowe-dashboard psgenerator ./marlowe-dashboard-client/generated
