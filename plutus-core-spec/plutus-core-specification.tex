...
%\usepackage[disable]{todonotes}  % Use this to hide the notes
...
\newcommand{\todokwxm}[1]{\todo[inline,color=blue!20,author=Kenneth]{#1}}
...
costs; the fee also includes a storage charge calculated from the size
of the code. If at any point during execution the amount of gas
consumed exceeds the amount than has been paid for then the program is
...
\todokwxm{This is issue \#1143.}
\medskip
\todokwxm{There's also some debate about whether builtins should always be fully applied.
  See issue \#1144.}
...
\item $\con{cn}$ represents a constant belonging to some built-in
  ...
  See Section~\ref{sec:builtins} for more information.
...
\item $\iwrap{A}{B}{M}$ and $\unwrap{M}$: see the note on recursive types in Section~\ref{sec:recursive-types}.
...
\item $\fixT{A}{B}$ represents a recursive type: see the note in Section~\ref{sec:recursive-types} below for more information.
...
\paragraph{Normalised Types.} All types can be reduced to a
\emph{normalised type} which cannot undergo any further reduction: see
section~\ref{sec:reduction}.  The type normalisation process always
terminates: the proof is similar to the proof of the fact that
reduction of terms in the simply typed lambda calculus always
terminates \citep[\S12]{Pierce:TAPL}.  The type-level structure of
Plutus Core is slightly more complicated than the simply typed lambda
calculus, but the extra forms (\texttt{ifix}, \texttt{all}, built-in
base types) don't provide any new opportunities for type reduction and
thus have little effect on the proof.

  \medskip
  \todokwxm{I think we've agreed that the term ``type value'' isn't
    very helpful, so I've replaced it with ``normalised type'' everywhere.
    It's possible that we might want to say ``normal type'' instead.}
...
checking, before a program is submitted to the chain.  Another issue
here is that normalisation can cause the size of a type to increase
(in the worst case, exponentially), and requiring normalisation to be
performed off-chain makes it easier to predict the fee for storing and
executing the program on the chain.
...
index. Pattern functors bind two
variables: one for building recursive occurrences and the other to act
as an index. Indices can be used to encode parameterised data
types, but also to control the shape of the data type:
...
\todokwxm{We could do with more detail (or perhaps an
...
\todokwxm{I don't think we know exactly what we want to say about
  erasure yet: there's the problem with \texttt{error}, for example.
  I hope that James will prove something at some point, and then we
  can mention it here. There's an issue about this (\#948).}
...
Figure~\ref{fig:contexts}, we define the grammar of contexts, which
...
given a valid context $\Gamma$, it can be extended with a fresh variable
and kind or a fresh variable and a valid type in $\Gamma$. Note that we do
not need a separate judgement for kind validity. Our kinds are so simple that
...
System F rules (\textsf{tyvar}, \textsf{tyall}, \textsf{tyfun}) together with some extensions with
extensions to higher kinds (\textsf{tylam}, \textsf{tyapp}) and to indexed
recursive types (\textsf{tyfix}). We also have the \textsf{tycon} rule to support builtin types.
...
have rules analogous to those for the simply-typed lambda calculus
(\textsf{var}, \textsf{lam}, \textsf{app}) and System F (\textsf{abs},
\textsf{inst}).  Higher kinds lead to computation in types so we need
the rule \textsf{conv}. Iso-recursive types introduce terms for
wrapping and unwrapping recursive values (wrap,unwrap), and to support
constants and builtins we have \textsf{con}, \textsf{builtin} and
\textsf{error}.  Type synthesis relies on type equivalence as defined
in \ref{fig:type-equivalence}.
...
\noindent Finally, type synthesis for builtins is presented in tabular form
rather than in inference rule form, in Figure~\ref{fig:builtins}.
...
% It seems that Harper talks about "contextual semantics" in the 
...
Appendix~\ref{appendix:algorithmic-restricted}.
...
\todokwxm{James says that the rule for \texttt{abs} breaks type preservation.
  See issue \#1145.  This is also related to type erasure.}
...
\medskip \todokwxm{This is inconsistent with the operational semantics
  where \texttt{error} is concerned.  The machine just stops instantly
  when it encounters \texttt{error}, but the semantics say that
  \texttt{error} should propagate outwards, changing its type as it
  goes (see Figure~\ref{fig:term-reduction}); this is because the
  \texttt{error} value has to have \textit{some} type, and it has to
  match whatever the enclosing term expects.  We'll have to state and
  prove something about type erasure at some point, but not right
  now. See issue \#948.}
...
for ``extensible builtins'' to allow new builtins for specialised
blockchains.

There are two basic types: \texttt{integer} and
...
at some time in the future, or that they may be used for extensible builtins.
...
\todompj{\ref{fig:constants} is hard to understand.}
...
The validator also has access to certain information about the
transaction currently being validated, such as the hash of the current
transaction. This information is passed in via an additional argument
which we will not discuss here; in the example we will assume that the
transaction hash is in scope as \texttt{txhash}.
...
\label{appendix:algorithmic-restricted}
