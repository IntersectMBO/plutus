...
\newcommand{\inDelayFrame}{\delay{\_}}
...
\noindent{\color{red} [kwxm] The introduction of \texttt{delay}
...
so much overhead. I'll look into this.  }
...
$\erase{M}$\redfootnote{I'm not wedded to this particular notation for
erasure, but it doesn't look too bad.}.  We extend this to sequences of
...
\subsection{Term Reduction}
Figure~\ref{fig:untyped-term-reduction} defines a reduction semantics for untyped
Plutus Core, analogously to Figure~\ref{fig:term-reduction}.
...
\noindent\red{I don't remember exactly what's going on, but I think
the rule marked $(*)$ is there to deal with polymorphic constants, of which
we don't currently have any.  We already have $\con{\ldots}$ for
built-in constants, so I'm not sure why we have zero-argument built-in
functions.  I'll have to think about this and consult Roman about
what's going to happen in future.  Eventually I'll either remove that rule
(and zero-argument builtins elsewhere) or replace it with a suitable untyped
version.}
