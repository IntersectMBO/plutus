...
\item $\conT{tcn}$ represents a built-in type: for example, $\conT{\textrm{integer}}{8}$
...
Figure~\ref{fig:Plutus_core_type_constants}: for example \texttt{(con
...
Note the following:
\begin{itemize}
\item The built-in functions in Figure~\ref{fig:Plutus_core_builtins}
...
\item Some of the entries in Figure~\ref{fig:Plutus_core_builtins}
  contain \textit{success conditions}.  If a success condition is
  violated then the progam terminates immediately and returns
...
\item We provide two versions of the division and remainder operations
  ...
  \item \texttt{divideInteger} and \texttt{modInteger} implement the
    standard mathematical integer division operations: \texttt{divideInteger}
    rounds downwards and \texttt{modInteger} always produces a non-negative
    result.  These correspond to Haskell's \texttt{div} and \texttt{mod} operators.
  \item \texttt{quotientInteger} and \texttt{remainderInteger}
    represent the operations found in many computer languages and CPUs:
    \texttt{quotientInteger} rounds towards zero and \texttt{remainderInteger $a$ $b$}
    will be non-negative if $a \ge 0$ and negative if $a < 0$.
    These correspond to Haskell's \texttt{rem} and \texttt{quot} operators.
  \end{itemize}
...
\subfile{figures/TypeCorrectness-Algorithmic-Unrestricted.tex}
...
Figures~\ref{fig:Plutus_core_grammar_algorithmic_restricted}--\ref{fig:Plutus_core_type_synthesis_algorithmic_restricted}
...
\subfile{figures/TypeCorrectness-Algorithmic-Restricted.tex}
