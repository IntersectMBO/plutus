...
We use the \texttt{flat} format to serialise plutus terms. The \texttt{flat} format
encodes sum types as tagged unions and products by concatenating their contents. We
proceed by defining the structure and the data types of untyped plutus core and how
they get serialised.

\subsection{Variable length data}

\texttt{Lists} are encoded by prefixing (using the most significant bit) the element
stored in the list with one of the two list constructor tags. If we want to follow the
current element with another one we use the bit `1` for the \texttt{Cons} constructor,
or `0` for the \texttt{Nil} constructor if this is the last element of the list.

\noindent We encode variable length data as a list of chunks, 7 bits each, with the least
significant chunk first and the most significant bit first in the chunk.
...
\begin{itemize}
  ...
  \item Signed \texttt{Integer}s are first converted to an unsigned value using the
    \texttt{ZigZag} encoding, then they are encoded as variable length data types.
...
Terms are encoded using 4 bits, which allows a total of 16 terms, of which 8 are
currently used.
...
\noindent Builtins use 5 bits for their definition, allowing for a maximum of 32 builtin
