...
We use the flat format to serialise plutus terms. The Flat format encodes variant types as tagged unions,
so for fully specifying the serialisation protocol, we need to define the structure and size of
the data types we serialise.

\subsection{Variable length data}

\texttt{Lists} are encoded by prefixing the element stored in the list with one of the two list
constructors. If we want to follow the current element with another one we use the bit
`1` for the \texttt{Cons} constructor, or `0` for the \texttt{Nil} constructor if this
...
\begin{itemize}
  \item Unsigned \texttt{Integer}s and \texttt{ByteString}s are encoded using the
    previously introduced encoding for variable data types.
  \item Signed \texttt{Integer}s are first converted to an unsigned value using the
    \texttt{ZigZag} encoding, then they are encoded as variable data types.
...
\noindent Builtins use 5 bits for their definition, allowing for a maximum of 32 values
of which 25 are used.
