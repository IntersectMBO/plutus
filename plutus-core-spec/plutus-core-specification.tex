...
\cite{Koopman:2014}, for example). Similarly, there are no explicit
constructs for recursion or branching, but the type system is
sufficiently expressive to allow us to use standard combinators (for
example the $Z$ combinator and Church/Scott encoded booleans) for such
purposes.
...
\blue{[Most of this paragraph is probably false $\rightarrow$]} 
...
then the payment is forfeit; similarly, if a program terminates early
...
Plutus Core Code, but nothing else.  Type checking itself takes time
and requires memory, so there is also a gas charge for on-chain type
checking.
...
\item $\fixT{\alpha}{A}$ is a type-level fixpoint operator $\fixtype{A}$: see the note on isorecursive types below.
...
\paragraph{Kinds.} \blue{Say something about the kind system.}
...
is used by the Plutus Core evaluator and other tools to check that the
code is of the correct version.
...
In $\abs{\alpha}{K}{V}$ we require the body $V$ to be a value.
Experience has shown that allowing general terms as bodies of type
abstractions can cause a number of difficulties.  For example,
Standard ML's \textit{value restriction} (\cite[22.7]{Pierce:TAPL})
solves a number of problems when (implicit) type abstractions include
non-values; other issues are explored in
\textit{cf.}~\cite[2.4]{Ahmed:2017}. Plutus Core's restriction to
values avoids these problems and is not very onerous in practice.
