...
\footnotetext[5]{\red{The introduction of \texttt{delay}
and \texttt{force} may be controversial since $\delay{M}$ can be
implemented as $\lamU{u}{M}$ and $\force{M}$ as
$\appU{M}{\lamU{x}{x}}$.  However, doing this has a cost in code size.
Experiments show that adding \texttt{delay} and \texttt{force}
increases the size of compressed validators by about 10\%, but using
$\lamU{u}{M}$ and $\appU{M}{\lamU{x}{x}}$ increases sizes by about
20\%.  A third option is to introduce a global variable $u$ of
type \texttt{unit} and replace $\force{M}$ with $\appU{M}{u}$: this
only increases sizes by about 12\%, but complicates specification
because programs have to be wrapped in an expression binding $u$ (and
maybe the CEK machine has to start with an environment binding $u$ to
$\lamU{x}{x}$).  Another option would be to extend the grammar with a
unit term, but then it would be tempting to add terms
for \texttt{true}, \texttt{false}, and so on.   
It's possible that in actual validator code (especially after type
normalisation), \texttt{delay} and \texttt{force} will typically appear in
expressions of the form $\force{\delay{e}}$, which could immediately
be replaced by $e$ alone, and then using $\lamU{x}{x}$ might not cause
so much overhead. I'll look into this.}\ \blue{Later: nope.  I looked at the on-chain validators for the use cases and this only happens about 10\% of the time.}}
...
$\erase{M}$\footnotemark[4].  We extend this to sequences of
...
\footnotetext[4]{\red{I'm not wedded to this particular notation for erasure,
but I'm not going to buy the argument that it'll confuse people. Other
possiblities: $\left|M\right|$, $\left\lVert{M}\right\rVert$,
$\overline{M}$ (that looks pretty bad), etc. I'll consult some experts to
find out if there's a standard notation.}}
