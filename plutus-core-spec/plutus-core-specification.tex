...
%% Final version before removal of types
...
\title{Formal Specification of\\the Plutus Core Language (version 1.99) }
...
\item $\con{cn}$ represents a constant belonging to some built-in
  type: for example \texttt{(con 4 ! 127)} is an integer of size 4 with
    the value 127.  See Section~\ref{sec:builtins} for more on this.
...
is the type of 8-byte integers.  See Section~\ref{sec:builtins} for more information.
\end{itemize}
...
types described below in Section~\ref{sec:size-note}.
...
%\ref{fig:Plutus_core_contexts}. Technically this judgement is defined
%mutually with the typing and kinding judgement defined in Figures
%\ref{fig:Plutus_core_type_synthesis} and
%\ref{fig:Plutus_core_kind_synthesis}. We give axiomatic rules for term
...
\newpage 
\noindent Figure \ref{fig:Plutus_core_kind_synthesis} defines what
...
\begin{figure}[H]
{
    ...
    \begin{prooftree}
      \AxiomC{\(\validJ{\Gamma}\)}
      \RightLabel{tysize}
      \UnaryInfC{\(\hypJ{\Gamma}{\istypeJ{s}{\sizeK{}}}\)}
    \end{prooftree}
...
\noindent In Figure \ref{fig:Plutus_core_type_synthesis}, we define the type
...
\texttt{bytestring}.  There is also a kind \texttt{(size)} which is
used to track the sizes of objects.  For each natural number $s\ge0$
there is a singleton type $\mathit{size}_s::\mathtt{(size)}$ with a
unique member $s$.  Given such a type $t::\mathtt{(size)}$, we denote
its unique member by $\uniqmem{t}$, so that $\uniqmem{\size_s} =
s$. In the Plutus Core syntax the type $\mathit{size}_s$ is denoted by
\texttt{(con size $s$)} and its unique member by \texttt{(con $s$)}.

The \texttt{integer} type, in conjunction with \texttt{size}, provides
integers of specified sizes, and the \texttt{bytestring} type provides
sequences of bytes, again of specified sizes.  These types are given
in Figure~\ref{fig:Plutus_core_type_constants}: for example
...
\texttt{10~!~12345} (written \texttt{(con 10~!~12345)} in the Plutus Core
  concrete syntax)
  is a ten-byte integer whose value is 12345 (and
\texttt{1~!~12345} would cause an error), and \texttt{20~!~\#3ac29d873246f} 
is a 20-byte bytestring containing the bytes
shown\footnote{In fact, the bounds are upper bounds, not actual sizes:
  in the current implementation, \texttt{(con 10~!~12345)} will only take up
  the minimum space required, not the full 10 bytes.}.  We use the
...
\item The types of some built-in functions involve sizes at both the
  type level and the term level.  For example, \texttt{resizeInteger}
  has the type

  $$
  \forall s_0::\mathit{size}\,.\,\forall s_1::\mathit{size}\,.\,\mathit{size}_{s_1}
  \rightarrow \mathit{integer}_{s_0} \rightarrow \mathit{integer}_{s_1},
  $$
   and one might enlarge an integer from size 3 to size 5 by
\begin{lstlisting}
 [{(builtin resizeInteger) (con size 3) (con size 5)}  (con 5) (con 3!200)].
\end{lstlisting}

\noindent The duplication of sizes is necessary because we require
size information at the type level to facilitate gas calculations, and
also at the term level so that the evaluator can check size bounds
at runtime, after type information has been erased.
