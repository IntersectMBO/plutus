...
\paragraph{Implementation.} A compiler and evaluator for Plutus Core can be found at
\url{https://github.com/input-output-hk/plutus}.

\paragraph{Formalisation.} The Agda formalisation of Plutus Core mentioned earlier
can be found
at \url{https://github.com/input-output-hk/plutus/tree/master/metatheory},
...
are explained in~\citep{EUTXO}. See also~\citep{Plutus-book} for 
details of Cardano's smart contract infrastructure.
...
See~\cite{unravelling-recursion} for more details.
...
%
  ...
  \item We use fixed Scott encodings for certain types and values,
    ...
    see Figure~\ref{fig:type_abbreviations} (\textit{one}
    is the unique non-\verb|error| inhabitant of the \textit{unit} type).
...
\section{Examples}
The section contains some examples of Plutus Core code.
...
Note that since Plutus Core has strict semantics, both branches above
will always be evaluated.  Thus even if $n$ is positive the code
above will still calculate $0-n$ and then discard it.  To avoid this we
can delay the evaluation of the branches by making them into functions
taking arguments of type \textit{unit} and only applying the function
once we know the outcome of the test:
