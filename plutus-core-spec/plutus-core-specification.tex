...
in Appendix~\ref{sec:typed-cek-machine}; however, the CK machine still
...
built-in functions awlays terminates, in both the typed and untyped
languages.} If this condition is satisfied then it can be shown that
the erasure operation preserves semantics in an appropriate sense: if
$P$ is a well-typed Plutus Core program then
\begin{itemize}
\item Evaluation of $P$ terminates
if and only if that of $\erase{P}$ does.
\item The evaluation of $P$ erminates in the error state $\blacklozenge$ if
and only if the evaluation of $\erase{P}$ also terminates in $\blacklozenge$.
\item $P$ evaluates to the value $V$ if and only if $\erase{P}$ evaluates to the value
$\erase{V}$.
\end{itemize}
This allows us to give semantics to those untyped
...
the CK machine (Figure~\ref{fig:typed-ck-machine}) which evaluates type-erased Plutus
Core programs.
...
machine can deal with closures, and that
\begin{itemize}
\item Evaluation of builtin functions always terminates.
...
\item For closures $C_1, \ldots, C_n$,
  \texttt{(builtin}\ \textit{bn} $A_1 \ldots A_k\  C_1 \ldots C_n$ \texttt{)}
evalutes to a closure $C$ in the CEK machine if and only if 
  \texttt{(builtin}\ \textit{bn} $A_1 \ldots A_k\  \overline{C_1} \ldots \overline{C_n}$ \texttt{)}
...
then use the same mechanism as in the CK machine, but this would
reintroduce the very inefficiencies which the CEK machine is designed
to avoid.)  A future version of this document will contain a
...
the following facts (assuming that the condition in Section~\ref{sec:typed-cek-builtins} holds):
\begin{itemize}
...
and only if the CK also terminates in the state $\blacklozenge$.
...
(Section~\ref{sec:typed-cek-machine}), except that environments now
...
builtin evaluation is performed in the untyped CEK machine. We assume
that the mechanism for evaluating built-in functions in the
untyped CEK machine can deal with closures, and that built-in
evaluation commutes with type erasure and discharge of environments in
an appropriate sense; more precisely, we require that the conditions
below hold.
...
fails if and only if evaluation of 
...
the untyped CEK machine also halts in the $\blacklozenge$ state.
