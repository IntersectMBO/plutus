...
there is a singleton type $\mathit{size}_s::\mathtt{(size)}$ with a unique
member $s$.  In the Plutus Core syntax, the type $\mathit{size}_s$ is denoted
by \texttt{(con size $s$)} and its unique member by \texttt{(con $s$)}.
