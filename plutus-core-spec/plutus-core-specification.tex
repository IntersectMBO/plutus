...
\blue{Say that the type system is expressive enough to let us type 
enough stuff to encode datatypes and use recursive functions}.
...
through, for example, the Scott encoding (\cite{Scott-encoding} and
\cite{Koopman:2014}, for example). Similarly, there is no explicit
support for recursive definitions, but the presence of type-level
fixed points allows us to form term-level fixed points, and thus to
write recursive functions in Plutus Core.
...
on-chain code.  Units of so-called \textit{gas} are consumed as a program
runs, and users must buy gas in advance.  If a program terminates successfully
then a refund is given for any unused gas, but if the program runs out of gas
then the payment is forfeited; similarly, if a program terminates early
due to some error condition then the gas used up to the point of termination
has to be paid for.
...
Plutus Core Code, but nothing else.  

\section{Syntax}

The grammar of Plutus Core is given in
Fgiures~\ref{fig:Plutus_core_lexical_grammar} and
...
this. Plutus Core programs are intended exist only as ASTs produced by
...
\newcommand\fixtype{\mu\,\alpha.A}  % Just for talking about the fix operator in the notes.

\subsection{Notes on the grammar}
\paragraph{Terms.}
\begin{itemize}
\item $\con{cn}$ represents a built-in constant: see \S\ref{sec:builtins}
\item $\abs{\alpha}{K}{V}$ represents a polymorphic value abstracted over a type; this would often be donated 
  by $\Lambda\alpha{::}K.V$.  Note that we require the body of the abstraction to be a \textit{value}, not a term.
  This is analogous to the value restriction in SML and is required \blue{WHY???}.
\item $\inst{M}{A}$ represents a polymorphic term instantiated at a particular type.
\item $\wrap{\alpha}{A}{V}$ and $\unwrap{M}$: see the note on isorecursive types below.
...
\item $\error{A}$ causes an error, termintating computation immediately.
\end{itemize}
...
\paragraph{Types.} Plutus Core contains a copy of the simply typed lambda calculus
  at the type level, and most of the 
\begin{itemize}
...
\item $\fixT{\alpha}{A}$ is a type-level fixpoint operator $\fixtype$: see the note on isorecursive types below.
...
\paragraph{Type Values.} All types can be reduced to a type value (or
  ...
  process always terminates. \blue{(WHY?)}



\subsection{Remarks}
\subsubsection{Type Normalisation}
 Type checking requires normalisation to be performed, for
  example to check that two types are equal.  This process can be
  expensive, so we require that code supplied for on-chain execution
  comes with all type annotations pre-normalised.  This can be done
  during off-chain type checking.
...
\subsubsection{Isorecursive types}
The fixpoint operator $\fixT{\alpha}{A}$ allows one to define
recursive types in Plutus Core: $\fixT{\alpha}{A}$ is a type
$\fixtype$ such that 
$\fixtype \cong (\fixtype)[\alpha \mapsto \fixtype]$.  
Note that we have an \textit{isomporhism} of types here rather than equality.  
Fixpoint types are \textit{isorecursive}~\cite[20.2]{Pierce:TAPL}
with explicit maps \texttt{wrap}: $(\fixtype)[\alpha \mapsto \fixtype] \rightarrow \fixtype$
and \texttt{unwrap}: $\fixtype \rightarrow (\fixtype)[\alpha \mapsto \fixtype]$.  
\\

\noindent\blue{Should we give a more detailed explanation of why we do this? Do we want an example here?}
...
implement the operational semantics described in
Section~\ref{sec:reduction}.  The CK machine is inefficient because it
implements application as $\beta$-reduction, so that when evaluating
$(\lambda x.M)V$, $V$ must be substituted bodily for $x$ wherever it
occurs in $M$.  This process can require considerable amounts of time
and space.  More efficient machines are available (for example the CEK
machine from~\cite{Felleisen-CK-CEK}, which uses environments binding
actual arguments to variables), and in practice we would use something
based on such a machine; however, the CK machine will still provide a
useful bridge for formalisation purposes.

\subfile{figures/PlutusCoreCkMachine}

\blue{There's a conflict with earlier substitution notation in $[V/x]M$. Does this matter?}

The machine alternates between two main phases: \textit{computation}
...
\label{sec:builtins}
Plutus Core come with a pre-defined set of built-in types and
...
\blue{Is it \texttt{con} or \texttt{builtin}?  I think there's a 
conflict between Figure~\ref{fig:Plutus_core_type_constants}
...
We provide standard artithmetic and comparison operations for integers
