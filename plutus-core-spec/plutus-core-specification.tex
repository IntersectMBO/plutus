...
in Appendix~\ref{sec:cek-machine}; however, the CK machine still
...
behave sensibly with respect to type erasure in the sense that
$$
...
untyped on the right).  If this condition is satisfied then it can be
shown that the erasure operation preserves semantics in an appropriate
sense: if $M$ is a well-typed Plutus Core term then evaluation of $M$
terminates if and only if that of $\erase{M}$ does, and $M$ evaluates
to the value $v$ if and only if $\erase{M}$ evaluates to the value
$\erase{v}$.  This allows us to give semantics to those untyped
programs which are obtained by erasing types from well-typed typed
programs.  A future version of this document will contain a detailed
specification of built-in types and functions which will allow us to
give precise semantics for \textit{all} untyped terms and programs.

\subsection{Term Reduction}
Figure~\ref{fig:untyped-term-reduction} defines a reduction semantics
for type-erased Plutus Core, analogously to
Figure~\ref{fig:term-reduction}.  \subfile{figures/UntypedTermReduction}
...
the CK machine (Figure~\ref{fig:ck-machine}) which evaluates type-erased Plutus
Core programs.
...
\label{sec:cek-machine}
In Section~\ref{sec:ck-machine} we described the CK machine, an
...
~\citep{Felleisen-CK-CEK}), which uses environments to store bindings
of values to variables, where they can be looked up when
required. This makes evaluation considerably more efficient, indeed to
such an extent that the CEK machine can be used in practical
situations.

\subsection{Structure of the CEK machine}
The CEK machine avoid the inefficiency of subsitiution by extending
the state of the machine with
\textit{environments} mapping variable names to \textit{closures} consisting of
values paired with environments; environments grow from left to right,
  and the result of looking up a variable $x$ in an environment $\rho$
  is denoted by $\rho[x]$: if $x$ is has multiple bindings in $\rho$
  then the most recent one is returned.\footnote{In the
  implementation, names are supplied with unique integer IDs which
  make them unambiguous; the environments in fact map these IDs to
  closures.} If $x$ does not occur in $\rho$ then an \texttt{error} is
  returned; however, it can be shown that if the machine is started
  with a closed term $M$ as input, then variable lookups will never
  fail.  When the machine is evaluating $[(\lambda x.M)\, N]$ in an
  environment $\rho$ it first evaluates $N$ in $\rho$ to produce a
  value $V$, then extends $\rho$ with the mapping $x \mapsto (V,\rho)$
  to obtain a new environment $\rho^{\prime}$, then proceeds to
  evaluate $M$ in the environment $\rho^\prime$: when any occurrences
  of $x$ are encountered, the machine retrieves the value $V$ from the
  environment and proceeds with evaluation as normal.  It is necessary
  to use closures because values may now contain variable names, and
  we have to know what the variables were bound to at the time when
...
The CEK machine is described in Figure~\ref{fig:cek-machine}.  It is
  is very similar to the CK machine of Figure~\ref{fig:ck-machine},
  except that
\begin{itemize}
...
\item When the machine terminates without an \texttt{error} it now
returns a \textit{closure} $(V,\rho)$ rather than a value.  The invoker
...
\subfile{figures/CekMachine.tex}
...
machine can deal with closures, and that the result of evaluating
\texttt{(builtin } \textit{bn} $(V_1, \rho_1) \ldots (V_n, \rho_n)$ \texttt{)}
in the CEK machine is a closure $(W,\tau)$ such that
$\overline{(W,\tau)}$ is identical to the value given by
evaluating \texttt{(builtin } \textit{bn}
$\overline{(V_1, \rho_1)} \ldots \overline{(V_n, \rho_n)}$ \texttt{)}
in the CK machine according to Figure~\ref{fig:builtins} (one way to
achieve this would be to discharge all of the closures and then use
the same mechanism as in the CK machine, but this would reintroduce
the very inefficiencies which the CEK machine is designed to avoid).
A future version of this document will contain a specification of
built-in function evaluation which will cover this issue in detail.

\subsubsection{Comparison of the CK and CEK machines}
If the CK and CEK machines are run with the same input then we can prove
the following facts:
\begin{itemize}
\item The CK machine terminates if and only if the CEK machine terminates.
\item The CK machine returns \texttt{(error A)} if and only if the CEK machine returns
\texttt{(error A)}.
\item The CK machine returns a value $V$ if and only if the CEK machine returns a
closure $(W,\rho)$ with $\overline{(W,\rho)} = V$.
\end{itemize}

\section{A CEK machine for type-erased Plutus Core}
\label{sec:untyped-cek-machine}
...
similar to which we those used to obtain the CEK machine for typed
...
The untyped CEK machine uses environments similar to those used in the
typed CEK machine (Section~\ref{sec:typed-cek-machine}), except that
these now bind variables to terms in untyped Plutus Core.  Again,
$\overline{(V,\rho)}$ denotes the value obtained by discharging a closure
$(V,\rho)$.
...
builtin evaluation is performed in the CEK machine for type-erased
Plutus Core. We assume that the mechanism for evaluating built-in
functions in the type-erased CEK machine can deal with closures, and
that built-in evaluation commutes with type erasure and discharge of
environments in an appropriate sense; more precisely, we require that
the condition below holds.

\medskip
\noindent Suppose that $V_1, \ldots, V_n$ are well-typed values in type Plutus Core and that
in the CK machine $$
\texttt{(builtin } \textit{bn}\ V_1 \ldots V_n \texttt{)}
$$
evaluates to the value $W$ according to Figure~\ref{fig:builtins}.  Suppose further that
$(W_1,\rho_1), \ldots, (W_n, \rho_n)$ are closures in the untyped
CEK machine such that $\erase{V_i} = \overline{(W_i, \rho_i)}$ for
$1 \le i \le n$ and that
$$
\texttt{(builtin } \textit{bn}\ (\erase{V_1}, \rho_1) \ldots (\erase{V_n}, \rho_n) \texttt{)}  
$$
evaluates to a closure $(W,\rho)$ in the untyped CEK machine.  Then we require that
$$
   \overline{(W,\rho)} = \erase{V}.
$$


\subsubsection{Comparison of the typed CK machine and the type-erased CEK machine}
Suppose that the condition of Section~\ref{sec:untyped-cek-builtins}
holds, and that $P$ is a typed Plutus Core program.  Then if $P$ is
...
\item The typed CK machine returns \texttt{(error A)} for some type $A$ if and only if
the untyped CEK machine returns \texttt{(error)}.
\item The typed CK machine returns a value $V$ if and only if the untyped CEK machine returns a
closure $(W,\rho)$ with $\overline{(W,\rho)} = \erase{V}$.
\end{itemize}
