...
\thispagestyle{empty}
 \begin{figure*}
    \hspace{-3cm}Let $txh$ be the transaction hash and $bnum$ be the block number, both as global parameters to normalization.\vspace{1em}

    \hspace{-3cm}\(\begin{array}{llllll}
...
þ       \texttt{greaterThanEqualsInteger}   &  \sig{s :: size}{integer_s, integer_s}{boolean}  &  s & s!i_0 , s!i_1   &   i_0 >= i_1\\
    ...
    \hspace{-3cm}$^{\dagger}$ \texttt{txhash} and its meaning $txh$ are the hash of the transaction, containing information as specified by the host blockchain.

    \hspace{-3cm}\caption{Builtin Signatures and Reductions}
