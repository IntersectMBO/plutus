...
\begin{subfigure}[c]{\linewidth}
    ...
    \begin{prooftree}
        \AxiomC{$bn$ computes on $\repetition{V}$ to $U$ according to Figure \ref{fig:builtins}}
        \UnaryInfC{\(\step{\builtinU{bn}{\repetition{\erase{V}}}}{\erase{U}}\)}
    \end{prooftree}
...
%% Faffing about with minipages to get two rules on the same line to avoid bad page break
    \hfill\begin{minipage}{0.3\linewidth}  
      \begin{prooftree}
        \AxiomC{\strut} % struts to get rules aligned 
        \UnaryInfC{\(\step{\ctxsubst{f}{\errorU}}{\errorU}\)}
      \end{prooftree}
    \end{minipage}
    \begin{minipage}{0.3\linewidth}
    \begin{prooftree}
        \AxiomC{\strut\(\step{M}{M'}\)}
...
\\
    \medskip
    \caption*{(b) Reduction via Contextual Dynamics} %% Oops
    \label{fig:untyped-reduction}
\end{subfigure}

\caption{Term Reduction for Erased Plutus Core}
