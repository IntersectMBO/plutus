...
... | step (ruleEC E p) = step (ruleEC (E l· M') p)
... | step (ruleErr E)  = step (ruleErr (E l· M'))
... | error E-error = step (ruleErr ([] l· M'))
... | done V with progress M'
... | step (ruleEC E q) = step (ruleEC (V ·r E) q)
... | step (ruleErr E)  = step (ruleErr (V ·r E))
... | error E-error = step (ruleErr (V ·r []))
progress (.(ƛ M) · M') | done (V-ƛ M) | done W = step (ruleEC [] (β-ƛ W))
progress (M · M') | done (V-I⇒ b p q r σ base x .M) | done W = step (ruleEC [] (β-sbuiltin b σ p q _ r _ (deval W) x W))
...
... | done (V-Λ M') = step (ruleEC [] β-Λ)
... | done (V-IΠ b p q r σ base x .M) =
  step (ruleEC [] (β-sbuiltin⋆ b σ p q _ r M x))
... | done (V-IΠ b {C = C} p q r σ (skip⋆ p₁) x .M) =
  done (convValue (Πlem p₁ A C σ) (V-IΠ b {C = C} p q r (subNf-cons σ A) p₁ (x ,, A) (conv⊢ refl (Πlem p₁ A C σ) (M ·⋆ A))) )
... | done (V-IΠ b {C = C} p q r σ (skip p₁) x .M) =
  done (convValue (⇒lem p₁ σ C) (V-I⇒ b p q r (subNf-cons σ A) p₁ (x ,, A) (conv⊢ refl (⇒lem p₁ σ C) (M ·⋆ A) )))
... | error E-error     = step (ruleErr ([] ·⋆ A))
progress (wrap A B M) with progress M
... | done V            = done (V-wrap V)
... | step (ruleEC E p) = step (ruleEC (wrap E) p)
... | step (ruleErr E)  = step (ruleErr (wrap E))
... | error E-error     = step (ruleErr (wrap []))
progress (unwrap M) with progress M
... | step (ruleEC E p) = step (ruleEC (unwrap E) p)
... | step (ruleErr E) = step (ruleErr (unwrap E))
... | done (V-wrap V) = step (ruleEC [] (β-wrap V))
