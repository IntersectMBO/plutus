...
module Rules where
  ...
  data CaseConstr : Relation where
    case-constr :
      ∀ {i : ℕ} →
      lookup? i Ns ≡ just N →
      -------------------------------------------------
      CaseConstr (case (constr i Ms) Ns) (iterApp N Ms)

  data CaseUnit : Relation where
    case-unit :
       ---------------------------------------------
       CaseUnit (case (con (tmCon unit tt)) [ N ]) N

  data CaseFalse₁ : Relation where
    case-false₁ :
      --------------------------------------------------
      CaseFalse₁ (case (con (tmCon bool false)) [ N ]) N

  data CaseBool : Relation where
    case-bool :
      ∀ {b}  →
      -----------------------------------------------------------------------
      CaseBool (case (con (tmCon bool b)) (N₁ ∷ N₂ ∷ [])) (if b then N₂ else N₁)

  data CaseInteger : Relation where
    case-integer :
      ∀ {n} →
      lookup? n Ns ≡ just N →
      ---------------------------------------------------
      CaseInteger (case (con (tmCon integer (+ n))) Ns) N

  data CaseCons₁ : Relation where
    case-cons₁ :
      ∀ {A x xs} →
      ----------------------------------------------------
      CaseCons₁
  ...
  data CaseNil : Relation where
    case-nil :
      ∀ {A} →
      -----------------------------------------------------------
      CaseNil (case (con (tmCon (list A) nil)) (N₁ ∷ N₂ ∷ [])) N₂

  data CasePair : Relation where
    case-pair :
      ∀ {A B x y} →
      ----------------------------------------------------
      CasePair
  ...
  data Reflexivity : Relation where
    ~-refl : Reflexivity M M
