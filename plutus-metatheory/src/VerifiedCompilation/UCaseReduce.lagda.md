...
open import VerifiedCompilation.Certificate using (ProofOrCE; ce; proof; caseReduceTag)
...
... | no ¬p = ce (λ { (casereduce _ _) → ¬p (iscase (isconstr _ (allterms _)) (allterms _))} ) caseReduceTag ast ast'
... | yes (iscase (isconstr i (allterms vs)) (allterms xs)) with lookup? i xs in xv
...          | nothing = ce (λ { (casereduce p _) → case trans (sym xv) p of λ { () }} ) caseReduceTag ast ast'
...
isCaseReduce? = translation? caseReduceTag isCR?
