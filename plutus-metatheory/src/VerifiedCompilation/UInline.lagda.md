...
open import VerifiedCompilation.Certificate using (Proof?; _>>=_; abort; proof; inlineT)
```
...
data ann : Set where
  ...
  error   : ann
  constr : List ann → ann
  case   : ann → List ann → ann
```

# Check aided by annotations
...
check a σ zz M M′ = abort inlineT M M′
