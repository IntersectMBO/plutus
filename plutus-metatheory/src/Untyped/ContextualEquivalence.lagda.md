...
data Irreducible : (X ⊢) → Set₁ where
  irreducible : {x : X ⊢} → ¬ (∃[ y ] x ⟶ y) → Irreducible x

data Stable : (X ⊢-⇛ Y ⊢) → Set₁ where
  □ : Stable {X} □
  ƛ : {𝒞 : X ⊢-⇛ (Maybe Y) ⊢} → Stable 𝒞 → Stable (ƛ□ 𝒞) -- Should this always be "stable" since λs are Values?
  _□·_ : {𝒞 : X ⊢-⇛ Y ⊢}
           {M : Y ⊢}
           → Stable 𝒞
           → Value M
...
𝒞-⟶ : ∀ (𝒞 : X ⊢-⇛ ⊥ ⊢) {M N : X ⊢} → Stable 𝒞 → M ⟶ N → 𝒞 ⟨ M ⟩ ⟶ 𝒞 ⟨ N ⟩
𝒞-⟶ □ S M⟶N = M⟶N
𝒞-⟶ (ƛ□ 𝒞) S M⟶N = {!!}
𝒞-⟶ (𝒞 □· M) S M⟶N = {!!} --_⟶_.ξ₁ (𝒞-⟶ 𝒞 M⟶N)
𝒞-⟶ (L ·□ 𝒞) S M⟶N = {!!}

-- Evaluation contexts instead...
--
-- ℰ ⟨ subst σ M ⟩ ⇓ → ℰ ⟨ subst σ N ⟩ ⇓
-- σ is a "closing substitution"
-- If M in Env Γ then everything in Γ is mapped to a term in ⊥

𝒞⇓ : ∀ (𝒞 : X ⊢-⇛ ⊥ ⊢) {M N : X ⊢} → Stable 𝒞 → (M ⇓ → N ⇓) → 𝒞 ⟨ M ⟩ ⇓ → 𝒞 ⟨ N ⟩ ⇓
𝒞⇓ □ S M⇓→N⇓ 𝒞M⇓ = M⇓→N⇓ 𝒞M⇓
𝒞⇓ (ƛ□ 𝒞) S M⇓→N⇓ 𝒞M⇓ = (ƛ□ 𝒞 ⟨ _ ⟩) , Value.ƛ , _⟶*_.refl
𝒞⇓ (𝒞 □· R) {M} {N} (S □· VR) M⇓→N⇓ 𝒞M⇓ = {!!} , {!!} , {!!} -- If R is not a Value is this ever true?
𝒞⇓ (L ·□ 𝒞) {M} {N} (VL ·□ S) M⇓→N⇓ 𝒞M⇓ = {!!}
