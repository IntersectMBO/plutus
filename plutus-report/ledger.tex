...
This means that to be allowed to mint \glspl{token} of a particular \gls{currency}, you must present the corresponding \gls{mps} with the transaction and it must pass.

This design allows our system to be both \emph{local} and \emph{lightweight}:
\begin{itemize}
\item
  We do not require any kind of central registration of \glspl{currency}: any hash can be used as a \gls{currency-id}, you just cannot mint any unless you can produce a \gls{script} with that hash.
  This ensures we do not violate \cref{req:ledger-locality}.
\item
  \Gls{minting} \glspl{token} is cheap: all that is required is to provide and run the script at the time of minting.
\item
...
The \gls{currency-id} indicates the \emph{controller} of the currency, specifically it is the hash of the \gls{mps} that controls when tokens of that currency may be minted or burned (see \cref{sec:minting}).
...
\subsubsection{\Glsentrytext{minting}}
\label{sec:minting}
\Gls{value} may also be created and destroyed, we call this \emph{\gls{minting}}.
To this end transactions are given two additional fields:
\begin{enumerate}
\item \code{mint} which contains \gls{value}
\item \code{mintScripts} which contains \glspl{mps} for all the \glspl{currency} minted in \code{mint}
\end{enumerate}

\subsubsection{\glsentrylong{mps}s}
The \glspl{script} which control the minting of \glspl{token} are called \glsfirstplural{mps}.
...
\begin{itemize}
\item Minting must be authorized: for each \gls{currency-id} of which a non-zero amount is minted in \code{mint}, the corresponding \gls{mps} must be run with the \gls{context} and return successfully.
\end{itemize}
