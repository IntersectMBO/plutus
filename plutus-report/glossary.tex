...
\newglossaryentry{ada}{
  name=ada,
  description={
    The primary currency on the \gls{cardano} blockchain. Unlike other \glspl{currency}, ada cannot be minted.
  }
}
...
\newglossaryentry{currency}{
  ...
  plural=currencies,
  description={
    A class of \gls{token} whose \gls{minting} is controlled by a particular \gls{mps}.
  }
}
\newglossaryentry{currency-id}{
  ...
  description={
    ...
    This is the hash of the \gls{mps} that controls the \gls{minting} of the \gls{currency}.
  }
}
...
\newglossaryentry{minting}{
  name=minting,
  description={
    A transaction which mints \glspl{token} creates new \glspl{token}, providing that the
    corresponding \gls{mps} is satisfied.
    The amount minted can be negative, in which case the \glspl{token} will be destroyed instead of created.
...
\newacronym[description={
  A \gls{script} which must be satisfied in order for a transaction to mint \glspl{token} of the corresponding \gls{currency}.
  See \cref{sec:multicurrency}.
}]{mps}{MPS}{Minting Policy Script}
