      ...
      ({ lib, pkgs, buildProject, ... }: {
        options = {
          ...
          coverage = lib.mkOption {
            type = lib.types.bool;
            description = "Enable Haskell Program Coverage for plutus libraries and test suites.";
            default = false;
          };
        };
      })
      ...
      ({ pkgs, config, ... }: {
        ...
        src = let root = ../../../.; in
          haskell-nix.haskellLib.cleanSourceWith {
            ...
            src = root;
            # Otherwise this depends on the name in the parent directory, which reduces caching, and
            # is particularly bad on Hercules, see https://github.com/hercules-ci/support/issues/40
            name = "plutus";
          };
        sha256map = {
        ...
        modules = [
          ...
          ({ pkgs, ... }: lib.mkIf (pkgs.stdenv.hostPlatform != pkgs.stdenv.buildPlatform) {
            packages = {
              ...
              # This contains support for doing testing, so we're not interested in cross-compiling
