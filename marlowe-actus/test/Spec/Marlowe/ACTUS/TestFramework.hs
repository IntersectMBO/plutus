...
import           Data.Vector                                       as Vector (Vector (..), catMaybes, head, map, toList)
import           GHC.Generics                                      (Generic)
import           GHC.Records                                       (getField)
import           Language.Marlowe.ACTUS.Analysis
import           Language.Marlowe.ACTUS.Definitions.BusinessEvents
import           Language.Marlowe.ACTUS.Definitions.ContractTerms  hiding (Assertion, Object)
...
testToStructure :: TestCase -> [ContractStructure]
testToStructure tc = let t = terms tc in maybe [] valueToStructure (Map.lookup "contractStructure" t)
  where
    valueToStructure :: Value -> [ContractStructure]
    valueToStructure (Array v) = let e = Vector.head v
                                  in [ContractStructure
                                         (unpack $ g "object" e)
                                         (read . unpack $ f "referenceType" e)
                                         (read . unpack $ f "referenceRole" e)
                                     ]
    valueToStructure _  = []

    f l (Object x) = case x HashMap.! l of String x -> x
    f _ _          = undefined

    g l (Object x) = case x HashMap.! l of
                       Object x -> case x HashMap.! "marketObjectCode" of
                                     String x -> x
...
testToContractTerms tc@TestCase{terms = t} =
  ...
  in ContractTermsPoly
     ...
     , ct_OPTP           = readMaybe (maybeConcatPrefix "OPTP_" (Map.lookup "optionType" terms')) :: Maybe OPTP
     , ct_OPS1           = readMaybe $ Map.lookup "optionStrike1" terms' :: Maybe Double
     , ct_OPXT           = readMaybe (maybeConcatPrefix "OPXT_" (Map.lookup "optionExerciseType" terms')) :: Maybe OPXT
     , ct_STP            = parseMaybeCycle $ Map.lookup "settlementPeriod" terms'
...
parseCycle (_ : rest) =
  let n' = read (takeWhile (< 'A') rest) :: Integer
   in case dropWhile (< 'A') rest of
        ...
        [p'] -> do
          return $ Cycle {n = n', p = read $ "P_" ++ [p'] :: Period, stub = LongStub, includeEndDay = False}
        _ -> Nothing
