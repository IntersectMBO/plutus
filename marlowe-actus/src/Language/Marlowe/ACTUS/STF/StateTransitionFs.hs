...
module Language.Marlowe.ACTUS.STF.StateTransitionFs (stateTransitionFs) where
...
stateTransitionFs :: EventType -> ContractTerms -> Integer -> Contract -> Contract
stateTransitionFs ev terms@ContractTerms{..} t continue = 
    let 
    ...
    in case contractType of
        PAM -> 
            stateTransition ev t continue $ \event st -> case event of 
