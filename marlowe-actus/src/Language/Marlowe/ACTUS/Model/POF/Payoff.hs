...
payoff ev RiskFactors{..} ContractTerms{..} ContractStatePoly {..} t =
    let
        y_sd_t = _y (fromJust ct_DCC) sd t ct_MD
    in case contractType of
        PAM ->
            case ev of
                ...
                FP  -> _POF_FP_PAM (fromJust ct_FEB) (fromJust ct_FER) o_rf_CURS ct_CNTRL nt fac y_sd_t
        ...
        LAM ->
            case ev of
                IED -> _POF_IED_LAM o_rf_CURS ct_CNTRL (fromJust ct_NT) (fromJust ct_PDIED)
                ...
                PY  -> _POF_PY_LAM (fromJust ct_PYTP) o_rf_CURS o_rf_RRMO (fromJust ct_PYRT) ct_cPYRT ct_CNTRL nt ipnr y_sd_t
                FP  -> _POF_FP_LAM (fromJust ct_FEB) (fromJust ct_FER) o_rf_CURS ct_CNTRL nt fac y_sd_t
        ...
        NAM ->
            case ev of
                IED -> _POF_IED_NAM o_rf_CURS ct_CNTRL (fromJust ct_NT) (fromJust ct_PDIED)
                ...
                PY  -> _POF_PY_NAM (fromJust ct_PYTP) o_rf_CURS o_rf_RRMO (fromJust ct_PYRT) ct_cPYRT ct_CNTRL nt ipnr y_sd_t
                FP  -> _POF_FP_NAM (fromJust ct_FEB) (fromJust ct_FER) o_rf_CURS ct_CNTRL nt fac y_sd_t
