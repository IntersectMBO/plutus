\begin{code}
module ValueTypes.Evaluation where
\end{code}
...
open import ValueTypes.Term.Reduction
...
\begin{code}
data Finished {Γ J}{A : ∥ Γ ∥ ⊢V⋆ J} :  (N : Γ ⊢ A) →  Set where
...
reduction finished.
\begin{code}
data Steps : ∀ {J}{A : ∅ ⊢V⋆ J} → ∅ ⊢ A → Set where
  ...
  unhandled-conversion :  ∀ {J}{A : ∅ ⊢V⋆ J} {L : ∅ ⊢ A} → Steps L
