...
module Language.PlutusCore.Untyped.Constant.Typed
    ( TypeScheme (..)
    , TypedBuiltinName (..)
...
import           Language.PlutusCore.Untyped.Pretty
...
type Evaluator f m = DynamicBuiltinNameMeanings -> f Name () -> m EvaluationResultDef
...
readKnownM
    :: (Monad m, KnownType a)
    => Term Name () -> EvaluateT ReflectT m a
