...
type EmitHandler uni r = DynamicBuiltinNameMeanings (WithMemory Term uni) -> Plain Term uni -> IO r
...
withEmitTerm
    ...
    => (EmitHandler uni r1 -> Plain Term uni -> IO r2)
    -> EmitHandler uni r1
    -> IO ([a], r2)
withEmitTerm cont handler =
    withEmit $ \emit -> do
        ...
        let dynEmitName = DynamicBuiltinName $ "emit" <> display counter
            dynEmitTerm = dynamicCall (Proxy @ExMemory) (Proxy @a) dynEmitName
