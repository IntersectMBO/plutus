...
data Shallow uni uni' = Shallow
    { _shallowEmbed :: NameMeaning uni -> NameMeaning uni'
    , _shallowMeans :: NameMeanings uni'
    , _shallowTerm  :: Term TyName Name uni' ()
    }

class KnownShallow a uni uni' | a uni -> uni' where
    toShallow :: proxy a -> Term TyName Name uni () -> Shallow uni uni'
...
class PrettyKnown a => KnownType a uni where
    ...
    default readKnown
        :: (Monad m, KnownShallow a uni uni', Evaluable uni')
        => Evaluator Term uni m -> Term TyName Name uni () -> ReflectT m a
    readKnown (Evaluator eval) term = fix $ \proxy -> do
        let Shallow embed means applied = toShallow proxy term
        res <- makeRightReflectT $ eval embed means applied
        _ res
--         case extractExtension res of
--             Just b  -> pure b
--             Nothing -> throwError "Can't read"
...
readKnownM
    :: (Monad m, KnownType a uni, Evaluable (Extend a uni))
...
instance KnownShallow () uni (Extend () uni) where
    toShallow term = Shallow shiftNameMeaning mempty applied where
        metaUnit = Mk.extensionType ()
...
instance KnownShallow Bool uni (Extend Bool uni) where
    toShallow term = Shallow shiftNameMeaning mempty applied where
        metaBool  = extensionType ()
