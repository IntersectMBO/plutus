...
instance Pretty a => PrettyCfg (TypeError a) where
    ...
    prettyCfg cfg (TypeMismatch x t ty ty') = "Type mismatch at" <+> pretty x <+> "in term" <> hardline <> indent 2 (squotes (prettyCfg cfg t)) <> "." <> hardline <> "Expected type" <+> squotes (prettyCfg cfg ty) <+> ", found type" <> hardline <> indent 2 (squotes (prettyCfg cfg ty'))
