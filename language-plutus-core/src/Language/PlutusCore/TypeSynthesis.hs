...
instance Pretty a => PrettyCfg (TypeError a) where
    ...
    prettyCfg cfg (TypeMismatch x t ty ty') = "Type mismatch at" <+> pretty x <+> "in term" <//> squotes (prettyCfg cfg t) <> "." <> hardline <> "Expected type" <//> squotes (prettyCfg cfg ty) <> "," <> hardline <> "found type" <//> squotes (prettyCfg cfg ty')
