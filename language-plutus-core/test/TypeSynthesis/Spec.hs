...
kindcheckQuoted getType = do
    ...
    _ <- annotateType ty >>= kindCheck (TypeCheckCfg Nothing $ TypeConfig True mempty)
    return ty
...
typecheckQuoted getTerm = do
    ...
    _ <- annotateTerm term >>= typecheckTerm (TypeCheckCfg (Just 1000) $ TypeConfig True mempty)
    return term
