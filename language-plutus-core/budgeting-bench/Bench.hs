    ...
    bgroup (show name) $
        as <&> (\(x, xMem) ->
            bgroup (show xMem) $ bs <&> (\(y, yMem) ->
                runTermBench (show yMem) $ applyBuiltin () (StaticBuiltinName name) [] [(mkConstant () x), (mkConstant () y)]
            ))
...
benchHashOperations name =
    bgroup (show name) $
        byteStringsToBench seedA <&> (\(x, xMem) ->
            runTermBench (show xMem) $ applyBuiltin () (StaticBuiltinName name) [] [(mkConstant () x)]
        )
...
benchVerifySignature =
    bgroup (show name) $
        bs <&> (\(x, xMem) ->
            runTermBench (show xMem) $ applyBuiltin () (StaticBuiltinName name) [] [(mkConstant () pubKey), (mkConstant () x), (mkConstant () sig)]
        )
    where
