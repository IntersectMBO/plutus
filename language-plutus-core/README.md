...
The CK machine can be used to evaluate programs. For this, feed a type checked program to the `runCk` function defined in the [`Language/PlutusCore/CkMachine.hs`](src/Language/PlutusCore/CkMachine.hs) module:
...
A term generation machinery sits in the [`Language/PlutusCore/TestSupport/Generator.hs`](src/Language/PlutusCore/TestSupport/Generator.hs) module. It allows to generate terms that contain built-ins (integers, bytestrings, sizes and booleans), constant applications and first-order functions. E.g.
