...
The goal is twofold:
To provide guidance in cases where it is not obvious what to do, or the choice is arbitrary.
To help resolve disagreements between team members. Ideally these can be settled by referring to a recommendation in this document, or if not, then perhaps the resolution of the disagreement will create a new recommendation.
...
=== When to use hierarchical re-exporting modules
...
1. Anything in https://github.com/ghc-proposals/ghc-proposals/pull/380[`GHC2021`]. Once we have a GHC version that supports the GHC2021 language, we will likely switch to using it.
2. `LambdaCase`: clear, helpful
3. `DerivingStrategies`: always
4. `GADTs`: well established, useful
5. `OverloadedStrings`: essential when working with `Text`, which you should
6. `NegativeLiterals`
7. `DerivingVia`: great
8. `RoleAnnotations`: if you need it, you need it
...
1. `RecordWildCards`
2. `TypeFamilies`: often very useful, but can make things tricky. Think before using.
3. `DataKinds`
4. `FunctionalDependencies`
5. `ViewPatterns`: can be very nice, can be a huge mess
6. `OverloadedLists`: sometimes a lifesaver, not as indispensable as `OverloadedStrings`
...
// Group your imports by source
// Organize your imports into blocks as follows:
// Imports from the current package
// Imports from the other packages in the project
// Imports from other packages not in the project
// ==== Reasoning
// It looks tidy?
...
// We don’t have any guidelines on what to put in cabal file metadata :( really this should be sorted out at a company level, perhaps.
