...
import PlutusCore.Evaluation.Machine.ExBudget (ExRestrictingBudget (..))
...
printBudgetStateTally term model (Cek.CekExTally costs) = do
  ...
  where
    ...
    computeStepKinds = takeWhile (/= Cek.BMatch) allStepKinds
    patternStepKinds = dropWhile (/= Cek.BMatch) allStepKinds
    getSpent k =
    ...
    totalComputeCost =
      ...
      foldMap (getSpent . Cek.BStep) computeStepKinds
    totalPatternCost = foldMap (getSpent . Cek.BStep) patternStepKinds
...
writeSerialised :: Output -> B.ByteString -> IO ()
writeSerialised outp prog = do
  case outp of
    FileOutput file -> B.writeFile file prog
