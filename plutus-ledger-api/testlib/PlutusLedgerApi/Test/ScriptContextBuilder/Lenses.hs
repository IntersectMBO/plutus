...
txInfoRedeemersAssocL = L.lens PV3.txInfoRedeemers (\ti r -> ti {PV3.txInfoRedeemers = r})

mintValueMapL :: L.Lens' MintValue (Map PV1.CurrencySymbol (Map PV1.TokenName Integer))
mintValueMapL = L.lens getter setter
  where
    getter mp = Map.fromList $ AssocMap.toList $ PlutusTx.fmap (Map.fromList . AssocMap.toList) $ mintValueToMap mp
    setter _ v =
      UnsafeMintValue $
...
valueMapL = L.lens getter setter
  where
    getter mp =
      Map.fromList $
        AssocMap.toList $
          PlutusTx.fmap (Map.fromList . AssocMap.toList) $
            PV1.getValue mp
    setter _ v =
      PV1.Value $
        AssocMap.unsafeFromList $
          sortBy (comparing fst) $
            Map.toList $
              AssocMap.unsafeFromList . sortBy (comparing fst) . Map.toList <$> v
...
lowerBoundClosureL = L.lens getter setter
  where
    ...
    setter (PV1.LowerBound e _) = PV1.LowerBound e
...
upperBoundClosureL = L.lens getter setter
  where
    ...
    setter (PV1.UpperBound e _) = PV1.UpperBound e
