...
module PlutusLedgerApi.V1.DataValue (
    -- ** Currency symbols
      CurrencySymbol(..)
    , currencySymbol
    , adaSymbol
...
AsData.asData
    [d|
        data AssetClass = AssetClass { unAssetClass :: Pair CurrencySymbol TokenName }
            deriving stock (Generic, Data)
            deriving newtype (Haskell.Eq, Haskell.Show, Eq, PlutusTx.ToData, PlutusTx.FromData, PlutusTx.UnsafeFromData)
            deriving anyclass (NFData)
            deriving Pretty via (PrettyShow BuiltinData)
    |]

{-# INLINABLE assetClass #-}
-- | The curried version of 'AssetClass' constructor
assetClass :: CurrencySymbol -> TokenName -> AssetClass
assetClass s t = AssetClass (Pair s t)
...
assetClassValue (AssetClass (Pair c t)) i = singleton c t i

{-# INLINABLE assetClassValueOf #-}
-- | Get the quantity of the given 'AssetClass' class in the 'Value'.
assetClassValueOf :: Value -> AssetClass -> Integer
assetClassValueOf v (AssetClass (Pair c t)) = valueOf v c t
