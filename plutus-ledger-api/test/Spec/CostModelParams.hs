...
tests =
  testGroup
    ...
    [ testCase "length" $ do
            ...
            166 @=? length v1_costModelParamsForTesting
            175 @=? length v2_ParamNames
            175 @=? length v2_costModelParamsForTesting
            223 @=? length v3_ParamNames
            223 @=? length v3_costModelParamsForTesting
    ...
    , testCase "default values costmodelparamsfortesting" $ do
            defaultCostModelParams @=? Just (toCostModelParams v3_costModelParamsForTesting)
    , testCase "context length" $ do
            let costValuesForTesting = fmap snd v3_costModelParamsForTesting
    ...
    , testCase "cost model parameters" $ do
         ...
         assertBool "v1 params is not proper subset of v2 params" $ v1_ParamNames `paramProperSubset` v2_ParamNames
         -- v2 is missing some cost model parameters because new builtins and term constructors are added in v3
         assertBool "v2 params is not proper subset of v3 params" $ v2_ParamNames `paramProperSubset` v3_ParamNames
    ]
  where
