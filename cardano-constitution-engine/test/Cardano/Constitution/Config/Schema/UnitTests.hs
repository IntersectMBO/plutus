...
module Cardano.Constitution.Config.Schema.UnitTests
    ( tests
    ) where
...
defaultConstitutionJSONSchema =
    $$(Aeson.readJSONFromFile "data/config.schema.json")
...
tests :: TestTree
tests = testGroup "Config" $ fmap  (uncurry testProperty) [
