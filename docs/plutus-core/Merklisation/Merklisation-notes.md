...
Our ASTs contain a lot of type information: the earlier table shows that typically 70% or more of the nodes in an AST are accounted for by types.  The table in https://github.com/input-output-hk/plutus/blob/master/docs/plutus-core/Erasure-Merklisation.md shows that a great deal of space can be saved if we use an untyped representation.  
