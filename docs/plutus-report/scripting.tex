...
We adopt an approach familiar from generic programming in dependently-typed languages, where we parameterize our language over a \emph{universe} of types.\footnote{
  ...
  We assume that we are working in some ``metalangauge'' which has types such as \code{Integer} that we can reuse in Plutus Core.
}
...
Thus, an evaluator for \code{const :: forall a b . a -> b -> a} takes two \emph{terms} and returns the first one.
