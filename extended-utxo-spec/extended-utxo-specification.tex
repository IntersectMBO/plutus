...
\begin{document}
\maketitle
\todokwxm{We don't say anything about serialisation, and this is important in $\ptx$,
  where we have hashes of serialised versions of scripts, not in-memory versions.}
\todokwxm{Michael suggests (and I agree) that we should try to make
  ...
  content of the output datascripts, ...).  Then we can say separately
  ...
  abour validating data scripts messes this up.}
...
During the validation of a transaction $T$, it may be necessary for
the validator $V$ of one of $T$'s inputs $I$ to examine a data script
$D$ belonging to one of $T$'s outputs; for example, $D$ may be being
used to propagate some state through an ongoing contract and $V$ may
need to check that the contents of $D$ are ``correct'' in some sense.

This presents a problem.  During validation, the only information
about the pending contract available to $V$ is that provided
in the $\ptx$ structure, and this only contains the hash of $D$,
not $D$ itself.
\bigskip
\todokwxm{What's wrong with embedding $D$ in $\ptx$? Types, that's
  what.  Why can't we deserialise?  Because that's expensive and would
  require something like dynamic types.  Why can't we embed the scripts
in the redeemer?  Apparently we could cheat.}
