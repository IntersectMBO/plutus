...
\item Each unspent output now has a hash $\dataHsh$ of a
  value of type \Data{} associated with it: we call this the output's
  \textit{data value hash} (see Note~\ref{note:data-values}). This must
  be provided in full by the spending input as $\dataVal$, much like the validator.
...
Figure~\ref{fig:eutxo-1-types-cardano}.
...
The reasons for this are explained in Note~\ref{note:scripts}.
...
See Note~\ref{note:data-witnesses} for more discussion.
...
and the hash of the data value.\footnote{See
  Note~\ref{note:data-values-in-ptx} for further explanation.}
...
Figure~\ref{fig:validation-functions-1} defines some auxiliary functions used in validation.
\begin{ruledfigure}{H}
  \begin{displaymath}
  \begin{array}{lll}
...
Figure~\ref{fig:eutxo-2-types-cardano}.
\begin{ruledfigure}{H}
  \begin{displaymath}
    \begin{array}{rll}
      \currency  &=& \H\\
...
The same strategy applies to data values.

\note{Data witnesses.}
\label{note:data-witnesses}
Although data values are only recorded as hashes in transaction outputs, it
is useful to be able to record the full data value on the transaction
that \emph{creates} an output.

This allows observers to determine the full data value, without it having to be
kept in the UTXO set.
...
Hence there is a $\dataWits$ field on transactions, which \emph{may}
...
These may be found in $\dataWits$.

Having access to the data values allows a validator to
inspect an outgoing data value, for example to confirm that its
contents are correct in some sense. This can be useful when a data
value is used to propagate information about the state of a contract to
