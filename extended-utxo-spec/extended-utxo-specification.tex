...
\begin{ruledfigure}{H}
  \center{
    \begin{tabular}{rl}
      ...
      $\s{ScriptHash} = \N$&: a collision-resistant hash of a script\\
...
some object, and the blockchain will provide an efficient way to
retrieve the original object given its hash.  In this case, the
\textsf{ScriptHash} of a script will coincide with its \textsf{Address}.
...
\begin{ruledfigure}{H}
  ...
  \begin{arraydefs}{rll}
    ...
    && \i{validatorHash}: \s{ScriptHash},\\
    &&  \i{datascriptHash}: \s{ScriptHash})\\
    \\
    \s{InputInfo } & = (&\i{inputRef}: \s{OutputRef},\\
                 && \i{validatorHash}: \s{ScriptHash},\\
                 && \i{redeemerHash}: \s{ScriptHash}),\\
                 && \i{value}: \qty)\\
     \\
...
successfully, but only contains hashes of the relevant validator and
data scripts, not the scripts themselves.
