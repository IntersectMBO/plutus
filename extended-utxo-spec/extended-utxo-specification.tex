...
arguments (i) the data script for $I$, (ii) the redeemer for $I$, and (iii)
...
\item  $N$ applies $R_0$ to the actual
  data scripts and their hashes to obtain a full redeemer of type $\rho$:
  \[
    R = R_0\, (\sealer\, D_1\, D_1^{\#}) \cdots (\sealer\, D_k\, D_k^{\#})
    \]
  \item Validation is performed as usual to obtain a boolean value
    $\llbracket V (D_I, R, P) \rrbracket$, where $D_I$ is the data script
    for $I$ and $P$ is the $\ptx$ object for the current transaction
  \item During validation, $V$ can access the $D_i$ and check the hash
    ...
    $P$ to make sure that $R_0$ has not replaced the expected $D_i$
    with some other $D_j$ which happens to have the same type.
