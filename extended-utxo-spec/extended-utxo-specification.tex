...
%% Alonzo
\newcommand{\ExUnits}{\ensuremath{\s{ExUnits}}}
\newcommand{\CostMod}{\ensuremath{\s{CostMod}}}
\newcommand{\PlutusVI}{\ensuremath{\s{PlutusV1}}}
...
We then describe the full Cardano EUTXO model for Goguen Smart Contracts
(\cref{sec:goguen-ledger}). Further extensions were proposed to integrate
Plutus Core as the scripting language.
...
\label{sec:eutxo-2}
We now extend the EUTXO-1 model further by introducing features that
...
identifier and a \textit{minting policy script} which may be used to
...
Custom currencies are represented by unique \textit{currency
  identifiers} and each currency has a number of \textit{tokens} that
...
currencies.  We also change the type of the \mint{} field on
...
minting a negative amount of that currency, as in EUTXO-1.  In
addition, transactions now have a set $\mintScripts$ of minting policy
...
%%\newpage
\section{Goguen: Cardano Ledger for Smart Contracts}
\label{sec:goguen-ledger}
For Cardano, further extensions were proposed to integrate Plutus Core as the
scripting language. This section captures those changes and list formal definitions
of all concepts that DApp developers need for their work.
...
\item The term \i{language}. Running scripts in all languages should be supported
  indefinitely.

\item New protocol parameters:
  \begin{itemize}
    \item \i{maxTxExUnits}.
    \item \i{maxBlockExUnits}.
    \item \i{maxValSize}.
    \item \i{coinsPerUTxOWord}.
    \item \i{collateralPercent}.
    \item \i{maxCollInputs}.
  \end{itemize}

\item More on cost models:
  \begin{itemize}
    \item \i{ExUnits}.
    \item \i{CostModel}.
    \item \i{Prices}.
    \item Collateral inputs.
    \item How everything comes together.
  \end{itemize}

\item Transaction structure:
  \begin{itemize}
    \item Mention why is \i{txOutDatumHash} a \verb|Maybe|.
    \item Introduce \i{ScriptPurpose}.
    \item Introduce \i{ScriptTag}.
    \item Introduce \i{RedeemerPtr}.
    \item \i{redeemerHash} not in \s{InputInfo}.
    \item \i{thisInput}, \i{mintScripts} not in \s{Context}.
    \item \i{dcerts}, \i{wdrl}, \i{signatures}, \i{infoId} in \s{Context}.
    \item \i{validityInterval} is not in \s{Slot} anymore, but is \s{POSIXTimeRange}.
    \item Define \i{TxInfo} and \i{Tx}. How much should a DApp developer care for?
  \end{itemize}

\item More on minting policies.

\item More on script validation.
...
\note{Minting policies for custom currencies.}
\label{note:minting-policies}
The new \textbf{Minting} rule in \cref{fig:eutxo-2-validity} enables
custom currencies to implement their own minting policies: for
example, one might wish to place some limit on the amount of a
currency that can be minted, or restrict the creation of the currency
to owners of particular public keys.

The idea is that a custom currency has a minting policy which is
defined by some script $H$, and the address $h = \scriptAddr(H)$ is
used as the identifier of the currency.

Whenever a new quantity of the currency is minted,
Rules~\ref{rule:custom-mint} and~\ref{rule:all-mpss-run} of
Figure~\ref{fig:eutxo-2-validity} imply that $H$ must be contained in
the $\mintScripts$ field of the transaction and that it must be
successfully executed; $H$ is provided with the details of the
transaction via the $\Data$ object produced by $\toTxData$, so it has
access to the \mint{} field of the transaction and knows how much of
the currency is to be mintd and can respond
appropriately.\footnote{We do not insist that every minting policy
  script in a transaction is associated with a currency which the
  transaction is actually minting, so a transaction may include
  apparently unnecessary minting policy scripts.  The creator of the
  transaction is responsible for providing the contents of the
  $\mintScripts$ field and is free to include or exclude such scripts
  as they see fit.}
