...
\vfill
\blue{(I'll improve the page breaks when the document is closer to completion.)}
...
\item A list $\lambda$ of type $\List{T}$ is
  ...
  \item For two types $S$ and $T$, $\Map{S}{T}$ denotes the type of
    partial functions from \textit{keys} in $S$ to \textit{values} in
    $T$.  If $m$ is such a map then $\dom m$ (the \textit{domain} of
    $m$) is the set of keys of $m$.  Given a map $m$ and a key $k \in
    \dom m$, we denote the result of looking up $k$ in $m$ by $m[k]$.
    Equality for maps is defined in the obvious way: $m_1 = m_2$ if and only if
    $\dom m_1 = \dom m_2$ and $m_1[k] = m_2[k]$ for all $k \in \dom m_1$.
    
  \item If the type $T$ is a monoid with binary operation $+$ (for
    example, a numeric type) and $f, g \in \Map{S}{T}$ then we define
    their sum $f+g \in \Map{S}{T}$ with domain $\dom (f+g) = \dom f
    \cup \dom g$ by
    \[
    (f+g)[x] =
    \left\{ \begin{array}{ll}
      ...
      $\Map{S}{T}$ itself becomes a monoid under the operation $+$
...
The function \verb|spends :: PendingTx -> TxOut -> Bool| checks if the
...
\item \s{Token} is an identifier for an individual token, probably a hash.
