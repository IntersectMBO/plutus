...
\todokwxm{Michael suggests (and I agree) that we should try to make
  ...
  Plutus Core, which would surely be a good thing.}  \bigskip
...
\label{note:validation-determinism} The $\ptx$ type is the
only information about the ``outside world'' available to a validator
script at the time of validation.  Allowing the validator access to
this information gives the EUTXO model a considerable amount of power.
\red{(like what?)}  However, it is important not to make too much
