...
\begin{itemize}
  \item \textbf{XUTXO-1} (Section~\ref{sec:xutxo-1}): this extends the basic UTXO model with
    enhanced scripting features, allowing the implementation of
    complex smart contracts..
  \item \textbf{XUTXO-2} (Section~\ref{sec:xutxo-2}): this adds multicurrency features to XUTXO-1,
    allowing users to define \textit{custom currencies}.
  \item \textbf{XUTXO-3} (Section~\ref{sec:xutxo-3}): this adds direct 
    support for non-fungible tokens to XUTXO-2.
\end{itemize}

We give formal specifications of each of these versions.  The
rationale for providing three separate specifications is that (1)
...
full power of XUTXO-3 and so could use a simpler model, perhaps
...
\item A list $\lambda$ of type $\List{T}$ is
  ...
  \item For two types $S$ and $T$, $\Map{S}{T}$ denotes the type
    of partial functions from \textit{keys} in $S$ to \textit{values}
    in $T$.  If $m$ is such a map then $\dom m$ (the
    \textit{domain} of $m$) is the set of keys of $m$.  Given a map $m$
    and a key $k \in \dom m$, we denote the result of looking
    up $k$ in $m$ by $m[k]$.
    
  \item If $T$ is a numeric type and $f, g \in \Map{S}{T}$ then we define
    their sum $f+g \in \Map{S}{T}$ with domain $\dom (f+g) = \dom f \cup \dom g$ by
    \[
    (f+g)[x] =
    \left\{ \begin{array}{ll}
        f[x] + g[x] & \mbox{if $x \in \dom f \cap \dom g$}\\
        f[x] & \mbox{if $x \in \dom f \setminus \dom g$}\\
        g[x] & \mbox{if $x \in \dom g \setminus \dom f$}
      \end{array}
      \right.
      \]
      We generalise this to the sum of a finite number of maps using the $\sum$
      notation in the usual way.
...
\red{So where are they actually specified?}
...
\newpage
\section{XUTXO-2: muticurrency support}
\label{sec:xutxo-2}
...
support this we introduce three new types: see
...
\begin{ruledfigure}{H}
  ...
  \s{ValueMap} &: &a map from \s{CurrencyID}s to quantities of the
  corresponding currencies\\
  & & (represented using the non-negative \s{Value} type). This allows an output\\
  & & to contain quantities of multiple custom currencies in addition to some \\
  & & quantity (possibly zero) of the native currency.\\

  \s{ValueMap$^{\pm}$} &: &This is like \s{ValueMap}, but quantities may be either positive or negative.
  \end{tabular}
  }
...
We add a \s{ValueMap} to the \s{Output} type, representing values of custom
currencies; we also add a \customforge{} field of type
$\s{ValueMap}^{\pm}$ to transactions to allow the creation and
...
but we also require a similar function to retrieve the values of
...
we require that the quantities of the individual custom currencies
...
is forged, rules~\ref{rule:custom-forge}, \ref{rule:all-inputs-validate-2}, and
\ref{rule:validator-scripts-hash-2} imply that $H$ must be executed; $H$ is
...
of blockchain applications; for example, they can represent ownership of
some object in a game.
