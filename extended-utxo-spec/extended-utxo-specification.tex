...
\usepackage{hyperref}  % This has to go at the end of the pacakges.
...
\todokwxm{Michael suggests (and I agree) that we should try to make
  ...
  abour validating data scripts \textit{really} messes this up.}
...
This document contains a specification of some extesnsions of the
...
the hash of a Plutus Core script we mean the hash of the serialsed
version.
...
\todokwxm{We should probably point out somewhere that the validating node
  typehecks scripts during validation.}
...
some of $T$'s output data scripts, $D_1,\ldots,D_k$ are ``correct'' in
...
\begin{itemize}
  ...
  must be able to inspect the contents of the data script of each of
  $T$'s outputs.
 \item Furthermore, the contents of the data scripts must be obtained
   in such a way that $V$ can be certain that they are genuine (if not,
   it might be possible for some participant in the transaction to
   lie about the contents and thus subvert the transaction).
\end{itemize}

\noindent Exactly how this is done will depend on the properties of the
scripting language.

\paragraph{Pending data script validation in Plutus.} The above
requirement presents a problem for Plutus Core because Plutus Core is
...
the $D_1,\ldots,D_k$ together with their hashes; during validation,
$R$ could forward the contents of the $D_i$ to $V$ for examination.
...
who could lie about what is actually contained in $D$.
...
redeemer $R$ which takes the (contents of the) $D_i$ as arguments and
forwards them to $V$, and for $R$ to be supplied with the true
contents of the $D_i$ during validation; this is done by the
...
We assume that the validator $V$ for an input $I$ needs to inspect the
contents of data scripts $D_1, \ldots, D_k$ of the outputs
$O_1, \ldots O_k$ of $T$ , and that the $D_i$ have (Plutus Core) types
$\tau_1, \ldots \tau_k$ respectively.  Recall also that $V$ takes as
arguments (i) the data script for $I$, (ii) the redeemer for $I$, and
the $\ptx$ structure of $T$.  Assume that these have the Plutus Core
types $\delta, \rho$ and $\pi$ respectively, so that the Plutus Core
type of $V$ is
$\delta \rightarrow \rho \rightarrow \pi \rightarrow \textsf{Bool}$.
The creator of the validator $V$ will know in advance that $V$
requires access to objects of type $\tau_1, \ldots \tau_k$, so
the type $\rho$ will describe an object which provides a means
to access things of these types (for example, $\rho$ may be a tuple
type involving the $\tau_i$, in addition to types of any other
values which $V$ exepcts the redeemer to provide).
...
\item A ``pre-redeemer'' $R_0$ for $O$ is constructed (off-chain) with
  the type
  $\sealed\, \tau_1 \rightarrow \sealed\, \tau_2 \rightarrow \cdots
  \rightarrow \sealed\, \tau_k \rightarrow \rho$
\item On-chain, the validating node $N$ typechecks $R_0$ and
  makes sure that it contains nothing of type \verb|Sealed ...|;
  if it does, then valuation fails.
\item  $N$ applies $R_0$ to the actual
  data scripts and their hashes to obtain a real redeemer
  \[
    R = R_0\, (\sealer\, D_1\, D_1^{\#}) \cdots (\sealer\, D_k\, D_k^{\#})
    \]
  \item Validation is performed as usual to obtain a boolean value
    $\llbracket V (D, R, P) \rrbracket$, where $D$ is the data script
    for $P$ is the $\ptx$ object for the current transaction
  \item During validation, $V$ can access the $D_i$ and check the hash
    from the corresponsing \verb|Sealed| object against the hash from
    $P$ to make sure that $R_0$ has not permuted its arguments or
    duplicated any of them,.
  \end{itemize}
  \todokwxm{We don't really need the hashes now: we could just use the
    index of the output.}  \todokwxm{Damn. Now we need to apply
