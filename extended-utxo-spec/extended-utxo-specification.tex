...
\item Each unspent output now has a hash $\dataHsh$ of a data
...
\paragraph{Scripts.} Note that data values and validator scripts as
components of inputs, even though they are conceptually part of an output.
...
The same strategy applies to data values. However, it is useful to
be able to record the full data value on the transaction that \emph{creates}
an output. Hence the $\dataWits$ field \emph{may} contain mappings from
the $\DataHash$es used in the transaction to their \Data{} values. This
information is also present in \ptx{}.
