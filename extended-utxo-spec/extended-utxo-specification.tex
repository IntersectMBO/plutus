...
\label{sec:intro}
The Cardano blockchain~\cite{Cardano, Cardano-ledger-spec} uses a
...
%  is denoted $\lambda_1 ::: \lambda_2$.
  ...
  \item If the type $V$ is a monoid with binary operation $+$ (for
    ...
    \left\{ \begin{array}{ll}
      ...
      the type $\Map{K}{V}$ itself becomes a monoid under the
...
\footnote{In the Cardano setting, scripts are Plutus Core
  programs~\cite{Plutus-Core-spec} (equivalently, expressions).
}
...
to do so.
\footnote{The validator plays a role similar to that of BitCoin's
...
\cite{Zahnentferner18-UTxO}, with the addition of the new features
...
increase efficiency and decrease storage requirements.  These have
been ommitted from this model because it subsumes all of the other
transaction types we're likely to encounter, and also because it's
difficult to give a definitive list of such special cases.
...
\footnote{We mentioned earlier that the functor $\Map{K}{-}$ preserves
  the property of being a monoid; however, in general it does
  not preserve the property of being a group, abelian or
  otherwise.  Let $K=\{X,Y\}$ and consider the map
  $m = \{(X,1)\} \in \Map{K}{\Z}$: for every map $n$, we have
  $X \in \dom (m+n)$, so we cannot have $m+n = \emptymap$, the
  identity element of $\Map{K}{\Z}$. However, if we consider equality
  modulo elements which map to 0 (ie, $m \equiv n$ if
  $\mu(m) = \mu(n)$), then we \textit{do} get a group.}
