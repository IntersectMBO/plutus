...
primitives =
  testNested "Primitives" . pure $
    testNestedGhc
      ...
      , goldenUEval
          "andApply"
          [toUPlc andPlc, toUPlc $ plc (Proxy @"T") True, toUPlc $ plc (Proxy @"F") False]
...
andPlc =
  plc (Proxy @"andPlc") (\(x :: Bool) (y :: Bool) -> if x then (if y then True else False) else False)
...
writeBitsIntegerToByteString =
  plc
    (Proxy @"writeBitsIntegerToByteString")
