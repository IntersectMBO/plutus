...
datat :: TestNested
datat =
  testNested "Data" . pure . testNestedGhc $
    [ monoData
    , polyData
    , newtypes
    , recursiveTypes
...
monoDataType = plinthc (\(x :: MyMonoData) -> case x of Mono2 i -> i; _ -> 1)
...
monoConstructor = plinthc Mono1

monoConstructed :: CompiledCode MyMonoData
monoConstructed = plinthc (Mono2 1)
...
defaultCase = plinthc (\(x :: MyMonoData) -> case x of Mono3 a -> a; _ -> 2)

irrefutableMatch :: CompiledCode (MyMonoData -> Integer)
irrefutableMatch = plinthc (\(x :: MyMonoData) -> case x of Mono2 a -> a)

atPattern :: CompiledCode ((Integer, Integer) -> Integer)
atPattern =
  plinthc
    (\t@(_ :: Integer, y :: Integer) -> let fst (a, _) = a in Builtins.addInteger y (fst t))
...
monoRecord = plinthc (\(x :: MyMonoRecord) -> case x of MyMonoRecord i _ -> i)
...
recordNewtype =
  plinthc
    (\(x :: RecordNewtype) -> case x of RecordNewtype (MyNewtype i) -> i)
...
recordWithStrictField = plinthc (\(x :: RecordWithStrictField) -> strictField2 x)
...
unusedWrapper = plinthc ((\x (y, z) -> x (z, y)) mkT (1, 2))

-- must be compiled with a lazy case
nonValueCase :: CompiledCode (MyEnum -> Integer)
nonValueCase =
  plinthc
    (\(x :: MyEnum) -> case x of Enum1 -> 1 :: Integer; Enum2 -> Builtins.error ())
...
stakingCredential = plinthc StakingPtr
...
strictDataMatch = plinthc (StrictTy 1 2)

type Synonym = Integer

synonym :: CompiledCode Integer
synonym = plinthc (1 :: Synonym)
...
polyDataType =
  plinthc
    (\(x :: MyPolyData Integer Integer) -> case x of Poly2 i -> i; _ -> 1)

polyConstructed :: CompiledCode (MyPolyData Integer Integer)
polyConstructed = plinthc (Poly1 (1 :: Integer) (2 :: Integer))

defaultCasePoly :: CompiledCode (MyPolyData Integer Integer -> Integer)
defaultCasePoly =
  plinthc
    (\(x :: MyPolyData Integer Integer) -> case x of Poly1 a _ -> a; _ -> 2)
...
basicNewtype = plinthc (\(x :: MyNewtype) -> x)

newtypeMatch :: CompiledCode (MyNewtype -> Integer)
newtypeMatch = plinthc (\(MyNewtype x) -> x)

newtypeCreate :: CompiledCode (Integer -> MyNewtype)
newtypeCreate = plinthc (\(x :: Integer) -> MyNewtype x)

newtypeId :: CompiledCode (MyNewtype -> MyNewtype)
newtypeId = plinthc (\(MyNewtype x) -> MyNewtype x)

newtypeCreate2 :: CompiledCode MyNewtype
newtypeCreate2 = plinthc (MyNewtype 1)

nestedNewtypeMatch :: CompiledCode (MyNewtype2 -> Integer)
nestedNewtypeMatch = plinthc (\(MyNewtype2 (MyNewtype x)) -> x)
...
paramNewtype =
  plinthc
    (\(x :: ParamNewtype Integer) -> case x of ParamNewtype (Just i) -> i; _ -> 1)
...
listConstruct2 = plinthc ([1] :: [Integer])
...
listConstruct3 = plinthc ((1 :: Integer) : (2 :: Integer) : (3 :: Integer) : [])

listMatch :: CompiledCode ([Integer] -> Integer)
listMatch = plinthc (\(l :: [Integer]) -> case l of (x : _) -> x; [] -> 0)
...
emptyRoseConstruct = plinthc (EmptyRose [EmptyRose [], EmptyRose []])
...
associated = plinthc (\(x :: AType Bool) -> x)
...
associatedParam = plinthc (paramId (Param 1) 1)

-- Here we cannot reduce the type family
{- FOURMOLU_DISABLE -}
-- Because of fourmolu/ourmolu bug (https://github.com/fourmolu/fourmolu/issues/475)
-- we need to wrap OPTIONS_GHC pragma with FOURMOLU disable and enable. Without this,
-- every comment preceding OPTIONS_GHC pragma will be pushed to the very top of the file.
{-# OPTIONS_GHC -Wno-incomplete-patterns #-}
{- FOURMOLU_ENABLE -}
