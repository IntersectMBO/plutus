...
defaultCaseDuplication =
  plc
    (Proxy @"defaultCaseDuplication")
    ( \(x :: A) -> case x of
        B -> 1
        _ -> 2
    )
...
defaultCaseDuplicationNested =
  plc
    (Proxy @"defaultCaseDuplicationNested")
    ( \(x :: A) (y :: A) -> case x of
        B -> 1
        _ -> case y of
          B -> 2
