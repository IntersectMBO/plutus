...
runCompiler moduleName opts expr = do
    ...
    let pirTcConfig = if _posDoTypecheck opts
        ...
        pirCtx = PIR.toDefaultCompilationCtx plcTcConfig
                 ...
                 & set (PIR.ccOpts . PIR.coDoSimplifierEvaluateBuiltins)   (opts ^. posDoSimplifierEvaluateBuiltins)
                 ...
                 -- 1. The only other choice you can make is new version + Scott encoding, and there's really no reason to pick that
                 -- 2. This is consistent with what we do in Lift
                 & set (PIR.ccOpts . PIR.coDatatypes . PIR.dcoStyle) (if plcVersion < PLC.plcVersion110 then PIR.ScottEncoding else PIR.SumsOfProducts)
