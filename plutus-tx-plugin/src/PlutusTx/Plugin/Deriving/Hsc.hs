...
throwError srcSpan msgDoc = do
  ...
  let diagOpts :: Ghc.DiagOpts
      diagOpts = Ghc.initDiagOpts dynFlags

      -- 1. Create the plain diagnostic
      innerDiag :: Ghc.DiagnosticMessage
      ...
      diagnostic :: Ghc.GhcMessage
      diagnostic = wrapDiagnostic innerDiag

      -- 3. Create the envelope
      msg :: Ghc.MsgEnvelope Ghc.GhcMessage
...
wrapDiagnostic :: Ghc.DiagnosticMessage -> Ghc.GhcMessage
