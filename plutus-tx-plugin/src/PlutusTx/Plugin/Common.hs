...
injectAnchors _ _ env = do
  ...
  let binds = GHC.tcg_binds env
      bindsAnchored = Compat.modifyBinds (transformBi tickExpr) binds
...
tickExpr :: GHC.LHsExpr GHC.GhcTc -> GHC.LHsExpr GHC.GhcTc
tickExpr le@(GHC.L ann expr)
  | isTickWorthy expr
  , Just sp <- GHC.srcSpanToRealSrcSpan (GHC.locA ann) =
      GHC.L ann (mkHsTick (GHC.SourceNote sp (GHC.LexicalFastString $ GHC.mkFastString "")) le)
  | otherwise =
      le

mkHsTick :: GHC.CoreTickish -> GHC.LHsExpr GHC.GhcTc -> GHC.HsExpr GHC.GhcTc
mkHsTick tick le = GHC.XExpr $ GHC.HsTick tick le

isTickWorthy :: GHC.HsExpr GHC.GhcTc -> Bool
isTickWorthy expr
  | isHsTick expr = False
...
isHsTick :: GHC.HsExpr GHC.GhcTc -> Bool
isHsTick (GHC.XExpr (GHC.HsTick _ _)) = True
isHsTick _ = False
