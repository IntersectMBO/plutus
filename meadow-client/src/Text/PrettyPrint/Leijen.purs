...
punctuate p arr =
  let
    lastIdx = Array.length arr - 1
  in
    Array.mapWithIndex (\idx d -> if lastIdx == idx then d else d <> p) arr
...
align d =
  column
    ( \k -> nesting (\i -> nest (k - i) d)
    ) --nesting might be negative :-)
...
instance showSimpleDoc :: Show SimpleDoc where
 ...
 = best 0 0 (Cons 0 headNode Nil)
  ...
  nicest' n k i ds x y =
    let
      ...
      width' = min (w - k) (r - k + n)
    in
      if fits w (min n k) width' x' then x' else let y' = best n k (Cons i y ds) in y'
...
renderCompact = scan 0 <<< List.singleton
  where
...
indentation n = spaces n
 --indentation n   | n >= 8    = '\t' : indentation (n-8) --                | otherwise = spaces n --  LocalWords:  PPrint combinators Wadler Wadler's encloseSep