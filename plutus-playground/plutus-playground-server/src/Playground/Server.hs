...
import           Playground.API               (FunctionsSchema(FunctionsSchema), API, Evaluation, SourceCode, contract, functions)
...
runFunction e = do
  ...
  r <- liftIO $ runInterpreter $ Interpreter.runFunction sourceCode $ functions e
  liftIO . print $ r
  case r of
    Left e  -> throwError $ err400 {errBody = BSL.pack . show $ e}
    Right _ -> pure []
