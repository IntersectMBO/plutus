...
import           Playground.Interpreter (CompilationError (CompilationError), column, filename, parseErrorText, row,
                                         text)
...
parseErrorTextSpec =
  describe "parseErrorText" $ do
    it "shouldn't blow up on an empty string." $
      parseErrorText "" `shouldBe` Nothing
