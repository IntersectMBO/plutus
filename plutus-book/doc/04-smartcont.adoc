[#04-smartcont]
= Smart Contracts
// Throughout: I think we should prefer talking about the "ledger" to the "blockchain".
// Almost everything we say is generic over how the ledger is implemented, in many ways
// the blockchain is an implementation detail. Hooray for abstraction!
...
reliance on the execution of a program, or smart contract, to control the transfer of funds
between a contract's participants,
...
via a distributed cryptographic ledger. This ensures the entire history
...
// This covers some very similar material to the Welcome chapter.
...
// Generally in this section I feel unclear about whether you're talking about
// Plutus or about some higher-level notion of a "smart contract" which we will
// implement with Plutus.
...
participants can only ever contribute assets to
...
// If we're talking about Plutus then you certainly can lock things indefinitely!
// Marlowe guarantees that you can't, but that takes work. It seems odd to make this
// part of your definition of a smart contract, given that there is really nothing
// that guarantees that things people write with Plutus will have this property.
...
Using this idea of a trusted program being in charge of assets involved
...
The overall scheme for any basic crowdfunder is usually the same.
...
minimum amount set by the entity offering the crowdfunded product.
...
the crowdfunder. The purpose of this type of contract is that the entity
...
as a result of being triggered by events of two types:
...
This party is expected to be privy to the true fate
...
// Do we have an escrow example yet? We should link to it when we do.
This scheme
...
the item, the seller of the item gets the escrowed money. If the
...
releasing the escrowed funds to the seller of the item.
...
// The two options both say to refund everyone! Maybe one is supposed to be "refund
...
Non-fungible tokens are tokens which are not interchangeable
with tokens of other kinds (in particular, the most common token kind is Ada itself).
This makes them suitable for representing distinct assets. One of the many possible examples
...
is associated with an address which identifies the script that controls the
supply of that token. We will look at how to handle custom tokens in chapter
<<token#token>>.
...
// What is the problem? I take it that the
// problem is assuring that the money is guaranteed to be repaid.
// This paragraph is generally a bit unclear.
A loan is a contract where a party promises to pay (in the
...
party for providing the loan money ahead of time. This is not a
very good solution, partially defeating the purpose of the loan -
...
// I don't understand this paragraph. I think you're suggesting issuing
// a debt token, but how does that provide any more guarantee that the loan
// will be paid back?
There is another approach to building smart contract loans and variations thereof.
One may introduce a non-fungible token which the entity acquiring the loan
...
https://www.actusfrf.org/[ACTUS] is an ongoing project which started with the aim to define a taxonomy
of all (or nearly all) financial contracts into a small
...
// The implementation sentence doesn't add much to my eye, since it's just
// using more terms that the reader won't know.
ACTUS Data Standard::
The Data Standard defines a universal set of legal terms
...
conceptually and as a major real-world practical advancement. From the ACTUS
website,
...
too, can be modeled on a ledger with smart contract and non-fungible token support.
