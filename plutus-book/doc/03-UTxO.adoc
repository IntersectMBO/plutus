...
formal specification, see <<deleg>>.
...
model we discuss here, consider the following figure:
...
address, whose type is by `TxOut = Addr * Coin`.
Note that users' addresses are always public keys, and the funds at them
belong to the entity who is able to prove that they possess they corresponding
...
used in the Cardano ledger implementation, but you should now should have a
