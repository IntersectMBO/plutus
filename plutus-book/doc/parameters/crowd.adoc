...
import Language.PlutusTx.Prelude hiding (plus) -- <2>
import Ledger
import Ledger.Ada
...
} deriving (Show, Generic, ToJSON, FromJSON, ToSchema) -- <3>
makeLift ''Campaign
----

<1> We need this to be able to derive `Generic` and `ToSchema` instances.

<2> The `plus` function in the `Prelude` works on `Integer`s, but we want to use
the one from the `Ada` module that works on `Ada` values.

<3> We need `ToJSON`, `FromJSON` and `ToSchema` instances
...
There are two scenarios where people will want to claim funds from the
script: The campaign owner _collecting_ after a successful campaign
...
To decide about a `Refund` claim, however, we have to know that the deadline
has passed and whether the
contribution being reclaimed was indeed originally made by the person
claiming the refund. So we need information about where a contribution came
...
// Why? If we check that the transaction is signed by them, which we do, shouldn't
// they be able to do what they want with the funds? That's what the crowdfunder in
// plutus-use-cases does.
...
it is not enough to simply compile the function as we did in the last chapter.
