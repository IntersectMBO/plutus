...
contribute campaign ada = do
  ...
  where
    ...
    handler tx = EventHandler $ const $ do
        logMsg $ pack $ "claiming refund from " ++ show campaign
        collectFromScriptTxn                                     -- <4>
            range
            (mkValidatorScript campaign)
            (mkRedeemerScript Refund)                            -- <5>
            (hashTx tx)
...
<4> Function `collectFromScriptTxn` is similar to `collectFromScript`,
but it additionally takes a transaction identifier to only collect from script
outputs created by the transaction with that identifier. We can compute the
identifier of a transaction with `hashTx`.

<5> We demand a _refund_.
