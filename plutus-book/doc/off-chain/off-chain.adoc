...
// As far as I can tell, we don't ever really define what we mean by "on-chain"
// and "off-chain", and we've used the terms quite a bit by this point.
Writing applications with Plutus requires writing on-chain code and off-chain code,
...
- also submits transactions to the blockchain, which may contain scripts that will be run by the nodes

* Inside a Plutus smart contract definition, we have _endpoints_
- endpoints are functions executed by the wallet to operate the smart contract
(often parametrized by user-input data)
- endpoints are _off-chain_ functions
- endpoints are used to build transactions and submit them to be processed
// this is repeating the second bullet point above a bit
- endpoints call functions which generate Plutus
Tx code (specifically the validator, redeemer, and data scripts)
to be included in transactions that need them and executed by _on-chain_ by nodes

// Some of this is a bit odd to say. It may contain imports? Yes, it's a Haskell program.
...
// The next two bullet points do not seem like the rest
// I also disagree with the conflation of off-chain and endpoint code. Off-chain just
// means off-chain: people can write other applications using off-chain code that don't
// conform to the contract-bundle/endpoint structure.
// The stuff about what you need to run off-chain code is also a bit confused, but that's
// our fault - at the moment the contract bundle performs the wallet functionality too, but
// that's not how it will look in the end.
...
by the endpoints
- executed by nodes to authorize spending of script outputs

* A Plutus contract should, at the very least, contain endpoints which generate transactions
that allow the user to
- _pay into_ a contract (this involves using the validator and data scripts to create
