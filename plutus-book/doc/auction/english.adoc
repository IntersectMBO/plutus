...
and it will just log a message and do not do anythiny if there is not exactly one such output.
...
We use `mkStepRedeemer` from module `Language.PlutusTx.StateMachine` to build
redeemer scripts.
...
mkEARedeemer a = RedeemerScript $
    $$(compileScript [||
        (mkStepRedeemer :: EAAction -> StateMachineRedeemerFunction EAState EAAction) ||])
    `applyScript`
    lifted a
----
