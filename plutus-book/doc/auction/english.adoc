...
<2> <<endmatter#token-name,Token name>> of the token. As discussed above,
    we assume that the token is defined by `eaSymbol` and that `eaName` is _unique_.
...
and it will simply log a message and do nothing if there is not an exact output.
...
Everything depends on the input
...
<3> If we do not find the output containing the token,
    we return a result of "exceptional".
...
we made ourselves, some other bidder could have made the same bid and managed
