...
instance IsData EAAction where
    toData (EABid key v) = Constr 0 [toData key, toData v]
    toData (EAClaimBid) = Constr 1 []
    toData (EAClaimToken) = Constr 2 []
    toData (EAReclaimBid key v) = Constr 3 [toData key, toData v]
    toData (EAReclaimToken) = Constr 4 []
    {-# INLINABLE fromData #-}
    fromData (Constr i [k, v]) | i == 0 = EABid <$> fromData k PlutusTx.<*> fromData v
    fromData (Constr i []) | i == 1 = Just EAClaimBid
    fromData (Constr i []) | i == 2 = Just EAClaimToken
    fromData (Constr i [k, v]) | i == 3 = EAReclaimBid <$> fromData k PlutusTx.<*> fromData v
    fromData (Constr i []) | i == 4 = Just EAReclaimToken
    fromData _ = Nothing
...
mkEAValidator ea = ValidatorScript $
    $$(compileScript [|| \(ea' :: EnglishAuction) ->
        wrap (mkValidator (eaStateMachine ea')) ||])
    `applyScript`
    lifted ea
    where wrap = wrapValidator @EAState @EAAction
