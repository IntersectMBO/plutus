// There's quite a lot of commented out code in these modules. I'd get rid of it, personally.
////
[source,haskell]
----
{-# LANGUAGE DataKinds                       #-}
...
// Why not just auction a Value? The only thing you do with eaSymbol and
// eaName are immediately convert them to a Value, so why not simplify
// it by just having it be a Value?
[source,haskell]
----
...
<2> <<endmatter#token-name,Token name>> of the token. As discussed above,
    we assume that the token defined by `eaSymbol` and `eaName` is _unique_.
...
<5> Minimal bid increase, i.e. how much higher than the previous bid a new bid
    has to be.
...
// I can't remember if we already discussed this, but you could do this
// whereby someone making a new highest bid must also transfer the previous
// bid to the person who made it. That way the amount in the machine is always
// the highest bid, and there's less bookkeeping required.
// Possibly illustrating lots of bookkeeping is the point!
What state do we need to keep track of an auction? We will need to record
...
<4> Reclaiming a bid after the bidder has been outbid. The arguments describe
...
at the script address, and each transaction will have that output as input and
...
and it will just log a message and do not do anything if there is not exactly one such output.
...
We find the highest bid (if there is one) by just grabbing the head of the list
...
which is the minimal bid if there has not been a bid yet
and the currently higest bid increased by the minimal increment if there has.

If the second component of the state is `True`, the auction is already over,
so no new bid is allowed, and we throw an exception.
...
script output are correct. Function `valueCorrect` is a helper for that.
...
Everything just depending on the input
data script and the action is already contained in `eaTransit`
(via returning `Nothing`),
so in `eaCheck`, we have to check those conditions that depend on the pending
transaction:
...
// I don't understand this. I think we should do that. If the person reclaiming
// the bid wants to send the output to a different public key, that seems fine,
// but I would expect them to need to sign the transaction reclaiming the money.
...
<1> The reclaim must be authorized by the public key associated with the bid.
    Note that we do not check _where_ the reclaimed bid is sent to, just that the
...
We use `mkRedeemer` from module `Language.PlutusTx.StateMachine` to build
redeemer scripts.
...
The on-chain part is done with this, and we can turn our attention to the
wallet endpoints.
...
Using this, it is realtively straight forward to implement the remaining
...
// Again, there's a fair bit of inconsistency between which endpoints take all the parameters
// separately and which get an EnglishAuction. My preference would be to just always do the latter.
[source, haskell]
----
...
<2> ...and payed to the highest bidder.

<3> This can only been done after the token owner has had time to claim the
    highest bid.
...
<3> Such reclaims can in principle be done at any time. They will fail, however,
    ...
    there will no longer be a unique output then.
...
that will allow us to execute a custom action at each slot after auction start,
...
<10> If our custom action indicated we were done, we stop. Otherwise we
     wait for the next slot and repeat.
...
<4> In both case, we are done and return `True` to stop tracking.
...
This is very tricky, though, even with our `trackAuction` helper.
The reason is that even if the current highest bid is as high as the last bid
we ourselves made, some other bidder could have made the same bid and managed
to get her transaction validated before ours. Just tracking script values will
not be able to help us here.

Luckily, even though unlifting of scripts has not yet been implemented in
Plutus, we can define a very limited version of it directly, a version that
allows us to unlift `Bool`:

// You can use `check` from the PlutusTx Prelude
[source,haskell]
----
...
<1> Function `evaluteScript` from module `Ledger.Scripts`
    will evaluate a script and return a `Left` if there
    has been an error during evaluation.

<2> The `if`-statement will simply return `()` for the
    `True` case, but throw an error for the `False` case.

Note that because we deliberately throw an error
to identify `False`, we can not distinguish `False` from a genuine
error in the script. Our `unliftedBool` will therefore only work correctly
if it is applied to scripts that are actually of type `Bool` and do not contain
any errors.

Just being able to unlift Booleans seems extremely limited
(and it is), but it allows us to "ask" simple questions
...
even though the data script itself is opaque and can not be inspected
directly.
...
<3> This time we use non-trivial custom state of type
...
<7> If we waited till `eaFinish ea`, we won the auction and claim the token.

<8> If bidding is over and we won, we just have to wait.

<9> If bidding is over and we lost, we are done.
...
<13> If we are not willing to place a bid that high, we are done.
