...
// If you like you can try out the typed transaction interface which should handle
...
// - Make the data script be a set of issued tokens.
// - Make the validator a state machine, where issuing a token checks that it
// is not in the existing set (this is done in Plutus so no inspection problems),
// and will forge that token as well as have a continuing output with the updated
// state (including the new token).
// That seems a *lot* nicer than this approach! You don't even need the admin token:
// you just need to use the unique transaction output to start the machine and then
// the machine output functions as the admin token.
