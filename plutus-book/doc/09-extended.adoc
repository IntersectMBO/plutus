...
and the UTxO. In the extended model, the position in the outputs list
carried by a transaction is the reference index `ix` of that output.
that will be use `[TxOut]`. In the UTxO, the outputs are stored as a finite
