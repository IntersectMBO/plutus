...
A term of type `Script` is a Plutus Core expression.
...
part of the `PendingTx` data structure. During script validation, we want to check
...
* `hash s`, the hash of the validator script, which is also script's address
