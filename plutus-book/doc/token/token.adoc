...
// Slightly repeating yourself here
...
* A _fungible_ token is represented by a `CurrencySymbol` for which there is
  ...
  special case).
- Ada is the prime example of a fungible token, and has the _empty bytestring_
  as its `CurrencySymbol` and `TokenName`.
...
* Mixed tokens are those with several ``TokenName``s _and_ arbitrary quantities, which might
  be one or not.
...
then for _every currency symbol in the forged value_,
there must be a script input whose validator script hash equals that currency
symbol.

What this means is that the currency name picks out a script which will always be run
whenever values of that currency are forged. That script can permit or deny the
transaction as it wishes. The script thus represents a _monetary policy_
(a policy that controls the supply of a currency) for custom tokens.
