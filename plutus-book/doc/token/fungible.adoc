...
fungibleDataScript = DataScript $ toData ()
...
instance IsData TradeAction where
    toData Execute = Constr 0 []
    toData Reclaim = Constr 1 []
    {-# INLINABLE fromData #-}
    fromData (Constr i [])
        | i == 0 = Just Execute
        | i == 1 = Just Reclaim
...
mkTradeValidator =
      ValidatorScript
    . applyScript $$(compileScript [|| \t -> wrap (validateTrade t) ||])
    . lifted
    where wrap = wrapValidator @() @TradeAction

tradeDataScript :: DataScript
tradeDataScript = DataScript $ toData ()

mkTradeRedeemerScript :: TradeAction -> RedeemerScript
mkTradeRedeemerScript = RedeemerScript . toData
