...
We are now ready to write down the validation logic, i.e. our currency's
monetary policy:
...
is visible on the blockchain and can then issue the forging transaction, which
...
be able to detect when the first transaction has been accepted into the
blockchain.
...
It we are done! Now everybody can use `forge` to forge her own currency!

This raises another question, though: How can we trade this new currency? -
Sure, we can send and receive the new currency in the same way that we can send
and receive Ada. But what if we want to sell some tokens of the new
...
make sure that either seller and buyer both pay what they promised before the
...
We write the usual helper functions and are done with the on-chain part:
...
<3> Has the deadline passed and my funds are still locked?
...
<2> Has the deadline passed, and my payment is still locked?
