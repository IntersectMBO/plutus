...
// This isn't necessary, right? We can just look at the txForge field in
// the PendingTx.
...
// This validator seems strange to me. If we just looked at the txForge field
// I think it would correspond more closely to how we've described things so
// far, and we wouldn't have to restrict it to a single output.
[source,haskell]
----
...
// Why not use payToScript in forge?
// For the triggers, I think it would be simpler to do what the plutus-use-cases example does: send
// 1 Ada to the address, and wait for the funds at the address to be greater than zero.
[source,haskell]
----
...
// I wonder if this should be in a separate chapter? Atomic swaps are a pretty interesting topic
// in themselves!
...
// As you've written it, you can drop the redeemer script too - the action
// is implied by whether the deadline has passed or not. If you wanted to
// allow people to reclaim before the deadline (i.e. back out) then you
// would need the action.
...
// Why not use `valuePaidTo` to get the total value paid to the two keys? I think
// it would make the code a lot simpler.
// In general, I think we should avoid explicitly matching on the set of outputs
// in transactions as it makes the validators very inflexible.
// Also, I'm not sure the reclaim case is right. What happens to person 2's funds
// when person 1 reclaims? The trade output has been consumed!
[source,haskell]
----
...
// If anything, this case seems more complicated, because you have to include all the
// code for setting up Fungible everywhere. If you just had the endpoints take two Values
// to trade it would be simpler IMO.
...
// handler1 could use spendScriptOutputs
[source,haskell]
----
