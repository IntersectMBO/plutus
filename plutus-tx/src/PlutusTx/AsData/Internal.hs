...
The workaround are the wrapper functions in this module. `wrapUnsafeDataAsConstr` and
`wrapCaseData` are marked OPAQUE so that GHC doesn't inline it, yet its unfolding is
made available via `-fexpose-all-unfoldings`. `wrapUnsafeDataAsConstr` and `wrapCaseData`
are supposed to be used by AsData only, and not elsewhere, and the Plinth compiler relies
on it to identify which functions are AsData matchers.
...
TODO:
Currently, this optimization only works when each branch handlers are directly
lambda-expressions. When we have non-lambda branch handlers, the inliner is unable to
drop the unused variables. We want to explore there's other approaches to remove unused
field bindings.
-}
