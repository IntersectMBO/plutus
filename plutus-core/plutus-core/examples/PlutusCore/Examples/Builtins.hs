...
instance (Bounded a, Bounded b, Ix a, Ix b) => Ix (Either a b) where
  ...
  unsafeIndex (Left x, n) (Right i) =
    unsafeIndex (x, maxBound) maxBound
      + 1
...
instance
  ( ToBuiltinMeaning uni fun1
  , ToBuiltinMeaning uni fun2
  , Default (BuiltinSemanticsVariant fun1)
  , Default (BuiltinSemanticsVariant fun2)
  )
  => ToBuiltinMeaning uni (Either fun1 fun2)
  where
...
instance
  ...
  => Default (BuiltinSemanticsVariant (Either fun1 fun2))
  where
  def = PairV def def
...
instance
  ...
  => KnownTypeAst tyname uni (MetaForall name a)
  where
...
instance
  (tyname ~ TyName, KnownTypeAst tyname uni a)
  => KnownTypeAst tyname uni (PlcListRep a)
  where
...
instance uni ~ DefaultUni => ToBuiltinMeaning uni ExtensionFun where
  ...
  toBuiltinMeaning _ TrackCosts = unsafePerformIO $ do
    ...
    let
      -- A function to run when GC picks up an individual budget.
      finalize =
