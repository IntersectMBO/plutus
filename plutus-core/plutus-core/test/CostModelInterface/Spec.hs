...
randomCekCosts =
  CekMachineCostsBase
    ...
    , cekPatternCost = pure $ ExBudget 7654321 666
    , cekPatternStructuralCost = pure $ ExBudget 8765432 555
    , cekPatternFailureCost = pure $ ExBudget 9876543 444
    }
...
testDeserialise :: IO ()
testDeserialise =
  assertBool "Failed to decode default ledger cost params" $
    isJust $
...
testApply = do
  ...
  assertBool "Failed to load the ledger cost params into our cost model" $
    isRight $
      applyCostModelParams defaultCekCostModelForTesting decodedParams
...
testMispelled = do
  ...
  assertBool "Failed to catch mispelled cost param" $
    isLeft $
      applyCostModelParams defaultCekCostModelForTesting paramsMispelled
  where
...
test_costModelInterface =
  testGroup
    "cost model interface tests"
    [ testGroup
        "extractCostModelParams works"
    ...
    , testGroup
        "missing entry in params is OK in self-update"
