...
newtype K = UnsafeK {unK :: ByteString}
  deriving newtype (Eq, Ord, Show, Read, Hashable, NFData, NoThunks)
...
newtype Quantity = UnsafeQuantity {unQuantity :: Integer}
  deriving newtype (Eq, Ord, Show, Read, NFData, Hashable, NoThunks)
  deriving stock (Generic, Data)
...
-- The heap of a 'Value' is inspected directly instead of relying on 'NoThunks' instances
-- of the fields (the strict 'IntMap' in particular).
deriving via
  InspectHeapNamed "PlutusCore.Value.Value" Value
  instance
    NoThunks Value

instance Data Value where
  gfoldl app z v = z pack `app` unpack v
  gunfold app z c = case Data.Data.constrIndex c of
    1 -> app (z pack)
    _ -> error "gunfold: Value"
  toConstr _ = packConstr
  dataTypeOf _ = valueDataType

packConstr :: Data.Data.Constr
packConstr = Data.Data.mkConstr valueDataType "pack" [] Data.Data.Prefix

valueDataType :: Data.Data.DataType
valueDataType = Data.Data.mkDataType "PlutusCore.Value.Value" [packConstr]
