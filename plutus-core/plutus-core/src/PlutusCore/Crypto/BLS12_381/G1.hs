...
instance Flat Element where
  ...
  decode =
    fail
      "Flat decoding is not supported for objects of type bls12_381_G1_element: use bls12_381_G1_uncompress on a bytestring instead."

  -- This will be a Haskell runtime error, but encoding doesn't happen on chain,
  -- so it's not too bad.
  encode =
    error
      "Flat encoding is not supported for objects of type bls12_381_G1_element: use bls12_381_G1_compress to obtain a bytestring instead."
  size _ = id
...
scalarMulE x
  | msmScalarOutOfBounds x = const $ fail "Scalar exceeds 512-byte bound for G1.scalarMul"
  | otherwise = pure . (coerce . flip (BlstBindings.blsMult @BlstBindings.Curve1) $ x)
{-# INLINE scalarMulE #-}
...
{-| Multi-scalar multiplication of G1 points.  We limit the allowable size of
