  ...
  prettyBy config (TypeMismatch ann t shapeOrTy ty') =
    "Type mismatch at"
      ...
      <> ( if _pcpoCondensedErrors (_pcpOptions config) == CondensedErrorsYes
            then mempty
            else -- TODO: we should use prettyBy here but the problem is
            -- that `instance PrettyClassic PIR.Term` whereas `instance PrettyPLC PLC.Term`
              hardline <> "Namely," <> hardline <> indent 2 (squotes (pretty t))
         )
...
instance
  ...
  AsNormCheckError (Error uni fun ann) tyname name uni fun ann
  where
