...
fromList xs = do
  ...
  pack <$> validateNestedMap "fromList" outerMap
{-# INLINEABLE fromList #-}
...
validateNestedMap :: String -> NestedMap -> BuiltinResult NestedMap
validateNestedMap context =
  traverse $ traverse \(UnsafeQuantity i) ->
    case quantity i of
      Just q  -> pure q
      Nothing -> fail $ context <> ": quantity out of bounds: " <> show i
{-# INLINEABLE validateNestedMap #-}
...
@a@ contains @b@ if for each @(currency, token, quantity)@ in @b@, if @quantity > 0@, then
@lookup currency token a >= quantity@, and if @quantity < 0@, then
...
unValueData =
  fmap pack . \case
    Map cs -> do
      ...
      -- Validate all quantities are within bounds
      validateNestedMap "unValueData" outerMap
