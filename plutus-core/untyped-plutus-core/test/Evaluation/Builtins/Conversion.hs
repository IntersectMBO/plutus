    ...
    let maxAcceptableInput = 2 ^ (8*integerToByteStringMaximumOutputLength) - 1
        ...
        makeTests endianness =
            ...
            in [
             ...
             testCase (prefix ++ "maximum acceptable input, maximum acceptable length") $
                      let actualExp = mkIterAppNoAnn (builtin () PLC.IntegerToByteString) [
                                       mkConstant @Bool () True,
             ...
             testCase (prefix ++ "input too big, maximum acceptable length") $
                      let actualExp = mkIterAppNoAnn (builtin () PLC.IntegerToByteString) [
                                       mkConstant @Bool () True,
                                       mkConstant @Integer () integerToByteStringMaximumOutputLength,
                                       mkConstant @Integer () (maxAcceptableInput + 1)
                      ...
                      in evaluateShouldFail actualExp,
             -- integerToByteString (maxLen+1) (maxInput+1) fails, just to make
             -- sure we can't go beyond the supposed limit
             testCase (prefix ++ "input zero, length too big") $
                      let actualExp = mkIterAppNoAnn (builtin () PLC.IntegerToByteString) [
                                       mkConstant @Bool () True,
