...
test_ScottToMetaUnit =
    testCase "ScottToMetaUnit" $ do
        ...
        toEvaluationResult (evaluateCekNoEmit runtime (eraseTerm $ applyTerm Scott.map)) @?= res
...
test_SerialiseDataImpossible =
    testCase "Serialising an impossible 'Data' object finishes" $ do
        let dataLoop :: Term TyName Name DefaultUni DefaultFun ()
            ...
            evalRestricting params = toEvaluationResult . fst . runCekNoEmit params budgetMode
