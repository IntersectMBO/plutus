...
test_BuiltinList =
  testCase "BuiltinList" $ do
    let xs = [1 .. 10]
        ...
        term =
          mkIterAppNoAnn
            (mkIterInstNoAnn Builtin.foldrList [integer, integer])
...
test_SwapEls =
  testCase "SwapEls" $ do
    let xs = zip [1 .. 10] $ cycle [False, True]
        ...
        term =
          mkIterAppNoAnn
            (mkIterInstNoAnn Builtin.foldrList [el, integer])
            [ fun
            , mkConstant @Integer () 0
            , mkConstant () xs
            ]
...
test_SerialiseDataImpossible =
  testCase "Serialising an impossible 'Data' object finishes" $ do
    let dataLoop :: Term TyName Name DefaultUni DefaultFun ()
        dataLoop = Apply () (Builtin () SerialiseData) $ mkConstant () loop
          where
            loop = List [loop]
...
test_ConsByteString =
  testCase "ConsVersion" $ do
    let asciiBangWrapped =
        ...
        expr1 =
          mkIterAppNoAnn
            (builtin () (Left ConsByteString :: DefaultFunExt))
    ...
    Right EvaluationFailure
      @=? typecheckEvaluateCekNoEmit
        (PairV DefaultFunSemanticsVariant2 def)
