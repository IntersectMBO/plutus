...
selectedPatternOnly = Cek.monoidalBudgeting $ \category budget -> case category of
  ...
  Cek.BStep Cek.BStructural -> budget
  Cek.BStep Cek.BMatchNext -> budget
  _ -> mempty
...
categoryBudget :: Cek.ExBudgetCategory DefaultFun -> UTerm -> ExBudget
categoryBudget wanted term =
  snd $ Cek.runCekNoEmit unitParameters (selectedCategoryOnly wanted) term

steppableCategoryBudget :: Cek.ExBudgetCategory DefaultFun -> UTerm -> ExBudget
steppableCategoryBudget wanted term =
  snd $ SteppableCek.runCekNoEmit unitParameters (selectedCategoryOnly wanted) term

assertBothCategoryBudget :: ExBudget -> Cek.ExBudgetCategory DefaultFun -> UTerm -> IO ()
assertBothCategoryBudget expected wanted term = do
  categoryBudget wanted term @?= expected
  steppableCategoryBudget wanted term @?= expected
  ...
  testGroup
    ...
    , testCase "failed alternatives are charged separately from pattern inspection" $ do
        let term =
              UPLC.Match
                ...
                (constantInteger 2)
                ( Vector.fromList
                    [ (DefaultPatternInteger 1, constantInteger 1)
                    , (DefaultPatternWildcard, constantInteger 2)
                    ]
                )
        assertBothEvaluateTo (constantInteger 2) term
    ...
    , testCase "fixed-width integer and Data tag patterns have constant scalar cost" $ do
        let integerTerm expected =
            ...
            slippageBoundaryBytes = BS.replicate 1608 97
            bytesTerm bytes =
              matchSingle
        ...
        mapM_
          ...
          (integerTerms <> dataConstrTerms <> byteTerms)
    ...
    , testCase "pattern step accounting cannot pollute the BConst counter" $ do
