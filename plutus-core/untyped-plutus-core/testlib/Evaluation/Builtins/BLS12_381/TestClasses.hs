...
class (Eq a, Show a, Arbitrary a, ArbitraryBuiltin a, DefaultUni `Contains` a) => TestableAbelianGroup a where
  groupName :: String
  zero :: a
...
instance Arbitrary G1.Element where
  arbitrary =
    frequency
      [ (9, arbitraryElement)
...
instance TestableAbelianGroup G1.Element where
  groupName = "G1"
...
instance HashAndCompress G1.Element where
  compressedSize = 48
...
instance Arbitrary G2.Element where
  arbitrary =
    frequency
      [ (9, arbitraryElement)
...
instance TestableAbelianGroup G2.Element where
  groupName = "G2"
...
instance HashAndCompress G2.Element where
  compressedSize = 96
