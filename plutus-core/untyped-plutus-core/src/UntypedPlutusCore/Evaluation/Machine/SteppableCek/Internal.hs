...
data Context uni fun ann
    ...
    | FrameAwaitFunValueN ann !(ArgStack1 uni fun ann) !(Context uni fun ann)
...
computeCek !ctx !env (Constr ann i es) = do
    ...
    pure $ case es of
        ...
        []         -> Returning ctx $ VConstr i EmptyStack0
...
returnCek (FrameConstr ann env i todo done ctx) e = do
    case todo of
        ...
        []             ->
          ...
          in pure $ Returning ctx $ VConstr i (MultiStack0 $ go (LastStack1 e) done)
    ...
    (VConstr i args) -> case (V.!?) cs (fromIntegral i) of
        Just t  ->
          case args of
            ...
            MultiStack0 rest -> computeCek (FrameAwaitFunValueN ann rest ctx) env t
