...
instance ThrowableBuiltins uni fun => MonadError (CekEvaluationException NamedDeBruijn uni fun) (CekM uni fun s) where
  ...
  a `catchError` h = CekM . unsafeIOToST $ aIO `catch` hIO
    where
      aIO = unsafeRunCekM a
      hIO = unsafeRunCekM . h
...
dischargeCekValEnv valEnv = go 0
  where
    ...
    go !lamCnt = \case
      ...
      var@(Var _ (NamedDeBruijn _ ndbnIx)) ->
        let idx = coerce ndbnIx :: Word64
         in if lamCnt >= idx
              then -- the index n is less-than-or-equal than the number of lambdas we have descended
...
lookupVarName varName@(NamedDeBruijn _ varIx) varEnv =
  case varEnv `Env.indexOne` coerce varIx of
    Nothing -> throwingWithCause _MachineError OpenTermEvaluatedMachineError $ Just var
      where
        var = Var () varName
    Just val -> pure val
