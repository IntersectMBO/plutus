...
module UntypedPlutusCore.Transform.Optimizer
  ...
  , pattern Trace.PolyBuiltinStage
...
newtype OptimizerT name uni fun pat ann m a
  = OptimizerT
  { getOptimizerT :: StateT (Trace.OptimizerTrace name uni fun pat ann) m a
  }
  deriving newtype (Functor, Applicative, Monad, MonadTrans)

instance MonadQuote m => MonadQuote (OptimizerT name uni fun pat ann m)

runOptimizerT
  :: OptimizerT name uni fun pat ann m a
  -> m (a, Trace.OptimizerTrace name uni fun pat ann)
runOptimizerT = flip State.runStateT Trace.initOptimizerTrace . getOptimizerT

evalOptimizerT
  :: Monad m => OptimizerT name uni fun pat ann m a -> m a
evalOptimizerT = flip State.evalStateT Trace.initOptimizerTrace . getOptimizerT

execOptimizerT
  :: Monad m
  => OptimizerT name uni fun pat ann m a
  -> m (Trace.OptimizerTrace name uni fun pat ann)
execOptimizerT = flip State.execStateT Trace.initOptimizerTrace . getOptimizerT

type Optimizer name uni fun pat ann = OptimizerT name uni fun pat ann Identity

runOptimizer
  :: Optimizer name uni fun pat ann a
  -> (a, Trace.OptimizerTrace name uni fun pat ann)
runOptimizer = runIdentity . runOptimizerT

evalOptimizer :: Optimizer name uni fun pat ann a -> a
evalOptimizer = runIdentity . evalOptimizerT

execOptimizer
  :: Optimizer name uni fun pat ann a
  -> Trace.OptimizerTrace name uni fun pat ann
...
recordOptimization
  ...
  -> Trace.OptStage
  -> Term name uni fun pat a
  -> OptimizerT name uni fun pat a m ()
