...
cse ::
  ...
  Bool ->
  -- ^ Exclude work-free sub-terms?
  BuiltinSemanticsVariant fun ->
  Term Name uni fun ann ->
  SimplifierT Name uni fun ann m (Term Name uni fun ann)
cse excludeWorkFree builtinSemanticsVariant t0 = do
  t <- rename t0
  let annotated = annotate t
      commonSubexprs =
        ...
        sortOn (Down . termAstSize)
          ...
          . join
          . Map.elems
          $ countOccs excludeWorkFree builtinSemanticsVariant annotated
...
countOccs excludeWorkFree builtinSemanticsVariant = foldrOf termSubtermsDeep addToMap Map.empty
  where
    addToMap ::
      Term name uni fun (Path, ann) ->
      HashMap (Term name uni fun ()) [(Path, Term name uni fun (Path, ann), Int)] ->
      HashMap (Term name uni fun ()) [(Path, Term name uni fun (Path, ann), Int)]
    addToMap t0
      | (excludeWorkFree && isWorkFree builtinSemanticsVariant t0)
        || isBuiltinSaturated t0
        || isForcingBuiltin t0 =
          id
      | otherwise =
          Map.alter
