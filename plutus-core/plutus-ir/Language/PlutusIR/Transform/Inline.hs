...
We *could* do it, but it doesn't seem worth it.
...
data InlineTerm tyname name uni a = Done (Term tyname name uni a)
...
doing something like 'The rapier' section from the paper.
-}
...
leave this to the dead code pass, but we might as well do it here, since it's a very similar decision.
...
processSingleBinding = \case
    ...
    TermBind a s v@(VarDecl _ n _) rhs -> do
        ...
        case maybeRhs' of
            Just rhs' -> pure $ Just $ TermBind a s v rhs'
