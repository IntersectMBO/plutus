...
data Error uni fun a = CompilationError a T.Text -- ^ A generic compilation error.
                     ...
                     | PLCTypeError (PLC.TypeError (PIR.Term PIR.TyName PIR.Name uni fun ()) uni a)
                     | PIRTypeError (TypeErrorExt uni a)
               deriving (Typeable)
makeClassyPrisms ''Error

instance PLC.AsTypeError (Error uni fun a) (PIR.Term PIR.TyName PIR.Name uni fun ()) uni a where
...
instance (PrettyUni uni ann) => Show (Error uni fun ann) where
    show = show . PP.pretty

instance (PrettyUni uni ann, Typeable uni, Typeable fun, Typeable ann) => Exception (Error uni fun ann)
