...
instance
  (PLC.PrettyUni uni, Pretty ann)
  => PrettyBy PLC.PrettyConfigPlc (TypeErrorExt uni ann)
  where
  prettyBy config (MalformedDataConstrResType ann expType) =
    vsep
      [ "The result-type of a dataconstructor is malformed at location" <+> PP.pretty ann
...
instance
  (PLC.PrettyUni uni, Pretty fun, Pretty ann)
  => PrettyBy PLC.PrettyConfigPlc (Error uni fun ann)
  where
