...
-}
newtype TermSubst tyname name uni fun ann = TermSubst {_unTermSubst :: UniqueMap TermUnique (InlineTerm tyname name uni fun ann)}
  deriving newtype (Semigroup, Monoid)

{- | Type substitution, similar to `TermSubst` but for types.
A map of unprocessed type variable and its substitution range.
-}
newtype TypeSubst tyname uni ann = TypeSubst {_unTypeSubst :: UniqueMap TypeUnique (Dupable (Type tyname uni ann))}
  deriving newtype (Semigroup, Monoid)
...
data InlinerState tyname name uni fun ann = InlinerState
  ...
  deriving
    (Semigroup, Monoid)
    via (GenericSemigroupMonoid (InlinerState tyname name uni fun ann))
...
applyTypeSubstitution t =
  gets isTypeSubstEmpty >>= \case
