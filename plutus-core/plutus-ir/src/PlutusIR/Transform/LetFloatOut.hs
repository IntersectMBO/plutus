...
data Pos = Pos
  ...
  -- ^ The lam name or Lam tyname or Let's representative unique
  , _posType :: PosType
  }
...
floatBackLets
  ...
  => term
  -- ^ the cleanedup, reducted term
  -> FloatTable tyname name uni fun a
  -- ^ the lets to be floated
  -> term
  -- ^ the final, floated, and correctly-scoped term
floatBackLets term fTable =
  ...
  where
    ...
    go = \case
      ...
      Let a r bs@(representativeBindingUnique -> letU) tIn -> local (+ 1) $ do
        ...
        bindingGrpToLet NonRec
          <$> floatRhs letU unfloatableGrp -- float inside the rhs of the unfloatable group, and merge the bindings
    ...
    floatRhs
      ...
      => PLC.Unique
      -> grp
      -- \^ the unfloatable group
      -> m grp
    -- \^ the result group extended with the floatable rhs'es (size(result_group) >= size(unfloatable_group))
    floatRhs letU bs = do
    ...
    floatAt
      ...
      -- \^ how to place the unfloatable-group into the PIR result
      -> c
      -- \^ term or bindings to float AROUND
      -> m c
    -- \^ the combined PIR result (terms or bindings)
...
withLam
  :: (r ~ MarkCtx tyname name uni fun a2, MonadReader r m, PLC.HasUnique name unique)
  => name
  -> m a
  -> m a
...
withAbs
  :: (r ~ MarkCtx tyname name uni fun a2, MonadReader r m, PLC.HasUnique tyname unique)
  => tyname
  -> m a
  -> m a
...
floatable (BindingGrp _ _ bs) = do
  ...
  pure $
    all (hasNoEffects binfo vinfo) bs
      &&
