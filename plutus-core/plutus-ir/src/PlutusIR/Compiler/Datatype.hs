...
unveilDatatype dty (Datatype _ tn _ _ _) t =
  let result1 = fromMaybe t $ typeSubstTyNames (\n -> if n == _tyVarDeclName tn then Just dty else Nothing) t
      result2 = typeSubstTyNames' (\n -> if n == _tyVarDeclName tn then Just dty else Nothing) t
