...
writeOutput outp v = do
  case outp of
    NamedOutput file -> T.writeFile file v
    StdOutput -> T.putStr v

{-| Convert a BuiltinCostModel to a Haskell module.
    The module exports a single value of type BuiltinCostModel. -}
costModelToHaskellModule :: FilePath -> BuiltinCostModel -> T.Text
costModelToHaskellModule basename model =
  T.unlines
    ...
    , ""
    , "module PlutusCore.Evaluation.Machine.CostModel.Generated" <> modName <> " where"
    , ""
    , varName <> " = " <> T.pack (show model)
    ]
  where
    modName = T.pack (T.toTile basename)
    varName = T.pack basename
...
main = do
  ...
  let hsModule = costModelToHaskellModule variant model
  writeOutput out hsModule
