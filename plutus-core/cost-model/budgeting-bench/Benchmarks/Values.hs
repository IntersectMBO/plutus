...
Both builtins take a policy list and a `Value`, and pay for the length of the list times the
depth of the `Value`'s outer map. Neither looks inside an inner map, so one model fits both.

Every benchmark case becomes one row of `benching-conway.csv`, named after those two sizes:
