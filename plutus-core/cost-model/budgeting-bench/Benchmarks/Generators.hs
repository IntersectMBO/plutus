...
genBoundedData imem bmem size = genD size
    where genD n =
              ...
              then
                  ...
                  -- We probably will get large lists occasionally, but if we
                  -- generate them too often we get enormous objects.  listOf'
                  -- attempts to give us very occasional large lists.
