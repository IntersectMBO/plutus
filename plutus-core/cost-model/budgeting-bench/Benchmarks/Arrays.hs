...
{- The cost of `multiIndexArray` is expected to be linear in the number of indices
   (y) and independent of the array size (x) and of the element size, since each
   lookup is a constant-time read.  The inputs are chosen so that the benchmark
   data can also demonstrate that independence rather than assume it:
