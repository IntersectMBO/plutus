...
getSampleTermValue
    :: (uni ~ DefaultUni, KnownType (Term TyName Name uni fun ()) a)
    => TermGen uni fun a
...
getSampleProgramAndValue
    :: (uni ~ DefaultUni, KnownType (Term TyName Name uni fun ()) a)
    => TermGen uni fun a -> IO (Program TyName Name uni fun (), EvaluationResult (Term TyName Name uni fun ()))
...
propEvaluate
    ...
    => (Plain Term uni fun -> Either (EvaluationException internal user termErr) (Plain Term uni fun))
       -- ^ An evaluator.
    -> TermGen uni fun a  -- ^ A term/value generator.
    -> Property
