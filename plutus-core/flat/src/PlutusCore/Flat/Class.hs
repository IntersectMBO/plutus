...
#if INL>=2
    -- With these, generated code is optimised for specific data types 
    -- (e.g.: Tree Bool will fuse the code of Tree and Bool)
    -- This can improve performance very significantly (up to 10X) but 
    -- also increases compilation times.
    {-# INLINE size #-}
    {-# INLINE decode #-}
    {-# INLINE encode #-}
#elif INL == 1
    -- INLINE level 1 - moderate inlining
    {-# INLINE size #-}
    {-# INLINE decode #-}
    {-# INLINE encode #-}
#elif INL == 0
...
instance {-# OVERLAPPING #-} (GFlatDecode a, GFlatDecode b) => GFlatDecode (C1 m1 a :+: C1 m2 b) where
      gget = do
        !tag <- dBool
...
instance {-# OVERLAPPING #-} (GFlatDecodeSum n1, GFlatDecodeSum n2, GFlatDecodeSum n3, GFlatDecodeSum n4) 
    => GFlatDecodeSum ((n1 :+: n2) :+: (n3 :+: n4)) where
      getSum cs = do
...
instance {-# OVERLAPPING #-} (GFlatDecodeSum n1, GFlatDecodeSum n2, GFlatDecodeSum n3, GFlatDecodeSum n4,
                               GFlatDecodeSum n5, GFlatDecodeSum n6, GFlatDecodeSum n7, GFlatDecodeSum n8) 
    => GFlatDecodeSum (((n1 :+: n2) :+: (n3 :+: n4)) :+: ((n5 :+: n6) :+: (n7 :+: n8))) where
      getSum cs = do
...
class GFlatSize f where 
    gsize :: NumBits -> f a -> NumBits
...
-- vs proxy implementation: similar compilation time but much better run times 
-- (at least for Tree N, -70%)
#if MIN_VERSION_base(4,9,0)
...
instance (GFlatSizeSum a, GFlatSizeSum b) => GFlatSizeSum (a :+: b) where
