...
module GetOpt
    ( Opts (..)
    , inputs, target, mode, budget, prettyStyle, optimiseLvl, verbosity, lastFileType
    , parseArgs
    , optDescrs
    , GetOpt.usageInfo
    ) where
...
data Opts = Opts
    ...
    , _target       :: SomeFile
    , _mode         :: Mode
    , _budget       :: Maybe ExBudget -- ^ Nothing means: unlimited budget
...
optDescrs =
 ...
 , Option ['V'] ["version"]
   ...
   -- VERBOSITY
 , Option ['q'] ["quiet"]
     (NoArg (set verbosity VQuiet)) "Don't print text (error) output; rely only on exit codes"
 , Option ['v'] ["verbose"]
     (NoArg (set verbosity VDebug)) "Print more than than the default"


 , Option [] ["print-builtins"]
     (NoArg (set mode PrintBuiltins)) "Print the Default universe & builtins"
 , Option [] ["print-cost-model"]
     (NoArg (set mode PrintCostModel)) "Print the cost model of latest Plutus Version as JSON"
 ...
 , Option ['e'] ["example"]
     (OptArg (maybe (set mode ListExamples) (addInput . Example)) "NAME") "Use example NAME as input. Leave out NAME to see the list of examples' names"
 ...
 , Option [] ["stdout"]
     (NoArg $ setOutput StdOut) "Write compiled program to stdout"

   -- OPTIMISATIONS
 , Option ['O'] []
     -- Making -On option also stateful (like -x/-a/-n) does not seem to be worth it.
     (OptArg (set optimiseLvl . maybe SafeOptimise read) "INT") "Set optimisation level; default: 0 , safe optimisations: 1, >=2: unsafe optimisations"
 , Option [] ["whole-opt"]
     (NoArg (set wholeOpt True)) "Run an extra optimisation pass after all inputs are applied together. Ignored if only 1 input given."
 ...
 , Option ['a'] ["ann"]
     (ReqArg (overFileTypeDefault (fLang . ann) read) "ANNOTATION") "Change annotation to `unit` (default) or `srcspan`"

 , Option [] ["run"]
     (NoArg (set mode (Compile Run))) "Compile and run"
 , Option [] ["bench"]
     (OptArg (set mode . Compile . Bench . maybe defBenchSecs read) "SECS") ("Compile then run repeatedly up to these number of seconds (default:" ++ show defBenchSecs ++ ") and print statistics")
 , Option [] ["debug"]
     (OptArg (set mode . Compile . Debug . maybe def read) "INTERFACE") "Compile then Debug program after compilation. Uses a `tui` (default) or a `cli` interface."
 , Option [] ["debug-dir"]
     (OptArg (set debugDir . fromMaybe defDebugDirPath) "DIR") "When `--debug`, try to search for PlutusTx source files in given DIR (default: .)"
 , Option [] ["budget"]
     -- having budget for bench-mode seems silly, but let's allow it for uniformity.
     (ReqArg (set budget . Just . read) "INT,INT") "Set CPU,MEM budget limit. The default is no limit. Only if --run, --bench, or --debug is given"
 ]
