...
data Options = Options
  { optUplcInput :: Input
  -- ^ uplc file or stdin input
  , optUplcInputFormat :: Format
  -- ^ textual or flat format of uplc input
  , optHsDir :: Maybe FilePath
  -- ^ directory to look under for Plutus Tx files
    ...
    OA.optional $
      OA.strOption $
        mconcat
          ...
          , OA.long "hs-dir"
          , OA.help $
              "Directory to look under for Plutus Tx source files."
...
getProgramWithText fmt inp =
  case fmt of
    ...
    Flat flatMode -> do
      ...
      progWithUplcSpan <-
        either (fail . show @ParserErrorBundle) pure $
          runExcept $
            PLC.runQuoteT $
              UPLC.parseProgram progTextPretty
