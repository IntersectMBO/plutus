...
data ScheduledEvent = AD_EVENT {o_rf_CURS :: Double}  -- Analysis Event Retrieves current contract states without alter these
                    ...
                    | PY_EVENT {o_rf_CURS :: Double, rrmo :: Double}   -- Penalty Payment Payment of a penalty (e.g. due to early repayment of principal outstanding)
                    ...
                    | RR_EVENT {o_rf_CURS :: Double}   -- Rate Reset Variable Scheduled rate reset event where new rate is fixed at event time
                    | RRF_EVENT {o_rf_CURS :: Double}   -- Rate Reset Fixed Scheduled rate reset event where new rate is already fixed
                    | SC_EVENT {o_rf_CURS :: Double}   -- Scaling Index Revision Scheduled re-fixing of a scaling index
