...
_STF_PP_PAM st@ContractState{..} pp_payoff _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL =
    let st' = _STF_PY_PAM st _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL
    in st' {nt = nt - pp_payoff}

_STF_PY_PAM st@ContractState{..} _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL = 
    let
        ipac' = ipac + (y _DCC sd t _MD) * ipnr * nt
        fac' = case _FEB of 
...
_STF_FP_PAM st@ContractState{..} _DCC _MD = st {
    ipac = ipac + (y _DCC sd t _MD) * ipnr * nt,
    fac = 0.0,
    sd = t 
}

_STF_PRD_PAM st@ContractState{..} _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL = 
    _STF_PY_PAM st _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL
...
_STF_IP_PAM st@ContractState{..} _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL = 
    let
        st' = _STF_PY_PAM st _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL
    in st' {ipac = 0.0}

_STF_IPCI_PAM st@ContractState{..} _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL = 
    let
        st' = _STF_IP_PAM st _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL
    in st' {nt = nt + ipac + (y _DCC sd t _MD) * nt * ipnr}


_STF_RR_PAM st@ContractState{..} _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL _RRLF _RRLC _RRPC _RRPF _RRMLT _RRSP o_rf_RRMO = 
    let
        st' = _STF_PRD_PAM st _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL
        delta_r = (min (max (o_rf_RRMO * _RRMLT + _RRSP - ipnr) _RRPF) _RRPC)
        ipnr' = (min (max (ipnr + delta_r) _RRLF) _RRLC)
    in st' {ipnr = ipnr'}

_STF_RRF_PAM st@ContractState{..} _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL _RRNXT = 
    let
        st' = _STF_PRD_PAM st _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL
    in st' {ipnr = _RRNXT}

_STF_SC_PAM st@ContractState{..} _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL _SCEF o_rf_SCMO _SCIED = 
    let
        st' = _STF_PY_PAM st _DCC _MD tfp_minus tfp_plus _FEB _FER _CNTRL 
        nsc' = case _SCEF of 
            SE_00M -> nsc 
...
_STF_CE_PAM st@ContractState{..} _DCC _MD = _STF_AD_PAM _DCC _MD