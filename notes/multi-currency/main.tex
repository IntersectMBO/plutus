...
%\footnote{The debt limit could be specified as an additional field when the currency is created (cf. Section \ref{sec:MintingPolicy}).},
...
%For example, in the case of Bitcoin, the policy is that a coinbase transaction must mint an exact number of bitcoins determined by the block number and any other transaction may not mint any bitcoin.
...
The \emph{multi-currency ledger} model defined here allows the creation of mutually independent native currencies with pre-established minting policies. The design goals of simplicity and minimalism have been achieved through:
\begin{enumerate}
...
\item a transaction type for creating currency and establishing its minting policy.
\end{enumerate}
